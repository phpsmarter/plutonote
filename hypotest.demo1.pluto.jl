...
# ╔═╡ 8117ecb8-1847-11ed-3498-9d718149df99
begin
	using CSV, Distributions, HypothesisTests,HTTP,DataFrames,Statistics,Plots,Latexify,StatsBase,RCall
end

# ╔═╡ 5dcccd8d-5de8-4ade-8085-e4006877250a
begin
	url="http://127.0.0.1:8080/machine1.csv"
	sigma = 1.2
     mu0A, mu0B = 52.2, 53
	getdata(url)=HTTP.get(url).body
    csv(data)=CSV.read(data,DataFrame)
	data=getdata(url)|>csv|>eachcol|>first
	xbar,stdev,n=mean(data),std(data),length(data)
    OneSampleTTest(xbar,sigma,n,mu0A )
end

# ╔═╡ ddfe78b2-21eb-4d0a-8bb7-2b14865983be
let
	# statistics with julia 的数据
	getdata(url)=HTTP.get(url).body
    csv(data)=CSV.read(data,DataFrame)
    retrivedata(url)=getdata(url)|>csv|>eachcol|>first
	getStat(data)=[mean(data),std(data),length(data)]
	urls=["http://127.0.0.1:8080/machine$(i).csv" for i in 1:3]
	data=[retrivedata(url) for url in urls]
	
	allData=vcat(data...)
    gmean,gstd,gn=getStat(allData)   #全局的统计数据
	singleStat=[getStat(test)  for test  in data]  #单次实验的统计数据
	#data1=singleStat[1]
	#z=zscore!(data[1], data1[1], data1[2])
	#scatter(z,frame=:origin)
	[describe(data[1])]
	#OneWayANOVATest(data...)
end

# ╔═╡ bf84b5c5-8c65-45c0-83b2-e6d429e8c992
let
	data1=[5,5, 4, 4 ,3]
	data2=[5, 4, 4, 3, 3]
	data3=[4,3 ,2, 2,1]
	df = DataFrame(five=data1, eight=data2,tweleve=data3)
    #OneWayANOVATest(eachcol(df)...)
	
end

# ╔═╡ 2a367108-6e96-4cae-b502-ee5cfff1cec7
let
    getdata(url)=HTTP.get(url).body
    csv(data)=CSV.read(data,DataFrame)
...
# ╔═╡ 5676cd27-9025-49ff-9b10-79de916baf12
let
...
# ╔═╡ 3cfb6752-e397-458b-b2ff-6477fd6f5736
let
    getdata(url)=HTTP.get(url).body
    csv(data)=CSV.read(data,DataFrame)
	
    retrivedata(url)=getdata(url)|>csv
    url="http://192.168.1.4:8080/iris.csv"
	iris=retrivedata(url)
    @rput iris
	R"""
	 model <- manova(cbind(sepal_length, sepal_width) ~ species, data = iris)
	 summary(model )
	 summary.aov(model )
    """
...
# ╔═╡ 123b927c-0b61-43f8-8ea6-a9b362f7061e
let
	dict=Dict("rep" => [120, 110],
                   "dem" => [90,95],
	    "ind"=>[40,45]
	)
	...
	df = DataFrame(sex=sex; rep=rep,dem=dem,ind=ind)
    
end

# ╔═╡ 5c70d650-34ff-43bf-8e2f-a16eef14be34
let
	matrix=[120 90 40; 110 95 45]
	df=DataFrame(matrix,["rep","dem","ind"])	
    ChisqTest(matrix)
end

# ╔═╡ 88d0a504-fc44-4fde-98a5-68f326761119
let
	matrix=[35 50 15;30 25 45 ;65 75 60 ]
	df=DataFrame(matrix,["psy","eng","bio"])	
    ChisqTest(matrix)
end
...
manifest_format = "2.0"
project_hash = "a76caea3d7085e73ca84c0c90848a5f48e6ba6d5"
