...
# ╔═╡ 339f5fe2-e71a-11ec-0dac-eb0ca3fc316e
begin
...
# ╔═╡ e6b70f89-b1e8-4e2d-bf48-1fd046b87e9d
PlutoUI.Resource("https://tva1.sinaimg.cn/thumbnail/e6c9d24egy1h2alsw1tzxj20m80gomxn.jpg")

# ╔═╡ 0eabf004-1124-4c1b-b7e7-d3e495593fda
md"# ch05 sec5.4  定积分定理"

# ╔═╡ be7d9148-459b-423f-885f-035c011bd50f
md"""
!!! theorem
    积分极限的定理5.2:

    $1. \int_{a}^{b}f(x)dx=-\int_{a}^{b}f(x)dx$

    $2.  \int_{a}^{c}f(x)dx+\int_{c}^{b}f(x)dx=\int_{a}^{b}f(x)dx$

"""

# ╔═╡ 7e947972-e384-4317-917a-5d364c47fa75
md"""
!!! theorem
    积分极限的定理5.3:
    ...
    $2.  \int_{a}^{c}cf(x)dx=c\int_{a}^{b}f(x)dx$

"""

# ╔═╡ 239c1498-ca55-4f6e-98ac-4bd7dbdd451c
md"""
!!! example
    example1 :估计 $\int_{0}^{2}(1+3x)dx$ 的值

利用加法和乘法定理:

$\int_{0}^{2}(1+3x)dx=\int_{0}^{2}1dx+3\int_{0}^{2}xdx$

"""

# ╔═╡ 868c1eff-1e1b-46f2-a019-95b54f3d7fcd
let
     f1(x)=1
	 f2(x)=x
	 f(x)=f1+f2
     tspan=0:0.02:2
	  p1=areaplot(f1,tspan, lable=L"f(x)=1",frame=:semi)
	  p2=areaplot(f2,tspan, lable=L"f(x)=x",frame=:semi)
	  plot!(p1,p2,frame=:zerolines,ylims=(0,2))
end

# ╔═╡ 31a90b10-0efb-4b49-ae51-ae697ddf41cc
md"""
## 曲线之间的面积

!!! note
    曲线之间的面积是积分之差
    对于 $x \in [a,b], f 和 g 之间的面积表示为:$

    $AD=\int_{a}^{b}(f(x)-g(x))dx$
...
# ╔═╡ df07aab0-6c27-4430-847c-57de6896c09d
let
	 f(x)=x^2-4x+5
	 g(x)=-x^2+4x-1
	 ...
	 tspan=0:0.2:4
	 tspan2=1:0.1:3
	 #low=[[t f(t)] for t in tspan2]
	 #upper=[[t g(t)] for t in tspan2]
	 plot([f,g],tspan,label=[L"f(x)=x^2-4x+5" L"g(x)=x^2+4x-1"],frame=:semi)
	 
	 
end

# ╔═╡ 6f9a8310-2a08-49a7-aa3b-d52464c0e649
md"""
## 用函数对称性求积分

!!! note
    如果函数是偶函数, 关于$y$轴对称, 那么有:
...
!!! example
    example 4
     ...
     $\int_{0}^{π}sin(t)dt=2$,求$\int_{-π}^{π}sin(t)dt$,$\int_{-π}^{π}\left | sint \right |dt$
    
"""

# ╔═╡ 4b8bc116-0341-4f80-a9d3-131b4b75821d
let
	tspan=-π:0.02:π
	f1(t)=sin(t)
	f2(t)=abs(sin(t))
	areaplot([f1,f2],tspan, layout=(1,2), label=[L"sint" L"\left|sint \right|"],frame=:zerolines,size=(600,250),legend=:bottomright)
end

# ╔═╡ aa89c95a-23bc-475c-9f2c-2ffcef85012b
md"""
可以看到 $\int_{-π}^{π}sin(t)dt=0$

而 $\int_{-π}^{π}\left | sint \right |dt=2\int_{0}^{π}sin(t)dt=4$
"""

# ╔═╡ 1f850314-0e42-4220-bd9e-8a7f092584f9
md"""
## 比较积分

有时会对于复杂的函数变化, 我们可以简单的变化形式设定上界和下界来估计它的积分. 
"""

# ╔═╡ a20df8f5-bd9c-42ad-a199-7217e0c9f109
let
	f(x)=sin(x)
	fm(x)=0.5
	fM(x)=1.2
	tspan=0.5:0.02:2.6
	ann=[
		(1.5,0.25,text("m",pointsize=10)),
		(1.5,0.55,text("f(x)",pointsize=12)),
		(1.5,1.25,text("M",pointsize=15))
	]
	areaplot(fM,tspan,label=false,ann=ann)
	areaplot!(f,tspan, frame=[:zerolines,:semi],label=false,xlims=(0,3),alpha=0.5)
	areaplot!(fm,tspan,label=false,alpha=0.5)
	
end

# ╔═╡ 75c7616b-7daf-4d96-9a66-8f9f05001aeb
md"""
  上图中, 红色部分的积分面积比 $m$ 大, 比$M$ 小
"""

# ╔═╡ 8aaef75e-6fc3-489d-8c88-2cc0430b72d2
begin
...
# ╔═╡ 9e2b2a3d-bd90-460f-91ad-81f3cc7869eb
let
	f1(x)=1
	f2(x)=x
	a=0
	b=2
	n=500
    sum1=getRiemannSum(a,b,n,f1)
	
    sum2=getRiemannSum(a,b,n,f2)

	total=sum1["rightsums"]+3*sum2["rightsums"]
	@show total
end

# ╔═╡ 77eb09af-ec23-4fd5-89dd-e86bd3bbd7f6
let
	a=1  #由两个函数相等求出
	b=3  #由两个函数相等求出
	n=250
	...
	f2(x)=-x^2+4x-1
	diff(x)=f2(x)-f1(x)
	sum=getRiemannSum(a,b,250,diff)
    @show sum
end

# ╔═╡ e6a4e34b-326b-4762-ae84-c6ab38714f69
