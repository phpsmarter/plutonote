...
# ╔═╡ 85cfd99e-e491-11ec-2f16-1dbeae93473f
begin
...
# ╔═╡ 34ffb8d6-1fb5-425d-ad08-411f9ea6928e
PlutoUI.Resource("https://tva1.sinaimg.cn/thumbnail/e6c9d24egy1h2alsw1tzxj20m80gomxn.jpg")

# ╔═╡ da5f6044-0b2f-40b1-9aa3-bacdedc7ea39
md"# ch03 section 3.2 指数函数的导数"

# ╔═╡ a8af85d7-a296-4fb9-8673-a82f7534dfae
md"""
!!! info
     指数函数$f(x)=a^x$的导数一般形式表示为: $f'(x)=k\cdot a^x$, 因为在指数函数的导函数与原函数成一定比例关系.因此一旦我们获得了导函数, 也就可以可以得到原函数. 这是微分方程建模的最简单的原理.所以指数函数
"""

# ╔═╡ 60704da2-fef6-4f2e-a90d-dffdb209f8f0
md"""
## 指数函数导函数和 常数 **ℯ** 的来历

### 代数计算 $g(x)=2^x$ 的导函数

$$g'(x)=\lim_{h \to 0}(\frac{2^{x+h}-2^x}{h})=\lim_{h \to 0}2^x(\frac{2^h-1}{h})=\lim_{h \to 0}(\frac{2^h-1}{h})\cdot 2^x$$

 令 $k=\lim_{h \to 0}(\frac{2^h-1}{h})$, 则 $g'(x)=k\cdot 2^x$ 这个极限值与指数函数的底数是由关系的. 这点很重要
 下面看看$h$ 取不同步长的极限值

"""

# ╔═╡ d777e83d-933e-4774-830a-821d8ac67553
let
	harr=[-0.1,-0.01,-0.001,0.001,0.01,0.1]
	f(h)=(2^h-1)/h
	limarr=[f(h) for h in harr]
	df=DataFrame(;h=harr,k=limarr)
	
end

# ╔═╡ 56f91baf-8947-4ea5-8003-0b6c1c6dc17b
md"
  $k=0.693     \therefore g'(x)=0.693\cdot2^x$

指数函数一般导函数定义为:

$$g'(x)=\lim_{h \to 0}(\frac{a^h-1}{h})\cdot a^x$$

不同底数的指数函数$k$ 值计算如下:
"

# ╔═╡ 55d0dbbd-3f4b-43d1-871f-2382063bfb11
let
	acollection=2:1:10
    h=0.001
    f(a)=(a^h-1)/h  #利用极小的 h值,计算不同底数的k 值
    karray=[f(a) for a in acollection]
	df=DataFrame(;a=acollection,k=karray)
end

# ╔═╡ be225695-42e6-4a26-8bc3-88b6f4e12a6e
md"""
在上面表中可以观察到当$a=3$ 时, $k$ 似乎接近于 $1$, 由于 $1$ 在数学中是一个很特别的数字, 那么我们来看看到底 $a=?$  可以让$k$  近似等于 1

$$k=\lim_{h \to 0}(\frac{a^h-1}{h})=1$$

$$\frac{a^h-1}{h} \approx 1$$

$$a^h-1 \approx h 所以  a\approx (1+h)^{1/h}$$

依照这个公式来计算$k=1$ 是 $a$ 的近似值

"""

# ╔═╡ cb9872c3-a632-40c6-9d1a-b989a7a59231
let
	harr=[-0.1,-0.01,-0.001,-0.00001,0.00001,0.001,0.01,0.1]
    f(h)=(1+h)^(1/h)
	acollection=[f(h) for h in harr]
    df=DataFrame(;h=harr,a=acollection)
end

# ╔═╡ 0c862f8e-3465-4a3c-a18a-b9f9a613f599
md"
  当 $a=2.718$ 时, $k$值近似等于 1

  这个$a$ 就是 欧拉常数: **$ℯ$**

  以 $ℯ$ 为底数的指数函数的导数就是他自己
...
# ╔═╡ dfd1f796-387c-4f65-9328-2172983054aa
let
	f(x)=2^x
	df(x)=(0.693)*2^x
	g(x)=3^x
	...
	e(x)=ℯ^x
	de(x)=e(x)
    pf=plot([f,df], label=[L"2^x" L"(0.693)2^x"])
	pe=plot([e,de],label=[L"e^x" L"e^x"])
	pg=plot([g,dg],label=[L"3^x" L"(1.1)2^x"])
	plot(pf,pe,pg,frame=:zerolines,layout=(1,3),size=(900,300))
end
