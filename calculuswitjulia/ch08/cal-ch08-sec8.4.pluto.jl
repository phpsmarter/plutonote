...
# ╔═╡ f32b4f86-e96a-11ec-3be6-559cf45af777
begin
...
# ╔═╡ 6535ab74-c3cd-49a7-929d-d91f15dd22ba
PlutoUI.Resource("https://tva1.sinaimg.cn/thumbnail/e6c9d24egy1h2alsw1tzxj20m80gomxn.jpg")


# ╔═╡ 32ebe3c5-a145-4e80-902a-e12e3db71b96
md"""
# ch08 sec8.4 密度和质心
"""

# ╔═╡ 4a65fc82-3bda-498b-823e-2ef0560546a3
md"""
## 密度问题以及如何对区域进行分割

密度问题有很多类型, 例如:

1. 二维的种群密度情况, 单位面积内的人口数量 单位 $个/平方公里^2$

2. 三维情况, 发酵罐了的酵母菌的密度 单位 $个/升$

3. 物质密度, 例如金属的密度都不同,单位为 $g/厘米^3$
...
# ╔═╡ b116fbec-900e-4b8e-a98f-4275e61b12e7
md"""
!!! example
    example 2,2

    1. 空气的密度是高度的函数,海平面以上  $\rho=f(h) kg/m^3$ ,求从海平面到高空25km处, 直径为 $2m$的圆柱体内的空气重量的表达式
    2. 如果密度函数为$P=1.28e^−0.000124h$, 求$1.$ 中圆柱体内空气的重量. 
...
![](https://tva1.sinaimg.cn/orj360/e6c9d24egy1h354c9jsq2j20j60figmz.jpg)

所以圆柱体的体积为:

$πr^2\Delta h,半径为 1, 所以体积为:\pi \Delta h  m^3$
...
# ╔═╡ 11905a43-2d3a-46b4-b1bf-2c79c0862142
let
	"""
...
# ╔═╡ 5418699d-3c98-457a-a5a3-c870aff39ad3
let
...
# ╔═╡ 91a64f08-4309-475c-b4e1-5827e98ee2a6
md"""
!!! example
    example 4
...
![](https://tva1.sinaimg.cn/orj180/e6c9d24egy1h355jz9rnqj20fa0i2ta8.jpg)

例如我们在半径为$2$的位置,画一个圆环, 当$\Delta r$足够小的时候,可以认为在半径方向人口密度均一,乘以圆环面积,就可以近似表示圆环内的人口, 累积所有小的圆环就得到总人口数量

可以近似把圆环拉直看成矩形, 宽度为圆环的周长, 高度为$\Delta r$

![](https://tva1.sinaimg.cn/mw690/e6c9d24egy1h356iap62tj20so05edga.jpg)
...
$圆环面积=π(r+\Delta r)^2-πr^2$

展开化简为:

$2πr\Delta r+π(\Delta r)^2$

当$\Delta r \to 0$ ,$π(\Delta r)^2$ 忽略不计, 和前面的结果一致
	
"""

# ╔═╡ 1a4e3a7f-7d55-4b2f-80d3-30896ad8c17b
let
...
# ╔═╡ 7f46c6b7-5239-42f1-9b68-7e80aedb1ea7
let
	gr()
	Δr=0.5  #用于绘制近似矩形
	radius=2
	...
	rectheight=Δr
	ann=[
		(7.3,0.8,text(L"|\leftarrow----------2\pi r- ----------\rightarrow|",pointsize=10,color=:red)),
		(1+rectwidth-0.1,1+(Δr/2),text(L"height=Δr",pointsize=10,color=:red,halign=:right))
	]
	plot([1,1+rectwidth,1+rectwidth,1,1],[1,1,1+rectheight,1+rectheight,1],label=false, color=:blue, lw=1,ylims=(0,5),frame=:zerolines,ann=ann)
	
end

# ╔═╡ 45863814-8c69-434c-b30f-b01431d2b072
md"""
## 质心
...
$2m\overline{--l/3--\bigtriangleup  ----l2/3---- } m$
...
力矩代表了整个系统绕支点旋转的趋势, 	如果系统内$总力矩=0$, 跷跷板就不会旋转.
...
![](https://tva1.sinaimg.cn/mw690/e6c9d24egy1h359jvrdwbj20rq036q35.jpg)
...
$\bar x=\frac{m_1x_1+m_2x_2+m_3x_3}{m_1+m_2+m_3}=\frac{\sum_{i=1}^{3}m_ix_i}{\sum_{i=1}^{3}m_i}$


所以可以归纳为:
"""

# ╔═╡ 829a04c7-cf67-4798-b633-2288d5e45809
md"""
!!! theorem
    在有$n$ 个质量点$m_1,m_2,...,m_n$的系统中, 每个质量点位置为$x_i$,质点的位置表示为:
...
# ╔═╡ 99f581a2-2b6b-4791-8f7d-30ff2ba06332
let
	gr()
	Δr=0.2  #用于绘制近似矩形
	radius=2
...
# ╔═╡ 380d969d-64f2-43ad-a766-a52bec8a287b
md"""
!!! example
    example6 
    用一般表达式计算 example5的实例中的质心

假设起点位置在跷跷板左侧, 因此大孩子的位置为$0$, 小孩子的位置为$l$, 总质量为$3m$, 带入公式

$\bar x=\frac{\sum x_im_i}{\sum m_i}=\frac{1}{3m}(2m\cdot 0+ m \cdot l)=\frac{ml}{3m}=\frac{l}{3}$


"""

# ╔═╡ 1bec1797-0da4-4058-a0b8-dc9bf0a60c39
md"""
### 连续质量密度的质心
![](https://tva1.sinaimg.cn/large/e6c9d24egy1h35bsh9rmuj20ts08i0tc.jpg)

跷跷板本身的质量是一个连续的系统, 可以为认为看做有$n$个小孩子坐在跷跷板上,每个小孩子质量不同,  每个小孩都会产生一个力矩.

假设密度是长度的函数,定义为:$δ(x)$,跷跷板内的质量分布是不均匀的, 

为了计算质心, 我们也取其中宽度为$Δx$ 的一小段, 当$Δx \to 0$ 时, 可以近似认为密度均匀的.
...
# ╔═╡ b8c6722f-60a8-4ad3-ae2f-4542eedb91ed
md"""
!!! theorem
    一个物体沿着轴$x$,$a,b$两点之间的质心$\bar x$ 表示为:
 
    $\bar x=\frac{\int_{a}^{b} x_i\delta(x_i)dx}{\int_{a}^{b} \delta(x_i)dx}$

      $\delta(x)$ 物体的密度函数, 自变量是物体宽度的不同位置
"""

# ╔═╡ 60432fc8-8849-4b34-a0c9-0d2c6bfbe7c7
let
	...
	ann=[
		 getannstr(7.0,0.9,"xᵢ"),
		 getannstr(7.0,1.8,"Δx",:red),
		 getannstr(7.0,1.7,"|←--------→|",),
		 getannstr(0.8,0.9,"a",:black),
		 getannstr(13.2,0.9,"b",:black),
		 (11,2,text(L"Mass: m_i \approx \delta(x_i)\Delta x",color=:black,pointsize=12,)),
		getannstr(15.2,1.25,"x",:black),
	]

	linz=[rand(x*2) for x in 1:5]
	plot([1,1+rectwidth,1+rectwidth,1,1],[1,1,1+rectheight,1+rectheight,1],label=false, lw=1,ylims=(0,3),frame=:zerolines,ann=ann,linecolor = :rainbow,linz=linz)
...
# ╔═╡ b8543627-8d34-4779-8a54-cc54d1840c25
md"""
!!! example
    example7  
...
$\bar x=\frac{\int_{0}^{2}̧x \cdot \frac{5}{2}dx}{5}=1$
...
$mass=\int_{0}^{2}15x^2dx$

用黎曼和公式计算为: 约为:$40kg$

杆的总动量为:

$moment=\int_{0}^{2}x15x^2dx$
...
# ╔═╡ 1b16443a-391d-4edf-91f2-13618e72f747
md"""
### 二维和三维区域的质心
...
表达式 $\delta A_x(x)Δx$表示垂直于$x$ 轴切片的动量. 这里目前假设密度均一.如果密度不均一需要二重和三重积分

"""

# ╔═╡ 2c844014-c717-4cc6-a118-1025c2a69381
md"""
!!! example 
    example8

    求等腰三角形的质心, 密度为均一的, 质量为$m$,底边长为$1$,高为$1$

因为质量均匀, 而且为等腰三角形,所以 在 y轴方向的质心是$\bar y=0$

三角形面积为$1/2 底 \cdot 高= 1/2 \cdot  1 \cdot 1=1/2$

密度为质量除以面积:$\rho=2m$,如果沿$x$轴, 每$\Delta x$宽度切片,为小的梯形

由于有相似三角形 :$\frac{底边}{底边}=\frac{高}{高}$

可知 $\frac{切片高度}{三角形底边}=\frac{1-x}{x}$

所以切片高度为$1-x$

当 $\Delta x \to 0$ , 切片梯形近似为矩形,面积为:
...
# ╔═╡ 80c35e5d-b505-4698-9536-b0206d31ba3e
let
	Δx=0.1
  function getannstr(x,y,str,color=:green)   
		   return (x,y,text(str,pointsize=10,color=color,halign=:center))
 ...
 ann2=[
  ...
  p1= plot([0,1,0,0],[-0.5,0, 0.5,-0.5], frame=:zerolines, ticks=false, lw=1, label=false,size=(300,300),ann=ann1)
  pslice=plot([0.25,0.25+Δx,0.25+Δx,0.25,0.25],[-0.38,-0.38,0.38,0.38,-0.38],label=false, lw=0.5)
...
# ╔═╡ 4f92f331-1849-4d0f-8c09-c3590f7b27dd
md"""
!!! example 
    example 9
...
根据描述绘图
    ![](https://tva1.sinaimg.cn/mw690/e6c9d24egy1h35p9q4zguj20ng0ii0ta.jpg)

由于半球的对称性, 在$x,y$方向的质心都在圆心位置. 不均匀的是在$z$轴上, 越靠近半球中心, $z$ 横切面的半径越大, 质量越大, 所以质心靠近底部. 变化部分在$z$ 轴, 所以我们在$z$ 轴进行切割.

在高度为$z$ 的位置切片, 厚度为$Δz$,切片是一个圆盘,  有上图所示的三角形, 斜边为半球半径, 一条直角边为切片的高度, 另一个直角边为切面的半径, 这是我们需要的值:

$A_z(z)(圆盘半径)=\sqrt{半球半径^2-切片高度^2}$

当$Δz \to 0$时, 近似为一个圆柱体,圆柱体体积表示如下:

$Vol = A_z(z)Δz \approx π(7^2-z^2)Δz$
...
$mass=\frac{2}{3}\pi r^3 δ$

根据之前定义的质心坐标公式:

$\bar z=\frac{\int z\delta A_z(z) dz}{mass}=\frac{\int z\delta A_z(z) dz}{\frac{2}{3}\pi r^3 δ}$


下面用黎曼和公式计算:
"""

# ╔═╡ 33168cf1-8957-4841-a74c-cf63bad74feb
let
	plotly()
	height=0.5
	Δx=0.02
...
# ╔═╡ 81432922-1a4d-4bfe-83d0-3eeec8b00f40
...
# ╔═╡ c9dcf24c-3c94-45f3-a98b-e51a5cbdbcd2
let
	a,b,n=0,25000,1000000
	p(h)= pi*1.28*ℯ^(−0.000124h)	
	res=getRiemannSum(a,b,n,p)
end

# ╔═╡ dfbeb26a-0030-4d13-b3ea-77bbbaf3f92c
let
	a,b,n=0,2,500
	δm(x)=15x^2
	Δmoment(x)=x*15x^2
...
# ╔═╡ a39aa207-c5e9-4ec4-a8ab-9b55bdc56c1e
let
	radius=7
	a,b,n=0,radius,500
...
# ╔═╡ f16b29b8-2f93-4f30-a6a5-31829fc442d3
@htl("""<script src="https://cdn.bootcdn.net/ajax/libs/mathjax/3.2.0/es5/tex-svg-full.min.js"></script>
