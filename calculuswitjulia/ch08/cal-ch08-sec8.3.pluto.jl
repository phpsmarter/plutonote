...
# ╔═╡ f189499c-e917-11ec-330e-d5eb711f77b0
begin
...
# ╔═╡ a13843c7-c3cd-45c5-804d-56abbe9901e4
PlutoUI.Resource("https://tva1.sinaimg.cn/thumbnail
/e6c9d24egy1h349b6on33j20m00l0dhk.jpg")


# ╔═╡ 4426b7d8-18a7-4937-bc4d-d607352f1fc0
md"""# 面积和弧线的极坐标表示

!!! info
    在某个时刻,空间点的位置是一种物理性质, 例如在太阳系中, 不同行星在某个时间点与太阳之间的关系都不同,  为了度量位置这个物理信息, 有多种方法.

    我们最早在几何中接触的是笛卡尔坐标系,也就是直角坐标系. 还有其他的坐标系,比如极坐标系. 要想知道为什么会存在不同的坐标系同,必须深刻理解函数的意义. 

    翻来覆去要认识的就是函数的定义: 需要一个定义域, 一个值域和一套规则.  同一个定义域的值通过不同的规则映射到不同的值域里,度量的值会不同, 因为函数规则不同,**不同规则**获取的值不能进行比较. 

    为什么需要不同的规则? 没有其他原因, 对于有些系统一种规则获取的数值理解起来容易, 但是在另一个系统就会带来很大的困扰. 
    这个定义看似复杂, 其实日常生活中我们一直在使用, 并且这套方法论的起源是和数学诞生同步的, 这个我们会在数学和认识板块讲.  
...
!!! example
    **英制和公制的系统关系**
    ...
    身高为:6英尺,2英寸, 这是英制度量值,我并不知道到库里到底矮到什么程度. 经过*坐标变换*,转化为公制,身高为:1米88,因为这是我们日常使用的公制系统, 这时候我才真正的理解,确实是"矮"的离谱.
...
# ╔═╡ 7503ffa2-8a3e-4154-aed3-b54ef871f1e8
md"""
 ###  直角坐标系和极坐标之间的关系
...
$\binom{x=rcosθ}{y=rsinθ}$

$\binom{r=\sqrt{x^2+y^2}}{tanθ=\frac{x}{y}}$
...
# ╔═╡ 13d080af-fb26-4a2e-9546-7276647889f1
let
   xoffset=0.1
   yoffset=0.1
   ...
   p4=plot!([x(angle),x(angle)],[y(angle)/2+annoffset,y(angle)-annoffset],arrow=0.2,color=:blue,lw=1,label=false)
   p5=plot!([x(angle),x(angle)],[y(angle)/2-annoffset,annoffset],arrow=0.2,color=:blue,lw=1,label=false)
    p6=plot!([0.5*x(angle)-annoffset,0],[-0.1,-0.1],arrow=0.2,color=:blue,lw=1,label=false)
    p7=plot!([0.5*x(angle)+annoffset,x(angle)],[-0.1,-0.1],arrow=0.2,color=:blue,lw=1,label=false)
	
end

# ╔═╡ 8acfb1af-23b1-4ab4-8d30-25f1c89b3a33
md"""
!!! example
    example 2
    ...
    ![](https://tva1.sinaimg.cn/small/e6c9d24egy1h349b6on33j20m00l0dhk.jpg)

    因为角度有周期性$2k*\pi+1/4pi$ 的角度都会重复, 结果可以是
    
   $\{(2k+1/4)\pi,3 \}, k \in [0,1,2]$ 
"""

# ╔═╡ 5a71c5ea-f3dc-47c6-a37a-ee9a21d97d74
let
	...
	plot!([1/4*pi],[3],proj = :polar, m=6,label="P")
	
	plot!(plotarr...,size=(360,360))
	
end

# ╔═╡ b473c887-89b7-420b-a567-28c2cd9e9219
md"""
## 用极坐标表示方程图形
...
# ╔═╡ 49f79cc0-fab7-427c-8666-5202ac01d094
md"""
!!! example
    example 3
...
直角坐标和极坐标的转换关系之一为: $y=rsinθ$,所以根据提示有$rsinθ=1$,所以$r=1/sin\theta$
$y=1$ 在直角坐标下下表示为 $1=\sqrt{x^2+y^2}$,就是单位圆. 在极坐标中画出也是单位圆. 


如下图
"""

# ╔═╡ 45528170-f7bb-4906-a031-8949f5b26893
let
...
# ╔═╡ 1ca7daea-4c06-4a42-b979-fc641b40fad4
md"""
...
# ╔═╡ b1898c72-91b0-4a62-94fc-3163a38bd231
let
...
# ╔═╡ 39151a26-f0e0-40c8-b972-5ee74b9705e0
md"""
...
# ╔═╡ 7b32ab46-7bbe-46db-9b10-e91eb23a5907
let
	anglecollection = range(0, stop= 2*π,length= 300)
...
# ╔═╡ c0c90588-67e1-4d77-bbb0-525126ae7534
let
	
	anglerange = range(0, stop= 2*π,length= 300)
	getradius1(θ)=3
	getradius2(θ)=1
	getangle1(r)=0
	getangle2(r)=1/4*pi
...
# ╔═╡ 3723634f-36a3-41a1-9e44-9386c4cd6d25
md"""
## 极坐标系中的面积计算
...
$sector=\frac{θ}{2π} \cdot \pi r^2=\frac{1}{2}r^2θ$


"""

# ╔═╡ 39aead2a-dd3e-4444-bbb3-7515866cb77e
md"""
!!! example
    example9

    使用上面的扇形面积公式,结合定积分计算 由 轮廓线方程 $r=3+2cos\theta$ 围成的面积

  将半径表达式带入扇形的面积公式:
   
$扇形面积 \approx \frac{1}{2}r^2 Δθ= \frac{1}{2}(3+2cosθ)Δθ$

为获取整个围成的面积,对扇形面积求和:

$\sum  \frac{1}{2}(3+2cosθ)Δθ$

求化简并求积分

$\frac{1}{2}\int_{0}^{2π}(9+12cosθ+4cos^2θ)dθ$
...
# ╔═╡ a1f1d7d1-f0b2-445b-966d-05d6f884eb6d
md"""
!!! note
    对于一般有轮廓线方程:$r=f(θ)$的曲线, 一个角度区间内轮廓线下的面积可由定积分求得,公式如下:

    $轮廓线下面积=\frac{1}{2}\int_{\alpha}^{\beta}f(\theta)^2d\theta$
"""

# ╔═╡ 36088167-6138-4771-9a05-4380f915fd98
md"""
## 用极坐标表示曲线在某点的斜率
...
# ╔═╡ fa21481a-9fa7-4b17-a026-a04495b7aeec
md"""
## 用极坐标表示弧线长度
...
# ╔═╡ ee5085a3-5364-436a-9019-3df7a884f8bb
...
# ╔═╡ 84aa3964-b9d8-454b-80aa-6ca9e72e463b
let
	a,b,n=0,2pi,500
...
# ╔═╡ 53fb82a9-ac98-4d50-bbb7-6ebfc724d1f1
@htl("""<script src="https://cdn.bootcdn.net/ajax/libs/mathjax/3.2.0/es5/tex-svg-full.min.js"></script>
