...
# ╔═╡ 18cd6e1d-5185-4ee1-9119-aa35333773a6
begin
...
# ╔═╡ 3f0e88a5-00e1-4c1b-827c-4ea153458d7c
PlutoUI.Resource("https://tva1.sinaimg.cn/thumbnail/e6c9d24egy1h2alsw1tzxj20m80gomxn.jpg")


# ╔═╡ 94c273b4-ed4f-11ec-2776-f5743376ed06
md"""
# ch11 sec11.1  什么是微分方程?
"""

# ╔═╡ 57bc03a4-5155-4d0c-9eea-2714052f24ae
md"""
在微积分中我们一直在研究变化问题, 借助微分方法研究函数的变化行为.我们在日常生活中, 可以感受到变化,比如环境光线的明暗交替变换, 季节的变化. 有些科学家很好奇, 这些变化背后是不是有什么规律. 是不是有迹可循.  这就是微分方程方法诞生的初衷. 

经过不断的发展, 微分方程决绝的问题越来越复杂, 揭示变化问题也越多. 


可以这么说是微分方程推动了数学的发展. 尽管数学有纯数学和应用数学, 但是因为有实践的需求数学才开始迅速发展. 微分方程是一门实践的学科.  

随着数据科学的发展, 微分方程的发展也更为快速, 结合了深度神经网络的微分方程应用规模更大, 速度更快, 解算精度也更高,由此微分方程能够解决的问题也就更多了.  


所以在学习数据科学数学基础内容时, 同时了解数据科学在微分方程方面的应用也是必要的. 

那么就首先看看最简单的微分方程的面貌. 

"""

# ╔═╡ f475ff57-e444-4574-89b7-701c9539369f
md"""
## 微分方程从变化开始
...
有一个理论认为, 一个人知道的东西越多, 它的学习速度就会变得更慢. 似乎很有道理, 和我们吃饭一样, 当我们快吃饱的时候, 吃饭速度就会比饿着的时候减慢. 如何用数学形式来表现这个问题?

我们先从假设开始:

假设总的学习任务为$100 \%$,已经学会的内容表示为 $y \%$, 学习内容是时间$t$的函数,比如重头开始学习一项性的技术, 那么有:

$y(t_0)=0 ,t_0=0,表示还没有学习到任何内容$  

$y(t_1)=y \% , t=t_1 表示在t1时刻已经学到了y(t_1)的内容$

从 $t_0 \to t_1$ 时间, 学习的变化率为:

$\frac{y(t_1)-y(t_0)}{t_1-t_0}$

变化率为学习到的内容除以时间间隔. 着就是前面讲的在某时间点附近的变化问题.  可以表示为:

$\frac{dy}{dt}$

由此我们获得了一个度量学习新技术的工具, 结合上面的理论, 学习内容越多,学习速度会减慢, 也就是$\frac{dy}{dt}$,会随时间轴的延长而减小. 


### 构造微分方程

在前面我们提到过, 在解决具体问题之前, 最好能画一个草图, 对于函数变化问题也一样, 我们最好能粗略的获得一个表达式,然后慢慢完善. 

假设明天就要考试, 你还有很多内容没有学, 那么从现在起到明天早晨这段时间,你希望的变化率是在这段时间把所有没有学的内容学完.  

用一个简单的表达式表示为:

$\frac{dy}{dt}=100-y$

虽然现在我们还没办法借解这个微分方程, 但是的确我们已经定义了了一个微分方程. 
"""

# ╔═╡ 05a6c57e-fa50-4b83-9465-74f244141285
md"""

## 一阶和二阶微分方程

微分方程的定义和函数的导数密切相关, 所以先来看看导数
...
# ╔═╡ c9a59356-2a03-4ef3-9ccc-5279ccfd51b2
