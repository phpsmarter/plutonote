...
# ╔═╡ fa7f6559-6539-450b-af6b-222250857feb
begin
...
# ╔═╡ 1626cb28-62eb-4db7-a267-4ffbbd570922
PlutoUI.Resource("https://tva1.sinaimg.cn/thumbnail/e6c9d24egy1h2alsw1tzxj20m80gomxn.jpg")


# ╔═╡ f50f875a-f021-11ec-0822-b9f09354b634
md"""
# ch11 sec11.4   分离变量法解微分方程


"""

# ╔═╡ b4d5f871-e61d-46bd-8e38-17c5d0ef141b
md"""
 这一小节在使用计算机进行数值计算的时候不会遇到, 但是分离变量的方法从分析角度对加深微分的概念有好处,所以也需要了解一下.

微分的符号使用,例如:

$\frac{dy}{dx}=\frac{-x}{y}$

  $dy,dx$ 代表了$x,y$的微小变化, 其实和我们编程中使用的其他变量一样, 可以在符号计算中进行各种代数操作,例如移项操作. 这是分离变量方法的依据 

对于上述微分方程, 我们前面已经了解过了, 这是圆轨迹中的变化情况, 原始函数是:

$x^2+y^2=C$

解微分方程就是要试图从微分方程中求出代表的原始方程是什么.
...
# ╔═╡ 0c4805fc-6f10-46a2-8c42-fcf3718a35a0
md"""
...
!!! example
    ...
    用分离变量法解微分方程: $\frac{dy}{dx}=ky$


等式两边同乘以$dx$,同时除以$\frac{1}{y}$,等式变形为:
...
对于有$𝑑𝑦∕𝑑𝑥 = 𝑘𝑦$ 形式的微分方程,总是可以获得 $y=(\pm A) ℯ^{kx}$ 这样的解. 微分方程中的$k$ 值揭示了函数的行为. 当$k>0$  为指数增长型函数. 当$k<0$ 为指数衰减型.

当$k$值固定, $B$值也会影响函数的行为.
...
# ╔═╡ 8cb0a5fc-1950-48ff-af3d-6a62641d5b00
let
	k=2.0
	tspan=-0.5:0.02:1
	Bcollection=-3:3
	...
	funccollection=[expo(B) for B in Bcollection]

	function getlabel(arr)
           label=[]
		   
		for (i, b) in enumerate(Bcollection)
			if  i==1
			    label=L"B=%$(b)"
			else
				label=hcat(label, L"B=%$(b)")
			end
		end
	...
	label=getlabel(Bcollection)

	p1=plot(funccollection,tspan, label=label,frame=:zerolines,legend=:bottomleft,title=L" k=2, \ B=[-3,3]")
	save("plot1", p1)
   
end

# ╔═╡ fb7fd097-8fb0-43b3-8af1-4e1df4acf677
md"""
!!! example

     example 2

     当 $k>0$,找到$\frac{dH}{dt}=-k(H-20)$ 的图形解
...
$|H-20|=e^{-kt+C}=e^{-kt}e^C=Ae^{-kt}$

所以有:

$H-20=(\pm A)e^{-kt}=20+Be^{-kt}, \ B=\pm A$

当$k>0,  B 值会决定函数的行为.如图, 背景为向量场图形$

$(store["plot2"])

"""

# ╔═╡ dc4f9c25-4d15-4ab7-a69b-db06528b9058
let 
    ...
    us =  scale*vec([x for (x, y) = Iterators.product(xspan, yspan)])
	vs =  scale*vec([df(y) for (x, y) = Iterators.product(xspan, yspan)])
	quiver(xs,ys , quiver = (us, vs),label=false,frame=:zerolines)
	p2=plot!([f1,f2,f3],xspan, lw=3,label=[L"H=20+10*e^{-t}" L"H=20" L"H=20-10*e^{-t}"])
	save("plot2",p2)

end

# ╔═╡ b3241ace-9956-4e3b-97f8-da7f0fa57db9
md"""
!!! example

     example 3

     $当 t=0, P=5$,找到$\frac{dP}{dt}=2P-2Pt$ 的图形解$

分离变量, 求积分得:

$\int \frac{dP}{P}=\int (2-2t)dt$
...
$|P|=e^{-t^2+2t+C}=e^Ce^{-t^2+2t}$

因为设 $A=e^C$, 所有$A>0$, 方程改写为:

$P=Be^{-t^2+2t}$

这是通解, 带入初始条件$当 t=0, P=5$:

$5=Be^{2\cdot 0-0^2}=B$

所以初值 $当 t=0, P=5$ 的微分方程特解为:
...
# ╔═╡ ee13945f-760d-481e-93aa-fa0795afa6fc
let
	tspan=0:0.02:2.2
	p(t)=5*ℯ^(2*t-t^2)
    p3=plot(p, tspan, label=L"P=5e^{2t-t^2}",lw=1, frame=:zerolines)
	save("plot3",p3)
end
