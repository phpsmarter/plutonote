...
# ╔═╡ 9188946e-ed84-11ec-1061-5f2e8dd7e15f
begin
        using PlutoUI,DataFrames,HypertextLiteral,LaTeXStrings,Symbolics, CairoMakie,GLMakie
    GLMakie.activate!()
...
# ╔═╡ 4c985228-6c05-43c9-8bc9-1e219fa8df2f
PlutoUI.Resource("https://tva1.sinaimg.cn/thumbnail/e6c9d24egy1h2alsw1tzxj20m80gomxn.jpg")


# ╔═╡ ecca5a40-4778-42df-9fff-904a263243d8
md"""
# ch11 sec11.2  斜率场
"""

# ╔═╡ 9c0f545c-a6bc-4dd6-85ac-bbad7c60afe3
begin
	store=Dict()

	function save(key::String, dict)
		return merge!(store,Dict(key=>dict))
	end
...
# ╔═╡ da94c798-1470-430f-b0ac-30a743667a5b
md"""

首先来看一下 $\frac{dy}{dx}=y$  的斜率场

下面绘制了上述导数的斜率场
$(store["fields1"])

导数的意义是当$x$ 变化$dx$个单位的时候, $y$ 如何变化.  对于空间的一个坐标$\{x,y\}$, $y$分量就是随着$x$ 变化时 $y$ 的变化.  如果画一条水平直线($y=1$), 那么这个直线上的所有点的斜率都是一样的,因为所有的$y$ 值相同,  当$y$ 越靠近$0$, 变化趋势越小,远离$0$ 点, 变化速度越快

**注意,这里实际画的是向量场, 斜率是一致的, 但是当$x$ 取正值和负值的时候,$y$的方向是相反的**
"""

# ╔═╡ f08bb702-96d7-45a7-a8a8-77c2cf11d059
let
	#   dy/dx=y 的向量场
    xs = LinRange(-3, 3, 20)
    ys = LinRange(-3, 3, 20)
    ...
    arrows!(ax, xs, ys, us, vs, arrowsize = 5, lengthscale = 0.1,
...
# ╔═╡ d654c7b1-c202-4e9a-97e3-243a5d4d4e49
md"""
!!! example 

    example 1

    根据微分方程 $dy/dx=2x$ 的斜率场 描述
...
从微分方程可以看到,$y$ 的变化是和$x$有关的, 变化率为$2x$,所以在$x$ 越大的位置, 变化率越大. 
但是在同一$x$ 值的垂直线上, 所有点的斜率是一样的. 如上图里红色线直线所示
...
因为对导函数$dy/dx=2x$ 求不定积分结果如下:

$y=\int 2xdx=x^2+C$

所以这个斜率场描述的就是函数 $f(x)=x^2+C$ 曲线簇各个点的斜率变化

"""

# ╔═╡ c29d4c42-7f7f-4a19-82a9-2b9d12e20cd2
let
	# 𝑓(𝑥,𝑦)=𝑥2+2𝑦2
    xs = LinRange(-3, 3, 20)
    ys = LinRange(-3, 3, 20)
    us = [x for x in xs, y in ys]
    vs = [(2x)*x for x in xs, y in ys]
	strength = vec(sqrt.(us .^ 2 .+ vs .^ 2))
    cmap = :gnuplot
    ...
    arrows!(ax, xs, ys, us, vs, arrowsize = 5, lengthscale = 0.1,
        arrowcolor = strength, linecolor = strength, colormap = cmap)
	vlines!(2.15,lw=1,color=:red)
	limits!(ax, -3,3,-3,3)
    colsize!(fig.layout, 1, Aspect(1, 1.0))
    save("fields2",fig)
end

# ╔═╡ 6e72df80-8252-4392-85f6-e83745a0b82d
let
	# 𝑓(𝑥,𝑦)=𝑥2+2𝑦2
    xs = LinRange(-3, 3, 20)
    ys = LinRange(-3, 3, 20)
    ...
    arrows!(ax, xs, ys, us, vs, arrowsize = 5, lengthscale = 0.1,
        arrowcolor = strength, linecolor = strength, colormap = cmap)
	lines!(ax, xs, x -> x^2; color = :black,lw=1)
	lines!(ax, xs, x -> x^2-1; color = :black,lw=1)
	lines!(ax, xs, x -> x^2-2; color = :black,lw=1)
	lines!(ax, xs, x -> x^2+1; color = :black,lw=1)
    limits!(ax, -3,3,-3,3)
    colsize!(fig.layout, 1, Aspect(1, 1.0))
     save("fields3",fig)
...
# ╔═╡ caca3f49-b9c6-4d6e-a6b6-706c9d395b90
md"""
!!! example 

    example 2

    绘制微分方程 $\frac{dy}{dx}=\frac{-x}{y}$ 


 $(store["fields4"])

 这个向量场描述的是同心圆的变化情况, 我们叠加一个半径为$2$ 的圆, 看看

 $(store["fields5"])

对圆的方程求导也可以得到 $\frac{dy}{dx}=\frac{-x}{y}$ ,具体推导参看 page 592

"""

# ╔═╡ 380b4c1c-1805-4f4a-9368-6092179bbd58
md"""
上面的斜率场图都是一组函数簇的斜率组成的, 怎么确定我们想要的某个特定方程?  这时我们需要更多的信息, 
在微分方程中, 这些信息被称为初值. 当我们把一组初始的输入,输出值带入微分方程的通解, 就可以确定函数的最终形式. 

!!! example

    example 3 

    导数为 $dy/dt=2-y$ 和$dy/dt=t/y$ 的斜率场如下图:
	- a. 根据初始条件绘出特解曲线:
	    - (i). 当$t=0$时, $y=1$ 
...
# ╔═╡ ffe7bbaa-f8a8-455b-b478-eb47d41930ce
let
	# 𝑓(𝑥,𝑦)=𝑥2+𝑦2
    xs = LinRange(-3, 3, 20)
    ys = LinRange(-3, 3, 20)
    us = [x for x in xs, y in ys]
    vs = [(-x/y)*x for x in xs, y in ys]
	strength = vec(sqrt.(us .^ 2 .+ vs .^ 2))
    cmap = :gnuplot
    fig = Figure(resolution = (600, 400))
    ax = Axis(fig[1,1], xlabel = "x", ylabel = "y", aspect = DataAspect())
...
# ╔═╡ 420b88c5-4312-4697-8e6d-f103ae87628a
let
	# 𝑓(𝑥,𝑦)=𝑥2+𝑦2
	tspan=-2pi:0.02:2pi
	radius=2
	rx=[radius*cos(t) for t in tspan]
	ry=[radius*sin(t) for t in tspan]
    xs = LinRange(-3, 3, 20)
    ys = LinRange(-3, 3, 20)
    us = [x for x in xs, y in ys]
    vs = [(-x/y)*x for x in xs, y in ys]
    ...
    arrows!(ax, xs, ys, us, vs, arrowsize = 5, lengthscale = 0.1,
        arrowcolor = strength, linecolor = strength, colormap = cmap)
	lines!(rx,ry, lw=1, color=:red)
	limits!(ax, -3,3,-3,3)
    colsize!(fig.layout, 1, Aspect(1, 1.0))
    save("fields5",fig)
end

# ╔═╡ fa6b4751-0674-402e-adb2-47157a35e23e
let
	xs1,ys1 = LinRange(-4, 4, 20),LinRange(-1, 6, 20)
    xs2,ys2 = LinRange(-5, 5, 20),LinRange(-5, 5, 20)
    us1 = [x for x in xs1, y in ys1]
    vs1 = [(2-y)*x for x in xs1, y in ys1]
	us2 = [x for x in xs2, y in ys2]
    vs2 = [(x/y)*x for x in xs2, y in ys2]
	
    cmap = :gnuplot
    fig = Figure(resolution = (800, 400))
    ax1 = Axis(fig[1,1], xlabel = "t", ylabel = "y", aspect = DataAspect())
	ax2 = Axis(fig[1,2], xlabel = "t", ylabel = "y", aspect = DataAspect())
    arrows!(ax1, xs1, ys1, us1, vs1, arrowsize = 5, lengthscale = 0.1,
        arrowcolor =:green, linecolor = :green, colormap = cmap)
	arrows!(ax2, xs2, ys2, us2, vs2, arrowsize = 5, lengthscale = 0.1,
        arrowcolor =:green, linecolor = :green, colormap = cmap)
...
# ╔═╡ a474c332-1a0d-4670-af20-e06d28a9873a
...
git-tree-sha1 = "81c394f2863855e8f317f2bc24ff1d4a456bacc6"
