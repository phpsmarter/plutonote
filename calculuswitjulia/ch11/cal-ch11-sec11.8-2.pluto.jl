...
# ╔═╡ 6a5ca80c-f20d-11ec-1b1c-3f8862099470
begin
...
# ╔═╡ f18d6c06-7f62-4ff4-9f61-aa8f42bf0e07
PlutoUI.Resource("https://tva1.sinaimg.cn/thumbnail/e6c9d24egy1h2alsw1tzxj20m80gomxn.jpg")


# ╔═╡ fc23c31d-548e-4827-bcfb-1f06f4cecc79

md"""
# ch11 sec11.8  微分方程组
"""

# ╔═╡ b7c3ea13-69f7-4bd8-ab16-c74d87fb8e8c
begin
	store=Dict()
...
# ╔═╡ 69b7711f-2d39-4d1f-b22c-55f7b3119664
md"""
##  交互作用-捕食与被捕食系统

种群之间的交互有几种类型, 有的种群之间会竞争食物, 有的是捕食与被捕食关系, 有的会形成共生关系, 彼此互相帮助,  捕食者-被捕食者系统的微分方程组称为 **洛特卡－沃尔泰拉方程（Lotka-Volterra equations）**

### 知更鸟与蚯蚓

看看一个假象的系统,  知更鸟作为捕猎者, 蚯蚓作为猎物. 知更鸟数量单位为 $r$,量级为$1$千,蚯蚓数量为$w$,量级为$1$百万.

如果没有捕食者, 蚯蚓的数量呈指数级增长:

$\frac{dw}{dt}=aw,\ a为>0 的常数$

如果森林里没有蚯蚓, 知更鸟的数量会呈指数级下降:

$\frac{dr}{dt}=-br,\ b 是>0的常数$


当两个种群有了交互作用, 变化率就就会改变, 对于蚯蚓, 由于捕食者存在, 增长率会下降, 对于知更鸟,由于有事物, 增长率会变为正值

$\frac{dw}{dt}=aw+知更鸟的作用$
$\frac{dr}{dt}=-br+蚯蚓的贡献$

当知更鸟在森林中发现蚯蚓时, 蚯蚓要么被吃掉, 要么逃跑, 定义一个常数$c$ 表示当蚯蚓遇到知更鸟是被吃掉的情况,这会使得蚯蚓的增长速度变慢:

$frac{dw}{dt}=aw-cwr$

对于知更鸟而言, 遇到蚯蚓可能捕食成功,也可能失败, 用常数$k$ 表示这种情况:

$\frac{dr}{dt}=-br+kwr$

为了容易理解模型, 我们设常数都为 1,$a=b=c=k=1$
...
$\frac{dr}{ds}=\frac{-r+wr}{w-wr}$


根据两个种群的变化率公式,画出斜率场, 如下图

 $(store["wr"])

图中$[1,1]$  是一个平衡点, 当蚯蚓的数量为$1$ 百万条时, 知更鸟的数量会保持静止,当蚯蚓的数量大于$1$百万时 当知更鸟的数量大于 1000, 会降低到 1000, 小于 1000 时,数量会增加.  

这里的变化是大致的概念, 由于系数没有实验数据求解,所以与实际是由差异的. 

"""

# ╔═╡ 302fa9ed-4c53-4942-b5ae-2462ace1e49c
let 
	scale=0.02
	df(w,r)=(-r+(w*r))/(w-(w*r))
	wspan=0:0.2:4
	rspan=0:0.2:4
	xs = vec([w for (w, r) = Iterators.product(wspan, rspan)])
    ys = vec([r for (w, r) = Iterators.product(wspan, rspan)])
    us =  scale*vec([w for (w, r) = Iterators.product(wspan, rspan)])
	vs =  scale*vec([df(w,r) for (w, r) = Iterators.product(wspan, rspan)])
	p1=quiver(xs,ys , quiver = (us, vs),label=false,frame=:zerolines ,xlabel="worm",ylabel="robin",lw=1,arrow=arrow(:headwidth, 1.0))
	save("wr",p1)
end


# ╔═╡ 0de3dd3a-4411-45ac-ba46-2256ba281fa2
@htl("""
