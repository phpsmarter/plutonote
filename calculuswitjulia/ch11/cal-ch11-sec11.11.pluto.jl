...
# ╔═╡ c00e05d8-e201-4125-8a48-1a30ff25d899
begin
...
# ╔═╡ ce4807a0-41ac-450d-a6db-04d0521a8c9d
PlutoUI.Resource("https://tva1.sinaimg.cn/thumbnail/e6c9d24egy1h2alsw1tzxj20m80gomxn.jpg")


# ╔═╡ 39c13e80-f2a1-11ec-02a4-e35132f8becf
md"""
# ch11 sec11.11 线性二阶微分方程
"""

# ╔═╡ 7fc71b85-ea40-48bf-911a-311d2e69ba1e
md"""
## 有阻力的弹簧:阻尼震荡

现实中的弹簧震荡都有一定的阻力, 空气中的弹簧悬挂物体随着运动速度变化,空气阻力会变化. 

汽车减震器内的弹簧工作在粘性很大的油中, 阻尼的衰减作用很大. 为了研究现实的震荡问题, 需要在简谐运动的微分方程中添加阻尼项 $a(ds/dt)$

$a$ 称为阻尼系数, 震荡微分方程改写为:

$- \ ks \ -a\frac{ds}{dt}=m\frac{d^2s}{dt^2}$

$ks 为弹簧形变产生的力,a\frac{ds}{dt} 为阻力,\frac{d^2s}{dt^2} 为加速度$
...
!!! definition
    ...
    $\frac{d^2s}{dt^2}+\frac{a}{m}\frac{ds}{dt}+\frac{k}{m}s=0$
...
# ╔═╡ 5ce542f0-21c8-4556-a1b6-6db5bf08fd20
md"""
## 线性微分方程的通解

上面定义的有阻尼震荡的微分方程是线性微分方程的一个例子,一般的线性微分方程有如下形式:

$\frac{d^2y}{dt}+b\frac{dy}{dt}+cy , \ b, c为常数$

在弹簧的实例可以看到,如果两个函数$f_1,f_2$满足微分方程条件,  通解可以写为两个函数的线性组合形式:

$y(t)=C1f_1(t)+C_2f_2(t)$

"""

# ╔═╡ c4cec318-23ff-4e3a-9128-75d29acb5900
md"""
## 线性微分方程的解: 特征方程

现在用复数形式来求解这个微分方程:

$\frac{d^2y}{dt}+b\frac{dy}{dt}+cy$

仍然采用假设-验证的方法, 推测微分方程的原函数为:

$y=Cℯ^{rt}$

将原函数带入微分方程得到:

$r^2Cℯ^{rt}+b\cdot rCℯ^{rt}+c \cdot Cℯ^{rt}=Cℯ^{rt}(r^2+br+c)=0$

假设 $C \neq 0, ℯ^{rt} >0$,那么方程中的系数项$r^2+br+c=0$


这个二项式称为微分方程的特征方程, 特征方程的解为:

$r=-\frac{1}{2}b \pm \frac{1}{2}\sqrt{b^2-4c}$


特征方程的解有三种类型, 依赖于$b^2-4c$的符号

"""

# ╔═╡ 47741036-d0ef-4279-a672-6e2e361ac1be
md"""
### 当 $b^2-4c>0$

特征方程有两个实数根,$r_1,r_2$ 原函数为两个指数函数的线性组合:

$y(t)=C_{1}ℯ^{r_1t}+C_{2}ℯ^{r_2t}$

当$r_1,r_2 都 <0$,则函数整个是衰减的, 称为过阻尼震荡 ,物体的位移会快速的衰减
"""

# ╔═╡ c73db64a-7b1d-4d70-926a-faa61c6ef702
md"""
### 当 $b^2-4c=0$
...
# ╔═╡ 95f3d069-dcd7-479e-85e0-a3c376cc3cc4
md"""
### 当 $b^2-4c<0$

特征方程有复根,根据欧拉方程,复根会变形得到三角函数, 就得到周期性震荡函数
"""

# ╔═╡ 35781d47-2bd0-4162-8d36-658ca588ecec
md"""
!!! example

    example 2

    弹簧上挂载质量为$m=10kg$的重物, 弹簧系数为$k=20 kg/sec^2$,物体有阻尼, 阻尼系数为$a=20 kg/sec$, $t_0$ 时刻, 弹簧拉升至静息位置以下$2m$ 位置.
...
# ╔═╡ b91d89ad-fab3-402f-ab6b-96c6773a9d3c
md"""
!!! example
...
特征方程为:$r^2+2r+2=0$,有两个共轭复根为:$r=-1 \pm i$

微分方程的解写为:

$s(t)=A_1e^{(-1+i)t}+A_2e^{(-1-i)t}$

带入初始值, 初始位移和导数求解 $A_1,A_2$

得到 $A_1=1-i,A_2=1+i$

因此特解为:

$s(t)=(1-i)e^{(-1+i)t}+(1+i)e^{(-1-i)t}$

为了便于立即周期性的变化, 变形为三角函数形式比较有利, 因此带入欧拉方程. 

$e^{it}=cost+isint, e^{-it}=cost-isint$

化简得到:

$s(t)=2e^{-t}cost+2e^{-t}sint$


三角函数项表示了周期性特征, 指数项指出了弹簧的衰减特征
...
!!! definition
    ...
    $\frac{d^2y}{dt}+b\frac{dy}{dt}+cy$ 可得到:
    特征方程 $r^2+br+c=0$ 根为:$r=\alpha \pm i \beta$

    通解为:

    $y=C_1e^{\alpha t}cos\beta t+C_2e^{\alpha t} sin\beta t$
    如果$a<0$,称为欠阻尼震荡, 如果$a=0$, 称为无阻尼震荡

"""

# ╔═╡ 9f408ffe-22d6-4bfb-9839-a35ba5597b8a
md"""
!!! example
    example 4
...
(a)  特征方程为$r^2-9=0$,所以有$r=\pm 3$,通解为:

$y=C_1e^{3t}+C_2e{-3t}$

(b)  特征方程为$r^2+9=0$,所以有$r=0+\pm 3i$,通解为:

$y=C_1cos3t+C_2sin3t$
"""

# ╔═╡ ff3f24f8-0ebc-4536-b7ed-e39eeeb6098b
md"""

!!! summary
    微分方程:$y''+by'+cy=0 $的解为:

    如果 $b^2-4ac>0$,则有$\ y=C_1e^{r_1t}+C_2e^{r_2t}$

    如果 $b^2-4ac=0$,则有$\ y=(C_1t+C_2)e^{-bt/2}$

    如果 $b^2-4ac<0$,则有$\ y=C_1e^{\alpha t}cos\beta t+C_2e^{\alpha t}sin\beta t$

"""

# ╔═╡ f2572edd-2d67-40b6-bd76-d61c38ab1695
begin
	store=Dict()
...
# ╔═╡ 42c66ae8-c390-4fe9-aa1e-b6ea7aa27721
md"""
...
!!! example
    ...
    $\frac{d^2s}{dt^2}+3\frac{ds}{dt}+2s=0$
    求解原方程, 初始值为$s=-0.5, t=0时,ds/dt=3$


特征方程为:

$r^2+3r+2=0, 两个实根为:r_1=-1, r_2=-2$
...
$s=C_1e^{-0}+C_2e^{-2(0)}=C_1+C_2=-0.5$

对函数求导得:

$-C_1e^{-0}-2C_2e^{-2(0)}=-C_1-2C_2=3$

解方程组可得 $C_1=2, C_2=-2.5$
...
下面是物体的位移图像:
 ...
 $(store["overdumpspring"])

"""

# ╔═╡ 97d600da-731f-45e5-8c01-1a49ee113112
let
	tspan=0:0.1:5
	s(t)=2*ℯ^(-t)-2.5*ℯ^(-2t)
    overdump=plot(s,tspan ,lw=2, frame=:zerolines,label=L"s(t)=2e^{-t}-2.5e^{-2t}",
	title=L"\frac{d^2s}{dt^2}+3\frac{ds}{dt}+2s=0  \ solution",legend=:bottomright,
	xlabel=L"t",ylabel=L"s")

	save("overdumpspring",overdump)
end

# ╔═╡ 2294bc4f-a112-4737-a7d4-35475a9affae
@htl("""
