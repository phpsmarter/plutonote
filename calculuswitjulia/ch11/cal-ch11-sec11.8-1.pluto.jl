...
# ╔═╡ 2e773b50-f1e9-11ec-2465-034e64f7302a
begin
...
# ╔═╡ 3be45714-c43d-400e-a47b-1fcd2c2d4b94
PlutoUI.Resource("https://tva1.sinaimg.cn/thumbnail/e6c9d24egy1h2alsw1tzxj20m80gomxn.jpg")


# ╔═╡ ba0b2026-590b-49d3-af06-056989062948
md"""
# ch11 sec11.8  微分方程组
"""

# ╔═╡ 8e40a4ed-66e6-4c1e-9b5f-a28c66313875
md"""

#  传染病实例
  1978 年一月, 英国一个寄宿制学校763 名学生刚放完寒假返校, 一周之后, 第一名学生出现流感症状, 第二天有两个学生出现症状, 第一个月里几乎一半的学生都出现了流感症状. 绝大多数的学校学生在二月中旬都被感染. 这里文字描述了传染速度的变化情况, 为了更为明晰的说明问题, 研究者引入了很多模型, 其中 $S.I.R$模型是常用的. 

 # S.I.R 模型

在流行病领域常用的模型是 S.I.R  模型,  在疾病传染的过程中, 人群分为三类:

-  易感人群(S), 还没有被传染, 但是处于被感染范围
-  感染人数(R), 已经被感染的人群
-  被传染过(I), 但是已经康复额人群


当传染继续的时候, 易感人群数量会减少, 因为会变为感染者. 
我们假设传染的速度与易感人群和感染人群的接触关系成比例

$\frac{dS}{dt}=-aSI$

添加负号的原因是,随着传染延续, 易感人群数量会减少,但是传染的速度是正值


传染的速度变化由两方面决定, 1是易感人群变为患病的速度. 2, 患病者逐渐康复的速度,康复速度定义常数$b$
...
如果总人口短时间不变, 那么$S+R+I$ 是一个常数, 只需要知道其中两个, 剩下一个就是确定的.因此就可以关注两个方程,

$\frac{dS}{dt}=-aSI$
$\frac{dI}{dt}=aSI-bI$

根据之前的初始和演化数据, 我们可以确定常数 $a,b$

看看之前寄宿制学校的数据

当第一个感染症状学生出现时, 三个分类的变化:

$S=I,\ S=762,\ 这种情况下,第二天出现症状为 2$,所以有:

$a=-\frac{dS/dt}{SI}=2/(762)(1)=0.0026$

常数$b$ 代表了从出现症状者到康复的比例, 根据调查, 感染学生里每天有一般的人会恢复, 所有估算$b\ approx 0.5$


由此方程变形为:

$\frac{dS}{dt}=-0.0026SI$
$\frac{dI}{dt}=0.0026SI-0.5I$


## 下面我们来看看传染期间的向平面问题
传染的变化由易感人群的变化和感染人数的变化共同决定. 
...
$\frac{dI}{dS}=\frac{0.0026SI-0.5I}{-0.0026SI}$

化简为:

$\frac{dI}{dS}=-1+\frac{192}{S}$
...
# ╔═╡ 85342eeb-989f-4b2a-81da-f72b9d6da450
	

# ╔═╡ cfd2e6d9-ed54-4898-8f98-a3f7a7645599
begin
	struct  point
		x::Float16
		y::Float16
...
# ╔═╡ 71220d64-62ae-4ceb-afc5-e82d61ed1682
begin
	store=Dict()
...
# ╔═╡ 244b7f6d-f728-4fda-a56b-d3ff2bb2b012
@htl("""
