...
# ╔═╡ dbf5b790-09ad-43c7-95e4-9b81afafd4a0
begin
...
# ╔═╡ ef0d5666-84c4-11ec-3f94-b9838edee326
md"# Function Family"

# ╔═╡ d845727c-425d-4a7e-80cc-5dbcc499026f
let
	p = plot(xlabel="x", ylabel="y(x)", ylim=(-3,3))
    res=[]  
	la=["x"  "x²"  "x³" "x⁴"  "x⁵"]
	 range=(-3:0.01:3)  # 值域
	 func_step=1:5      # 函数的power的离散值
		for p in func_step  # 遍历
		  results =map(x->x^p,range)	 # p表示指数
	      push!(res,results)             # x区间对应的y值 压入数组
		end
	  
	plot!(range,res,framestyle=:origin,label=la)
end
