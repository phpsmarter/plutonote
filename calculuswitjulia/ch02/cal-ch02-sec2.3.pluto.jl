...
# ╔═╡ 19419244-e2c6-11ec-3ba5-0d81178fc112
begin
...
# ╔═╡ c1d01a21-7dc7-46fa-93b8-792d2696e914
PlutoUI.Resource("https://tva1.sinaimg.cn/thumbnail/e6c9d24egy1h2alsw1tzxj20m80gomxn.jpg")

# ╔═╡ 4c780018-0573-4cd5-a8d5-cc2378ed7c1d
md"""
#  ch02 sec 2.3  导函数
"""

# ╔═╡ 531ddb9c-9100-4cc6-b8ae-5df583305569
md"""
**example1** 计算并绘制 $f(x)$ 在 −2, −1, 0, 1, 2, 3, 4, 5 

这里实际是绘制斜率场, 按照一定间隔绘制出斜率的图形, 会大致直观表示函数的变化特性
"""

# ╔═╡ 12304757-3246-4875-91f9-15ecf0026568
let
	tspan=-2.2:0.02:5.2
	spot=[−2, −1, 0, 1, 2, 3, 4, 5 ]
	f(x)=4.2*cos(x+0.2)  # 并不知道原函数是什么, 所以猜测了一个函数表达式
	plot(f,tspan,label=false,frame=:origin)
	
end

# ╔═╡ d66cec73-471b-46f0-90b6-3618980ab67c
let
	spot=[−2, −1, 0, 1, 2, 3, 4, 5 ]
	f(x)=4.2*cos(x+0.2)
	h=0.001  #步进一小段距离
	m(h,p=0)=(f(p+h)-f(p))/h
    marr=[m(h,p) for p in spot]
	df=DataFrame(;spot=spot,slope= marr)
	 
end

# ╔═╡ c45c0be2-209c-4810-b934-0cbd9ad85858
md"""
 由于所取 h为很小的距离, 两点之间的斜率可以近似表示该点切线的斜率, 然后可以使用这个斜率绘制出一小段线段,直观显示在该位置的变化趋势. 
"""

# ╔═╡ 49deb05b-51e5-4afc-9804-22392d359d67
let
	tspan=-3:0.02:5.2
	spot=[−2, −1, 0, 1, 2, 3, 4, 5 ]
	f(x)=50*cos(x+0.2)
	range=0.4
	h=0.001  #步进一小段距离
	m(h,p=0)=(f(p+h)-f(p))/h  #经过某个点的斜率计算, 步进一小段距离
	p1=plot(f,tspan,frame=:zerolines,ls=:dash, lw=:0.5) #函数图
	plotarr=[p1]
    for p in spot  # for 循环生成斜率场图数组
	   slope=m(h,p)
	   intercept=f(p)
	   affline(x)=slope*x+(f(p)-slope*p) 
	   linesegment=[[p-range affline(p-range)];[p+range affline(p+range)]]
	   line=plot!(linesegment[:,1],linesegment[:,2],label=false,lw=2)
	   push!(plotarr,line)
	end
    plot!(plotarr...,size=(400,500))

	
end

# ╔═╡ a91a1ac3-fa94-4b4a-980a-131c0440a8a7
md"""
!!!  notice
      在数值计算部分实际我们已经定义了导函数, 给定定义域, 定义域就是要求导数的点,值域就是求出的导数, 还有求导数的方法
$$定义域 x\in [−2, −1, 0, 1, 2, 3, 4, 5 ]$$

$$值域 y \in [4.09064,3.01143,-0.836469,-3.91532,-3.39445,0.247268,3.66165,3.70953]$$

$$映射规则:{f(x) }'=\frac{f(x+h)-f(x)}{h}$$
 
"""

# ╔═╡ 3d0f8cef-5677-4b14-a3fc-5dc30a57d974
let
	tspan=-2.2:0.02:5.2
	spot=[−2, −1, 0, 1, 2, 3, 4, 5 ]
	h=0.001  #步进一小段距离
    ...
    marr=[m(h,p) for p in spot]
	df=DataFrame(;spot=spot,slope= marr)
	plot(f,tspan,frame=:origin,label=L"f(x)")
	plot!(df.spot,df.slope,label=L"{f(x)}'")
...
# ╔═╡ 97b35ada-a66f-4820-b038-58fcf957e3c7
md"""
 导函数的图形反映了函数的变化的特性, 注意几个要点:
 1.  与x轴的交点(y=$0$)的点
 2.  正负符号交换的区间,这里是我们寻找函数最大值或者最小值的区域
 3.  同符号折返的点, 例如下降变上升或者上升变下降(称为反射点)
 4.  导函数是把定义域的值映射为该点的切线的斜率, 度量的是在某个$x$处的变化情况,这与原来函数的意义不同

**后面的学习要反复理解这几条,  函数能够表现出和生物一样的各种特性和脾气, 一旦你理解了,你就能和它融洽相处**
"""

# ╔═╡ c6816a72-2810-4859-8726-9a8de5413adf
md"""
## 导函数的图像告诉了我们内容?

!!! notice
      如果在区间内 ${f(x)}'>0$,说明$f(x)$ 在区间内递增. 
      
      如果在区间内 ${f(x)}'<0$,说明$f(x)$ 在区间内递减. 
    
"""


# ╔═╡ 54c625de-69a4-49e5-b13d-eb1557384d8a
md"""
## 数值角度看导函数
**example3  药物浓度在血液中的变化情况**
"""

# ╔═╡ f211a963-e3a9-408b-80ae-ebdeca06a208
let
	yoffset=0.1
	h=0.1
	tspan=0:h:1
	tspanp=0:h:0.9
	ct=[0.84,0.89,0.94,0.98,1.00,1.00,0.97,0.90,0.79,0.63,0.41]
	@show df=DataFrame(;time=tspan,ct=ct)
	ctp=[]   # ct'
	size=length(ct)-1
	for i in 1:size
	    prime=(ct[i+1]-ct[i])/h
	    push!(ctp,round(prime,digits=2))
	end
	function midpointval(i)
	     a=ct[i]
		 b=ct[i+1]
		return a>=b ? (a) : (b)
	...
	ann=[
       (0.05,midpointval(1)-yoffset,text("↑s:$(ctp[1])",halign=:left,valign=:top,pointsize=8))
	   (0.45,midpointval(5)-yoffset,text("↑s:$(ctp[5])",halign=:left,valign=:top,pointsize=8))
	   (0.95,midpointval(10)-yoffset,text("↑s:$(ctp[10])",halign=:left,valign=:top,pointsize=8))
	]
	
    plot(tspan,ct,label="ct",color=:blue,xticks=0:0.1:1,ann=ann)
	scatter!(tspan,ct,label=false,ylims=(0,1.1),mc=:black)
	
	
end

# ╔═╡ 094bf7e1-4a61-4a56-a557-2db99fdc0ba1
md"""
## 从函数表达式获取导函数

### 平行于 x 轴的直线斜率为 0, 导数也为 0

"""

# ╔═╡ 803f9143-0bbb-4bd9-b7e8-256b1ac51260
let
	xspan=-10:10
	y(x)=2
	ann=[(5,1.8,text("f'(x)=0",pointsize=10,color=:red)),
	      (-5,1.8,text("⇑slope=0",pointsize=10,color=:red))
	]
	plot(y,xspan, frame=:zerolines,label=L"f(x)=2",lw=1,ylims=(0,4),ann=ann)
end

# ╔═╡ b94856de-d376-45f9-8254-3ccd85025938
md"""
 ### 线性函数的导数
如: 
...
# ╔═╡ de5c7f92-821e-44b8-8cda-ec2f511bffda
md"""
### 幂函数的导函数
这部分请参考教材 page 127
