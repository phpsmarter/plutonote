...
# ╔═╡ 0ce0e03c-e3e2-11ec-33e3-39ceff8144c2
begin
...
# ╔═╡ d854b3c3-e21e-4218-9678-11edc1bfa93f
PlutoUI.Resource("https://tva1.sinaimg.cn/thumbnail/e6c9d24egy1h2alsw1tzxj20m80gomxn.jpg")

# ╔═╡ 4850bbe6-499d-43bc-b092-d848c3515033
md"# ch02 sec 2.5 二阶导数"

# ╔═╡ deee8ab1-9b4c-4631-bfc8-72c9a6ebe330
md"""
 一阶导数是度量函数在某点的变化率, 二阶导数时度量一阶导数的变化. 例如瞬时速度是度量某个时间点物体的位置移动的能力. 二阶导数加速度是度量瞬时速度的变化情况. 

这个问题暂时不理解没关系, 后面会不断的重复.
...
一阶导数函数为直线, 但是在 $x=0$ 符号会发生改变, 
"""

# ╔═╡ cd99013f-33f8-4512-8b0e-56dddceb8acc
let
 ...
 ddf(x)=2
 ann(x,y,str)=  (x,y,
text(str, pointsize=16, color=:black, halign=:center, valign=:center, rotation=0))
   #ann3=[ann(0.5,0.5,"从左到右,一阶导数从负变正,二阶导数为正,函数上凹")]
   #ann4=[ann(0.5,0.5,L"从左到右,一阶导数从正变负,二阶导数为负, 函数下凸")]
   ann1=[
	 (-0.75,0.20,text(L"f'<0",pointsize=12,color=:green)),
	 (0.75,0.20,text(L"f'>0",pointsize=12,color=:green)),
	 (0,0.30,text(L"f''>0",pointsize=12,color=:black)),
	 (0,0.25,text(L"cover up",pointsize=10,color=:black))
 ]
 ann2=[
	 (-0.75,-0.20,text(L"f'>0",pointsize=12,color=:green)),
	 (0.75,-0.20,text(L"f'<0",pointsize=12,color=:green)),
	 (0,-0.25,text(L"f''<0",pointsize=12,color=:black)),
	 (0,-0.30,text(L"cover down",pointsize=12,color=:black))
 ]
  p1=plot(f1,tspan, label=L"f(x)=x^2",lw=1,frame=:zerolines,ann=ann1)
  p2=plot(f2,tspan, label=L"f(x)=-x^2",lw=1,frame=:zerolines,ann=ann2)
  # p3= plot(ann=ann3,grid=false,ticks=false,label=false)
  # p4= plot(ann=ann4,grid=false,ticks=false,label=false)
  plot!(p1,p2,layout=(1,2),size=(1000,350))
 
end

# ╔═╡ 698299bc-606f-4ff6-b932-b5d3c185ebcd
md"""
上图左边:"从左到右,一阶导数从负变正,二阶导数为正,函数上凹"

上图右边:"从左到右,一阶导数从正变负,二阶导数为负, 函数下凸"
"""


# ╔═╡ 88b8f284-edf9-481f-a96b-073ee76bf96c
md"""
## 速度和加速度
如果速度表示为$v(t)$加速度可以表示为:

$$\frac{v(t+h)-v(t)}{h}$$

在$t$ 时刻, 选取有一个小的间隔$h$ , 速度的变化率
...
# ╔═╡ 938ed848-bcdf-4db5-a19b-325822653db9
md"""
如果  $y=s(t)$  表示物体在$t$ 时刻的位置
-  速度: $v(t)=\frac{dy}{dt}=s'(t)$
-  加速度: $a(t)=\frac{d^2y}{dt^2}=s''(t)=v'(t)$
"""
