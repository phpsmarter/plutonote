...
# ╔═╡ ac52d7d4-e7ba-11ec-00c6-d5af41f1e809
begin
...
# ╔═╡ d7704525-ae3b-4a79-a8d2-f52bac27c28a
PlutoUI.Resource("https://tva1.sinaimg.cn/thumbnail/e6c9d24egy1h2alsw1tzxj20m80gomxn.jpg")


# ╔═╡ 7d881d36-92d9-4a81-9f17-adf7ae1bf965
md"""
 # ch06 sec6.4 微积分第二基本定理

"""

# ╔═╡ 52a3087d-2c5c-4c52-afc6-c499eb2820d6
md"""
## 使用定积分构造反导函数

下面看看如何用定积分来构造原函数

假设有函数 $f(x)=e^{-^2}$. 它揭示了函数$F(x)$的变化情况, 我们的目的就是通过积分的方式找到这个函数.

根据积分基本定理有:

$F(b)-F(a)=\int_{a}^{b}e^{-t^2}dt$

设定 $a=0$,$b$ 替换为$x$,得到:

$F(x)-F(0)=\int_{0}^{x}e^{-t^2}dt$

如果反导函数满足 $F(0)=0$ 则

$F(x)=\int_{0}^{x}e^{-t^2}dt$

由此就得到了一个反导函数的另一种表达式. 对于定义域的的$x$,可以通过黎曼和的方法求出近似值. 

通过这种构造方式得到反导函数$F(x)$  求导数必然会得到 $f(x)$.


**在一般的微积分思维逻辑中,要先给出一个满足连续性的函数,然后求导, 研究变化情况.在实际中绝大多数的情况下我们只知道到变化是什么, 原本的函数是什么并不清楚, 因为复杂的变化用我们之前学过的一些基础函数单独表示不了.**
...
(1).  黑箱模型
      ...
      看看饮料,比如可口可乐, 如果你在很多地方买过可乐, 你会发现标注的生产地其实有好几个. 如果喝起来味道一样, 我们就感觉不到灌装厂的区别.  对于可乐我们关注的的是好喝不好喝,而不是具体在那里生产的. 

(2).  "鸭子类型"

      这是编程技术里的一种方法. 具体说就是如果叫起来和鸭子一样"嘎嘎!"的都是鸭子, 不管他是不是鸭子.      
      常规思维, 我们要想听到"嘎嘎"叫.必须先有鸭子,然后才能有"嘎嘎". 
...
![计算中的上帝](https://tva1.sinaimg.cn/mw690/e6c9d24egy1h320qnvrtyj207i0b93yo.jpg)

但是如果有我们有一个系统能够重现这种变化, 到底有没有上帝就不是重点了. 这就是积分第二定理的威力所在.
"""

# ╔═╡ ef3b0d27-0279-4e46-8e80-3f2923f2be9a
md"""
!!! theorem
...
# ╔═╡ 1975744a-f8fc-4484-9e7b-c0d6ed4bb769
md"""
**构建定理和基础定理的联系**
...
# ╔═╡ 6f6bbd28-52df-4f26-8f09-f9b2e92de4d9
md"""
$F(x)=\int_{a}^{x}\frac{sint}{t}dt$ 
...
# ╔═╡ d080b1cf-92ce-4acb-bae6-6eb46caee0af
md"""
!!! example
    example1
    用数据 [0,1,2,3] 计算$Si(x)$

由于积分在$0$处无定义, 用 $a=0.0001$ 代替$0$, $x_1=0.00011$. 

结果如下, 黎曼和公式使用之前定义的, 值返回左和与右侧和的平均值
"""

# ╔═╡ a4e76457-8bf6-49d2-92aa-1c4a2f698b3b
...
# ╔═╡ c1a4df6b-571e-4879-abb9-473fa680a19e
let
	f(t)=sin(t)/t
	a= 0.00001
	n=250
...
# ╔═╡ 97c1c6f7-b4a5-4bdb-b0d0-325722167a0e
