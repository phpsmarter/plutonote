...
# ╔═╡ 501644f0-e789-11ec-13d9-c99f8c386c1c
begin
...
# ╔═╡ 8d7e9624-be11-4331-a0d7-0539f300f986
PlutoUI.Resource("https://tva1.sinaimg.cn/thumbnail/e6c9d24egy1h2alsw1tzxj20m80gomxn.jpg")


# ╔═╡ 27f27740-48c6-4366-a282-23ae62ff4871
md"""
# ch06 sec6.2 用分析方法构造反导函数
"""

# ╔═╡ 779a237a-29f2-4341-8d3b-70eee4b5ce0a
md"""
问题: $f(x)=0$ 的反导函数是什么?

如果一个函数的导数为$0$,表示函数在**区间内**没有变化, 所以返回值是一个常量,所有返回常量的函数导数都为 0

$如果F'(x)=0, 则 F(x)=C$

微积分总是在考虑局部为, 所以注意区间问题, 对于一个分段函数在一个区间内返回常数, 可能在其他区间是变化的.

例如行驶的汽车, 在某段时间停下来, 速度没有变化,$F'(x)=0$,在这段时间内行驶的距离为$F(x)=0$.但是在其他时间段速度会有变化.

不同区间函数表达式可能会不同,变化率会不同, 导数也不同,对应的反导函数也不同 
"""

# ╔═╡ 2804a14b-ec82-4b98-8751-508c5c4ec7a1
md"""
反导函数的通解

如果两个函数($F(x),G(x)$)在同区间内导数相同, 可以表示为:

$G(x)=F(x)+C$

不定积分的定义:

所有的$f(x)$ 的反导函数形式都是$F(x)+C$,符号表示为:

$\int f(x)dx=F(x)+C$

不定积分和定积分表达的意义不同, 定积分表示的函数在区间内的变化率, 而不定积分表达的是一组函数. 

 **这里可以看到,我们可以用不定积分来度量函数簇的性质, 度量物理性质是数学的出发点**
"""

# ╔═╡ 6d09764d-effc-4e0d-83ef-9bbc93f8966b
md"""
...
由于$C \in R$, 斜率为$k$ 的直线铺满了整个平面, 因为这里有两个变量$x$和$C$,因此我们可以把$F(x)=kx+C$ 写成线性列组合的形式,如下:
...
**两种方法结果是一样的, 但是计算机非常擅长做乘法, 因此线性组合的形式对提高计算性能有帮助,同时也利于对概念的理解.**

"""

# ╔═╡ db32b434-fda8-4afc-9b2a-46ea36c8408f
md"""
## 求反导函数
...
# ╔═╡ 18d6e8fe-6a3e-4d5e-ba13-fcd8516b4299
md"""
!!! theorem
    反导数定理:加法和倍乘
...
# ╔═╡ 189967c0-9416-4498-92cd-fe5b4e17d3c7
