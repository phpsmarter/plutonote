...
# ╔═╡ 53833ea2-e797-11ec-2b24-69391fd60570
begin
...
# ╔═╡ cb6f2d51-fbe4-44ec-be07-8f42a4993d74
PlutoUI.Resource("https://tva1.sinaimg.cn/thumbnail/e6c9d24egy1h2alsw1tzxj20m80gomxn.jpg")


# ╔═╡ 060c15a3-6b61-417f-a8dc-34cda12b2314
md"# ch06 sec6.3 微分方程和运动"

# ╔═╡ ceb34760-d988-4215-ab5b-407e2b865c07
md"""
函数的形式如果类似:

$\frac{dy}{dx}=f(x)$

这就是一个微分方程, 求微分方程的通解其实就是求 $f(x)$ 的反导函数 $F(x)+C$


微分方程模型在实践应用中无处不在.

用数据科学和机器学习方法来解微分方程也有了很大的发展,所以学科融合的威力在增强,数学为数据科学和人工智能提供基础, 反过来现在人工智能又加速快数学的研究速度!
...
# ╔═╡ e90f5cad-cde0-4d4c-805a-24a05558a515
md"""
!!! example
    example 1
   
    绘图描述微分方程 $\frac{dy}{dx}=2x$ 的通解

 微分方程 $\frac{dy}{dx}=2x$ 反导函数为:
...
我们取一组整数来绘图 $C \in [-4,1,0,2,5]$


"""

# ╔═╡ 61a53dce-144c-4704-b092-fa91fa0ec587
let
   carr= [-4,0,1,2,5]
   tspan=-4:0.02:4
   function generatefunc(c)
	   return function(x)
		       return x^2+c
	           end
   end
   ...
   for (i,c) in enumerate(carr)
	    str= c>=0  ?  L"x^2+%$c" : L"x^2%$c"
	    if i==1
		   
		   lab=[str]
		   push!(funcarr,generatefunc(c))
		else
		   
		   lab=hcat(lab,str)  #plots中 label 需要用向量, 所以用 hvact 拼接
           push!(funcarr,generatefunc(c))
		end
   end
   ...
   plot(tspan,funcarr,frame=:zerolines,label=lab,ylims=(-4,20),size=(400,400))
end

# ╔═╡ 494b9069-97e4-4ae4-b4c6-65e70bc25162
md"""

$**如何找出特解?**$

当我们获取了通解形式 $y(x)=x^2+C$,  带入一组曲线上的值 $\{x_0,y(x_0)\}$ 可以求出唯一的$C$,
就可以求出特解


**注意 这里的$x_0$,$0$ 下标只表示是微分方程的初始条件,并不一定是 $t=0$ 时刻,但是$t=0$时刻的值可以作为初始条件**

"""

# ╔═╡ 715f3b97-6857-4712-8915-6242ba2fe6b0
md"""
!!! example
    example2

    当 $y(3)=5$ 时,求$\frac{dy}{dx}=2x 的特解$
...
# ╔═╡ cf30ce3c-18b1-4aa1-85c4-e1cbcef0cdaf
md"""
## 运动的方程
...
如果速度向量竖直向上为$+$, $g$的符号为$-$ 表示与速度向量方向相反. 

**再次重复,不同的量值单位度量的是不同的物理性质, 速度单位为$m/s$,加速度单位为$m/s^2$,速度度量的是位移的变化, 而加速度度量的是速度的变化.**

分解一下,$\frac{ds}{dt}Δt$ ,$dt$和$Δt$ 都是时间单位$s(秒)$,所以被消掉, 留下的单位是$m(米)$, 表示在$Δt$ 时间内走过的距离, 这里速度和时间乘积度量的是距离

再看看 , $\frac{dv}{dt}Δt$,同上,时间单位被消去,留下的单位是$m/s$, 加速度和时间的乘积度量了速度

从文字上注意差异, 加速度虽然比速度只多了一个"加", 但这不是数学和物理意义上的"加"法,  数学和物理意义上的加法, 量纲和域要相同. 

  $v+g$ 不行,单位不同,  $v+gΔt$ 可以 ,$v$ 和$gΔt$ 单位都是 $m/s$
 
  $s+v+a$ 也不行  但是 $s+vΔt+g{Δt}^2$ 可以, 因为 $s$,$vΔt$,$g{Δt}^2$ 单位相同


$g=\frac{m}{s^2},  {Δt}^2=s^2$
...
$s(t)=-\frac{g}{2}t^2+v_{0}t+s_0$


如果考虑空气阻力,因为空气会对运动的物体施加压力, 所以引入了新的变化来源, 为了厘清变化要引入新的微分方程, 从而组成一个系统(system), 方程组就叫做 system.

"""

# ╔═╡ 6b156cca-cb8d-4997-be1a-9b5760db2ab8
