...
# ╔═╡ 5c61e2aa-94bb-4423-ab8e-0df7c798be57
begin
...
# ╔═╡ 2956f4a1-4547-4917-ba37-398fe416b1cd
PlutoUI.Resource("https://tva1.sinaimg.cn/thumbnail/e6c9d24egy1h2alsw1tzxj20m80gomxn.jpg")


# ╔═╡ f6af3dea-ed35-11ec-1b2c-03035fdf8851
md"""
# ch10 sec10.5  傅里叶级数
"""

# ╔═╡ 6f5d54f5-df9c-4a27-ba02-b54918514078
md"""
!!! definition
    **周期为 2π函数的傅里叶多项式系数定义为:**

    $a_0=\frac{1}{2π}\int_{-π}^{π}fxdx$
    $a_k=\frac{1}{π}\int_{-π}^{π}cos(kx)dx \ \ \ \ \ , k>0$
    $b_k=\frac{1}{π}\int_{-π}^{π}sin(kx)dx \ \ \ \ \ ,k>0$

     $a_0$ 定义方法其实就是区间内积分面积除以区间长度, 也就是平均值

下面看一下如何构建傅里叶多项式
"""

# ╔═╡ c3eec29a-7fa8-422f-aabd-e3d51a833d7a
md"""

## 傅里叶级数

与泰勒多项式和泰勒级数的关系一样, 当傅里叶多项式的项数趋近于无穷大的时候, 就称为傅里叶级数


"""

# ╔═╡ 5af6fddb-1318-4c42-9395-1340b439127e
begin
	datacollection=Dict()
...
# ╔═╡ 28289267-7c2b-42ed-8818-4b356a2dcb19
md"""
前面我们已经看到泰勒多项式可以很好的在某个区间内对函数的取值进行近似. 但是泰勒级数有局限, 超过一定的分为, 近似值就远离原始值. 

在数学中, 傅里叶级数可以解决大区间的问题, 尽管傅里叶级数在局部近似效果比泰勒基础差. 傅里叶级数更像是传统意义上的函数. 

所以应用范围更为广泛.  但是近似的原理是样的, 这也是在学习傅里叶级数之前学习泰勒级数的原因. 


傅里叶级数使用三角函数作为构建的基本模块, 因为三角函数的性质和行为数学家了解的很彻底, 所以应用起来就方便.

由于三角函数具有周期性,所以以三角函数为构建模块的傅里叶级数很自然的可以近似周期函数. 因为自然界很多的现象都是周期性变化的, 所以傅里叶级数的应用范围极其广泛.



假定一个函数的波形为方波, 以$2π$ 为周期重复变化. 

 $(datacollection["squarewave"])

由于三角函数也有周期性, 而且三角函数的图像随着振幅,频率和相位发生各种变化.所以可以猜想通过三角函数的线性组合是不是可以近似方波的函数.



这里之所以提到线性组合问题, 是因为具体的函数不知道表达式到底是什么, 在近似的时候希望用于近似的工具函数能够包括这个需要研究的函数.  借用线性代数的概念, 我们希望要研究的函数是近似工具函数的子空间. 

当工具函数有$n$ 项时, 能够代表一个无限维度的空间, 需要研究的函数就有容身之所了. 
...
$$\begin{align*}
& f(x) \approx F(x) \\ 
 =  & \ a_0+a_1cos(x)+a_2cos(2x)+a_3cos(3x)+...+a_ncos(nx)  \\ 
           \\
  +& \ b_1sin(x)+b_2sin(2x)+b_3sin(3x)+...+b_nsin(nx)  \\
           \\
            \\
  =&  \ a_0+\sum_{k=1}^{n}a_kcos(kx)+ \sum_{k=1}^{n}b_ksin(kx)
\end{align*}$$



$F_n(x)$ 称为 $n$阶傅里叶多项式,  $a_k,b_k$ 称为傅里叶系数. 现在问题变成了, 要让多项式近似函数, 
这些系数怎么选择
"""

# ╔═╡ 52600f06-4454-4362-94e0-829839de1138
md"""
!!! example 
    example 1 
    构建周期为$2π$ 方波函数的连续性傅里叶多项式
...
$a_0=\frac{1}{2π}\int_{-π}^{π}fxdx=\frac{1}{2π}\int_{-π}^{0}0dx+\frac{1}{2π}\int_{0}^{π}1dx=\frac{1}{2}$

  $a_1$ 为:

$$a_1=\frac{1}{π}\int_{-π}^{π}1cosxdx=0$$

  $b_1$ 为:

$$b_1=\frac{1}{π}\int_{-π}^{π}1sinxdx=\frac{2}{\pi}$$

所以方波函数的一阶傅里叶近似为:

$f(x) \approx F_1(x)=\frac{1}{2}+\frac{2}{\pi}sinx$


图形为

 $(datacollection["fourierapprox"])


可以一直构造下去, 详细的步骤见参考书上细节


当构建到$7$ 阶时, 函数为:

$F_7(x)=\frac{1}{2}+\frac{2}{\pi}sinx+\frac{2}{3\pi}sin3x+\frac{2}{5\pi}sin5x+\frac{2}{7\pi}sin7x$

图形为:

 $(datacollection["7fourierapprox"])
"""

# ╔═╡ 6638d980-cb76-4db3-a426-d651557d291a
let
	tspan=-pi:0.02:pi
	function squarewave(t)
	      return (-pi <= t < 0 ) ?  0 : 1
	end
    p1= plot(squarewave,tspan,frame=:box,label=L"square \ wave \ on \  [-π,π]",size=(300,300))
	save("squarewave",p1)
	
end

# ╔═╡ c3440c8d-552d-4262-bfb4-c5a64a295c6e
let
    tspan=-pi:0.02:pi
	function squarewave(t)
	      return (-pi <= t < 0 ) ?  0 : 1
	...
	p1= plot(squarewave,tspan,frame=:box,label=L"square \ wave",size=(400,300))
	p2= plot!(F₁,tspan, frame=:box,label=L"1st order approx",size=(400,300))
	p3=plot!(p1,p2,size=(600,300))

	save("fourierapprox",p3)
end

# ╔═╡ 56c16183-b233-476c-9354-30731c6c43ae
let
    tspan=-pi:0.02:pi
	function squarewave(t)
	      return (-pi <= t < 0 ) ?  0 : 1
	...
	F₇(x)=0.5+(2/pi)*sin(x)+(2/3pi)*sin(3*x)+(2/5pi)*sin(5x)+(2/7pi)*sin(7x)

	p1= plot(squarewave,tspan,frame=:box,label=L"square \ wave",size=(400,300))
	p2= plot!(F₇,tspan, frame=:box,label=L"7st order approx",size=(400,300))
	p3=plot!(p1,p2,size=(600,300),legend=:bottomright)

	save("7fourierapprox",p3)
end

# ╔═╡ 886e4159-fe64-4f10-8d0b-d6e05b1973ce
