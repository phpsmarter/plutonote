...
# ╔═╡ c8520d52-e570-11ec-1329-4ff82d3973a5
begin
...
# ╔═╡ 6e192c0a-5726-449d-801d-154c0e24d144
PlutoUI.Resource("https://tva1.sinaimg.cn/thumbnail/e6c9d24egy1h2alsw1tzxj20m80gomxn.jpg")


# ╔═╡ 03c852c8-35f4-49cd-a774-c808bd24069a
md"# ch04 sec3.3  优化和建模"

# ╔═╡ a457f353-3ba1-4b90-99de-1b9e1a3476d9
md"""
数学建模的过程就是把实际问题转化为函数的过程. 在建模过程中就要知道所有获得函数的各种特性,了解函数的极值和边界是必须要完成的任务 .

!!! note
    example1

    假设要建造一个容积为 40 立方英寸的金属铝水罐,要使用最少的材料. 
    
    ![](https://tse1-mm.cn.bing.net/th/id/R-C.6230d2c2aa43695d68b708def2d38127?rik=F835FYh4DDpYmQ&riu=http%3a%2f%2fwww.yunsuan.org%2fstatic%2fupload%2fb0%2fb0710cc0-e180-11e4-aca9-00163e022551.jpg&ehk=OhyNTJdy06RgYT5kRp4s1ipLwZ1txgoyIM3wCGa5We4%3d&risl=&pid=ImgRaw&r=0)

"""

# ╔═╡ 3d82d3c3-aee1-4a82-aa5a-911a7a683012
md"""
...
体积限制为40 ,因此 $π(r^2)h=40$  所以 $h=\frac{40}{(πr^2)}$

  $side(h,r)=2πr\frac{40}{(πr^2)}$  所以  $side(r)=\frac{80}{r}$

体积为 40 的材料面积为公式为:

$m(r)=2πr^2+\frac{80}{r}$

求导得: $\frac{dM}{dr}=4πr-\frac{80}{r^2}=0$  所以临界点为:$4πr=\frac{80}{r^2}$

所以 $πr^3=20$   ,$r=1.85 inches$


材料用量为:$m(1.85)=2π(1.85)^2+\frac{80}{1.85}=64.7 in^2$
"""

# ╔═╡ 39d19483-4532-4d76-89a6-24b57d76f3bc
md"""
!!! tips
       建模优化实践中的几个提示:
    ...
    2.如有可能,画出几张草图大致了解一下问题

    3.从之前获取的信息尽可能得到用于构造函数的方程,消减不必要的变量

    4.找到临界点, 在区间和临界点调用函数, 找到最大值和最小值
"""

# ╔═╡ 3ddd5f89-2055-49ea-9afb-00c0150de6c8
md"""
!!! example

    example3:
    由 $y=\sqrt{x} , x=9, y=0$  围成的区域, 在 $y=\sqrt{x}$ 上找到一个点, 使其到$x=9$ 和$y=0$的距离围成的矩形面积最大化

也就是求下图中蓝色区域最大值
    
"""

# ╔═╡ e438f001-56fd-4e9f-92b9-202b8f8f3631
@bind apoint Slider(0.2:0.2:8.8,default=2,show_value=true)

# ╔═╡ 850f1487-7334-42b1-a978-dec892e49a20
let
	a=apoint
	yoffset=0.30
	y(x)=sqrt(x)
	xspan=0:0.02:9
	ann=[
		(a,y(a)+yoffset,text("[$(a),$(round(y(a),digits=2))]",pointsize=10)),
		 (a,0.07,text("a",pointsize=12,color=:red))
	]
	plot(y, xspan,label=false,frame=:origin,color=:gray,lw=3,ann=ann)
	scatter!([a],[round(y(a),digits=2)],label=false,ms=6)
	vline!([9],label=false,color=:gray,lw=3,ylims=(0,y(9)))
	areaplot!([a,a,9,9],[0,y(a),y(a),0],label=false,seriescolor = [:blue],fillalpha = [0.2],
)
end

# ╔═╡ afe02ac5-30f3-48ef-9cbe-78ef5944fea4
md"""
矩形的高度由$y(x)=\sqrt{a}$ 求出, 所以高度为 $\sqrt{a}$
...
$\frac{dR}{da}=\frac{9}{2}a^{-1/2}-\frac{3}{2}a^{1/2}=0$

经过化简得:

$18=6a 所以  a=3$  

这里不用考虑两个边界值, 因为在边界处面积都为 $0$

所以矩形的 宽度为$6$ 高度为$\sqrt{3}$ 的时候, 围成的面积最大化
"""

# ╔═╡ babe89b9-85ac-4c4e-8890-f77b4560dcd6
md"""
!!! example
    example4

    ![](https://tva1.sinaimg.cn/small/e6c9d24egy1h2yqc35mnsj20ex0ee0ss.jpg)

    底面封闭的盒子, 表面积为定值$A$,底边为正方形,宽度为$a$

    a. 求盒子体积的公式, 因变量为底边宽度$x$, 体积的值域范围是多少
...
# ╔═╡ 2372ec7b-d210-4593-a97f-d47642a83a6e
md"""
表面积表达式为:
...
$V=x^2h=x^2(\frac{A-2x^2}{4x})=\frac{A}{4}x-\frac{1}{2}x^3$

当$h->0$ 时, $A=2x^2$  所以 $x 定义域为: [0,\sqrt{\frac{A}{2}}]$


"""

# ╔═╡ e36054ad-34f5-45ee-9500-b83a81b61966
@bind  Aval Slider(40:100,show_value=true)

# ╔═╡ 6f3bf332-7689-47fa-ad46-8a66c0477eb6
let
	A=Aval
	xspan=0:0.05:sqrt(A/2)
	f(x)=(A/4)x-(1/2)*x^3
	ann=[
		(sqrt(A/2),2,text(L"\sqrt{\frac{A}{2}}",pointsize=10,color=:red,halign=:right))
	]
	plot(f,xspan, label=L"\frac{A}{4}x-\frac{1}{2}x^3",frame=:origin,ylims=(0,80),
	xlabel=L"x",ylabel=L"V",ann=ann,xlims=(0,sqrt((2A)/3))
	)
end

# ╔═╡ abf73979-bf59-4238-ac5d-2227843dac6d
md"""
求体积最大值, 对函数求导得到:

$x=\pm \sqrt{\frac{A}{6}} ,取正值$
...
# ╔═╡ b55c7157-326f-4d8c-a9c3-69f40121abfb
