...
# ╔═╡ b4f4f371-f1f1-48cc-a3c8-d6648a3722ce
begin
...
# ╔═╡ c5901b40-b48c-42fc-8c3a-394ce3ce8f5a
PlutoUI.Resource("https://tva1.sinaimg.cn/thumbnail/e6c9d24egy1h2alsw1tzxj20m80gomxn.jpg")


# ╔═╡ cf4da988-ec44-11ec-0359-8164a5f71126
md"""
# ch09 sec9.2 几何级数
"""

# ╔═╡ 2d7c9b1d-813c-4c31-9b53-42f299fa54f1
begin
	datacollection=Dict()

	function save(key::String, dict::Dict)
		return merge!(datacollection,Dict(key=>dict))
	end
...
# ╔═╡ 1d4a3924-9f06-42a5-b470-91451ce010a5
md"""
 序列的各项相加就得到级数(series)

## 反复用药的剂量问题

耳部感染的病人遵医嘱, 需要在治疗期间多次服药, 在两次服药期间,部分药物会被身体排出体外, 如何计算某个时刻体内的药物浓度?

来看实例, 假设服用的氨苄西林, 一天要服四次, 每次$250mg$.经过实验已经知道了,经过 $6$ 小时, 体内还有$4\%$的药物,问题: 服用第 $10$ 剂的时候,体内药物还有多少, 第$14$剂药物还有多少?


假设用$Q_n$ 代表药物的服用第$n$ 剂药物时体内药物的量
...
$Q_2=\begin{align*}
\overset{\underbrace{250(0.04)}}{第1剂}+\overset{\underbrace{250}}{新服用的药物} &=  260 mg
\end{align*}$

$Q_3=\begin{align*}
Q_1(0.04)^2+Q_2(0.04)+250
&
= \overset{\underbrace{250(0.04)^2+250(0.04)}}{第1,2剂的剩余量}+\overset{\underbrace{250}}{新服用的药物}=260.4mg
\end{align*}$


$Q_4=\begin{align*}
\overset{\underbrace{250(0.04)^3+250(0.04)^2+250(0.04)}}{第 1,2,3剂的剩余量}+\overset{\underbrace{250}}{新服用的药物}
&
= 260.416mg
\end{align*}$
...
$Q_{10}=250(0.04)^9+250(0.04)^8+...+250(0.04)^2+250(0.04)+250$


每片药物进入身体的时间点不同, 各个药片是独立随着时间演进被身体排出体外的, 所以总的药物剂量是各个不同时间点服药药物的线性组合. 这也是线性组合的意义, 每个药片之间是线性无关的. 
...
$(0.04)Q_{10}=250(0.04)^{10}+250(0.04)^9+...+250(0.04)^3+250(0.04)^2+250(0.04)$

两式相减得到:

$Q_{10}-0.04Q_{10}=250-250(0.04)^{10}$

移项化简得:

$Q_{10}=\frac{250(1-0.04^{10})}{1-0.04}$

所以$Q_{40}$ 为:

$Q_{40}=\frac{250(1-0.04^{40})}{1-0.04}$


  $Q_{n}$通项式为:

$Q_{n}=\frac{250(1-0.04^{n})}{1-0.04}$
...
# ╔═╡ 23092c19-8b02-40f0-a880-d1521c511cee
let
	data=datacollection["几何级数1"]
	key=keys.(data["res1"])
	vals= values.(data["res1"])
	df=DataFrame(day=key,dosage=vals)
end

# ╔═╡ d38a3699-56cc-4ddb-9519-de8bfabd01c4
 let
	 daycollection=[1,2,5,10,100,1000,10000]
	 tablet=250
	 rate=0.04
	 function  dosgewith(tablet,rate)
	     r=rate
		 t=tablet
       return (d)->Dict(
		"dosage at $(d)th day"=>t*(1-r^(d))/(1-r)
	   )
	 end
     getdosage=dosgewith(tablet,rate)

	 res1=[ getdosage(d) for d in daycollection ]
	 save("几何级数1",Dict("res1"=>res1))
	
 end

# ╔═╡ 99257d23-3c63-4eb0-b627-78c0d17a0bcf
md"""
## 有限几何级数的和

与药物剂量的实例一样, 有限几何级数通过代数化简可以得到:

$S_n=ax+ax^2+...+an^{n-1}=\frac{a(1-n^n)}{1-x} \ , x \neq 1$



## 无限几何级数的和

 $n \to +\infty$时, 只有在$|x|<0$时,序列才会收敛

$\lim_{n \to +\infty}S_n=\lim_{n \to +\infty} \frac{a(1-x^n)}{1-x}=\frac{a(1-0)}{1-x}=\frac{a}{1-x}$


!!! definition

    如果$|x|<0$,无限几何级数的和可以表示为:

    $S=a+ax+ax^2+...+ax^{n-1}+ax^{n}+...=\frac{a}{1-x}$
"""

# ╔═╡ 02444d08-8385-418d-8f96-134aff959742
md"""
## 定期在存款到账户的问题

和药物实验一样, 在每个时间点存入的钱都会随着时间线的独立的变化, 只不过在储蓄账户里是吸入新的利息而不是排出. 每一笔存款之间都是线性无关的

假设每年存入 $1000$块,复合年利率为$5\%$,计算$n$年之后账户余额

和药物实验同样列出前几项, 看看规律:

$B_1=1000$

$B_2=B_1(1.05)+1000=\overset{\underbrace{1000(1.05)}}{第一年存的钱}+\overset{\underbrace{1000}}{新存入存款}$	


$B_3=1000(1.05)^2+1000(1.05)+1000=\overset{\underbrace{1000(1.05)^2}}{第一年存的钱}+\overset{\underbrace{1000(1.05)}}{第二年存的钱}+\overset{\underbrace{1000}}{新存入存款}$	


可以看到模式为:

$B_n=1000(1.05)^{n-1}+1000(1.05)^{n-2}+...+1000(1.05)+1000$
...
如果我们能够永生, 那么每年存入 1000 块, 账户的钱会无穷大, 所以存款的无限级数没有收敛

"""

# ╔═╡ 898059e0-3eef-4817-8490-4119f67813ab
 @htl("""<script src="https://cdn.bootcdn.net/ajax/libs/mathjax/3.2.0/es5/tex-svg-full.min.js"></script>
    """)
