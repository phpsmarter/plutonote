...
# ╔═╡ 6539dae2-67be-40b6-9377-cb5aae28921d
begin
...
# ╔═╡ 0b7830f6-babd-454a-9fad-319617d57b6f
PlutoUI.Resource("https://tva1.sinaimg.cn/thumbnail/e6c9d24egy1h2alsw1tzxj20m80gomxn.jpg")


# ╔═╡ e67ca2fe-ec57-11ec-24a1-4d740084ef9c
md"
# ch09 sec9.5  幂级数和收敛区间
"

# ╔═╡ c702fc18-217a-4a9f-96c8-ef780342cbd6
md"""

当$a$为常数时, 幂级数定义为:

$C_0+C_1(x-a)+C_2(x-a)^2+...+C_n(x-a)^n+...=\sum_{n=0}^{\infty}C_n(x-a)^n$

为了研究幂级数的收敛性, 设定$x$为固定值, 对其部分求和,如果部分求和的极限存在, 我们就定义为级数在$x$处收敛于极限值

"""

# ╔═╡ 72129804-0a19-4208-84c4-afde74ad4640
md"""
!!! example
    example 1

    找到下式的通项式, 并把级数表示为和的形式

    $\frac{(x-2)^2}{4}-\frac{(x-2)^6}{9}+\frac{(x-2)^12}{16}-\frac{(x-2)^10}{25}+...$


表达式为:

$\sum_{n=2}^{\infty}\frac{(-1)^n(x-2)^{2n}}{n^2}$


!!! example
    example 2

    判断幂级数 $\sum_{n=0}^{\infty}\frac{x^n}{2^n}$ 在
...
$\sum_{n=0}^{\infty}\frac{x^n}{2^n}=\sum_{n=0}^{\infty}\frac{(-1)^n}{2^n}=\sum_{n=0}^{\infty}(- \frac{1}{2})^n$
...
$\sum_{n=0}^{\infty} \frac{x^n}{2^n}=\sum_{n=0}^{\infty}\frac{3^n}{2^n}=\sum_{n=0}^{\infty}(\frac{3}{2})^n$
...
# ╔═╡ 3a0d0fe8-de17-47fc-8594-2a7ec83fa62d
md"""
## 从数值和图形角度来看收敛性

考虑如下级数:

$(x-1)-\frac{(x-1)^2}{2}+\frac{(x-1)^3}{3}-\frac{(x-1)^4}{4}+...+(-1)^{n-1}\frac{(x-1)^n}{n}+...$


当$x=1.4,1.9,2.3$ 时 看看级数的图形和数值情况

当$x=1.4,1.9$时,  随着级数项的增加, 各项和收敛性一个值

当$x=2.3$ 时, 随着级数项的增加,序列发散

从下图可以观察到, $0<x\leq 2 $ 时, 序列是收敛的. 

"""

# ╔═╡ 70fcaf83-b1f8-4b39-981b-15c1a8af8931
let
	x1,x2=1.4,2.3
	d14,d19,d23=[],[],[]
	nspan(n)=1:1:n
    f(x)=(n)->(-1)^(n-1)*((x-1)^n/n)
	f14,f19,f23=f(1.4),f(1.9),f(2.3)
	function p(f)
     return  function (n)
		    
		   return  val=mapreduce(f, +, nspan(n))
   ...
   processor23=p(f23)
	...
	df=DataFrame(;x14=d14,x19=d19,x23=d23,n=1:15)
end

# ╔═╡ 8787ccc7-a9eb-4ed6-a8dd-9d2540957e61
let
	function s(n)
	     (x)->(-1)^(n-1)*((x-1)^n/n)
	end
	nspan=[2,5,6,8,11,14]
	xspan=-2.4:0.01:3
    funarr=[s(n) for n in nspan]
	label=[L"S_2(x)" L"S_5(x)" L"S_6(x)" L"S_8(x)" L"S_{11}(x)" L"S_{14}(x)"] 
    plot(funarr,xspan,frame=:origin,ylims=(-2,2),legend=:topleft,label=label)
	vline!([2], lw=1, color=:black, ls=:dash)
	
end

# ╔═╡ 9b235ffe-e6a8-450a-bd96-4a751e7ed290
md"""
...
!!! definition
    ...
    - 当级数只在$x=a$ 处收敛, ,收敛半径$R=0$
    - 无论$x$ 取什么值, 级数都收敛, 则收敛半径$R 为  \infty$
    - 如果$x$满足条件使得$|x-a|<R$ 情况下收敛, $|x-a|>R$ 时发散, 收敛半径就是$R$
...
# ╔═╡ c6476eb0-1bdc-4b7d-8ff8-39b45ccccfa2
