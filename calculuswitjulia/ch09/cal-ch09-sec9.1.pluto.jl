...
# ╔═╡ 8773c7c8-ebf4-11ec-3966-2d0434281134
begin
...
# ╔═╡ 52e34c15-eb95-48b7-a07d-288d3256935a
PlutoUI.Resource("https://tva1.sinaimg.cn/thumbnail/e6c9d24egy1h2alsw1tzxj20m80gomxn.jpg")


# ╔═╡ e5e55857-eace-4999-8ea2-9bb81e882ae0
md"""
# ch09 sec9.1  序列

序列是一组有无限个数字组成的列表$s_1,s_2...s_{n-1},s_n$, $s_1$ 为第一项, $s_2$ 为第二项; $s_n$ 表示通项式. 
"""

# ╔═╡ e389238f-fcf0-4210-adc6-79089d445e62
begin
	datacollection=Dict()
...
# ╔═╡ d52080e6-3fc9-456f-8925-2faa61a4cdfb
md"""
## 从数值,代数和图形角度看
...
# ╔═╡ fef98259-0513-49d2-b6d4-cd437c716090
let
#1 的序列
data=read("example1")
res=data["res1"]
...
# ╔═╡ 11d2f9f8-9c2e-49d4-b309-3e642aaefb37
let
	#2 的序列
data=read("example1")
res=data["res2"]
...
# ╔═╡ fb30d133-042d-49ba-b57f-10ddf6dc0926
let
  function f(n) 
		str="$(n+1)"
		L"\frac{%$(n)\cdot %$str}{2}"
	...
	function g(n)
	 ...
	 res1= [g(n) for n in range ]
	 res2= [f(n) for n in range]
	 ...
	 save("example1",example1)

end

# ╔═╡ adfc0a36-dd64-49c4-ad41-b0e2283a1dcc
md"""
## 递归定义序列 
...
!!! example
    ...
    -  (1). $s_n=s_{n-1}+3\ , n>1 \ , s_1=4$

    -   (2). $s_n=-3s_{n-1} \ , n>1 \  ,s_1=2$
    -   (3). $s_n=\frac{1}{2}(s_{n-1}+s_{n+1}) \  , s_1=0 \ ,s_2=1)$
    -   (4). $s_n=ns_{n-1} \ , n>1 \ ,  s_1=1$
    
"""

# ╔═╡ 53f77a19-f41c-4b95-94b0-d737301f3018
let
	data=read("example2")
	@show  data["res1"]
	@show  data["res2"]
...
# ╔═╡ 0979f365-626a-4f31-8a9d-3c2ae0657d1d
let
	...
	function f3(n)
		...
		elseif n==2
			...
			return  1/2*(f3(n-1)+f3(n-2))
		end
	end
	f4(n)=n==1 ? 1 : n*f4(n-1)
    res1=[f1(x) for x in nstep]
	res2=[f2(x) for x in nstep]
	res3=[f3(x) for x in nstep]
	res4=[f4(x) for x in nstep]
	example2=Dict("res1"=>res1,"res2"=>res2,"res3"=>res3, "res4"=>res4)
	save("example2",example2)
	
end

# ╔═╡ 88e3a7e4-12a2-4825-b2fd-edfa6b2d8d04
md"""
## 序列的收敛性

序列的极限定义和函数的定义一样, 当$n\to +\infty$时, 序列趋近一个固定值$L$

!!! definition

    序列 $s_1,s_2,s_3,...s_n$ 如果当 $\lim_{n \to \infty}s_n=L$ 时 $L$ 称为 序列的极限.序列是收敛的.如果$L$ 不存在,就定义序列是发散的
"""

# ╔═╡ dd69862f-c5f3-49d7-bde4-268bd67d43e3
md"""
...
(a)  当序列$s_n=(0.8)^n n \to \infty$时, 序列趋近于$0$

(b)  $e^{-n} 会递减, 当 n \to  +\infty时, 分子分母都趋近于 1,所以极限为 1$

(c)  $n^2的增长速度比 n快, 所以 序列会一致递增, 是发散到的$

(d)  $(-1)^n,会随着 n的奇偶性符号发生变化, 所以序列在{0,2} 之间跳变, 序列是发散的$

注意(d),只能收敛到一个值. 
"""

# ╔═╡ 5a298982-4cfe-4083-9928-10b7fcca7381
let
    nspan=1:100
	f1(n)=(0.8)^n
	f2(n)=(1-ℯ^(-n))/(1+ℯ^(-n))
	f3(n)=(n^2+1)/n
	f4(n)=1+(-1)^n
    scatter([f1,f2,f3,f4],nspan,layout=(2,2),ms=2,label=[L"(a)"  L"(b)"  L"(c)"  L"(d)"])
end

# ╔═╡ 10fa440e-a172-4223-aaab-3f67487849bd
