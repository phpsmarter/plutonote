...
# ╔═╡ a104a8fe-0ed1-11ed-21b7-5b98a3eeeb42
begin
...
# ╔═╡ c80501d1-7719-44ac-8610-eae31f6cad2d
let
	@variables a, b,p,e
	 a,b=[1, 2, 2],[1, 1 ,1]
	 proj(b,a)=((a'*b)/(a'*a))*a
	 p=proj(b,a)
	 e=b-p
	 [latexify(b),latexify(p),latexify(e),latexify(p+e)]
	 
end

# ╔═╡ 65edbf4b-4077-4a63-87c4-7bef47327bc7
let
	...
	latexify(reduce_matrix)
	
end

# ╔═╡ 47279e63-b18c-4390-974d-c3b1c651bd2d
md"""
## 莱昂替夫的投入生产模型
"""

# ╔═╡ 5f59e550-489c-47ef-94eb-73a93af9ab42
let
	
	
	@variables x,y,z,eqs,d,I,C,IC
	eq1=0.5x+0.4y+0.2z
	eq2=0.2x+0.3y+0.1z
	eq3=0.1x+0.1y+0.3z
	d=[50,30,20]
	...
	eqs=[eq1,eq2,eq3]
    #coeffs,b=[(eq).lhs for eq in eqs],[(eq).rhs for eq in eqs]
	C=Symbolics.jacobian(eqs,[x,y,z])
	IC=I-C
	augment_matrix=hcat(IC,d)
    augment_matrix = Symbolics.value.(augment_matrix)          # SEE HERE
    reduce_matrix=rref(augment_matrix)
	latexify(reduce_matrix)
...
# ╔═╡ f26f8880-639f-4e53-8d87-4a97ec85b843
let
	@variables A,U,Σ,Vᵀ
	A=[0 1 0 0; 
	   0 0 2 0;
	...
	#latexify(A)
	U,Σ,Vᵀ=svd(A)
	[latexify(A),L"svd \to",L"U=", latexify(U), L"\Sigma=",latexify(diagm(Σ)),L"V^T=",latexify(Vᵀ)]
end

# ╔═╡ 17d1f37b-9cfb-472d-a14d-0b591c7e106f
begin
	...
	scatter(Score[1,:], Score[2,:],frame=:origin,label=false)
	plot!([0,3], [0, 4], arrow=true,  lw=3, color=:red, label=false)
end

# ╔═╡ c8dca9d6-c3d3-4aea-a708-26ce3e21aefa
let
	@variables  a, b
	a=1+im
	latexify(a)
...
# ╔═╡ 95496df5-fc9a-4a1f-b774-4c1d21ca23cf


# ╔═╡ ca515a2a-651a-42fd-9d27-1567715a16b8
