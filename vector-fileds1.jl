...
# ╔═╡ a4ffcb30-70be-4db7-84bb-0122f33fefe1
begin
	using AlgebraOfGraphics,CairoMakie
end

# ╔═╡ a0b8e4c0-c07a-11ec-1c90-09e5260db40e
md"# slope fields"

# ╔═╡ 626da539-f74e-4837-807d-4237751fa55c
let
	f = Figure(resolution = (800, 800))
	Axis(f[1, 1], backgroundcolor = "white")
	ps = [Point2f(x, y) for x in -5:1:5 for y in -5:1:5 ]
	
	ns = map(p ->p[1]^2+p[2]^2 , ps)
	arrows!(ps,ns , arrowsize = 10, lengthscale = 0.3)
	
	f
end

# ╔═╡ e4bee82d-fb5c-4da0-a23a-ac3cada73ef6
let
	f = Figure()
	Axis(f[1, 1])
...
# ╔═╡ 750862d7-e7a0-4f74-92c0-f5623ad18c9a
let
	x, y = collect(-8:0.4:8), collect(-8:0.4:8)
    z = [sin(X) for X ∈ x, Y ∈ y]

    surface(x, y, z, axis=(type=Axis3,), color=:red)

end

# ╔═╡ 77e8cab9-a9cd-484b-bfe8-ebf88a11eefa
let
	x, y = collect(-8:0.4:8), collect(-8:0.4:8)
    z = [sin(X) for X ∈ x, Y ∈ y]
...
# ╔═╡ 7a5ab739-e5a0-43f1-8bd1-f33ae9cb1bfe
