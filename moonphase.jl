...
# ╔═╡ 0195e715-00a9-4b5f-b7cb-5c63d581b7b9
begin
	using GLMakie, FileIO,Makie.GeometryBasics
    using Downloads: download
 
end

# ╔═╡ f457e122-d102-11ec-245c-bf8c8575cc78
md"# moon phase"

# ╔═╡ 9e1d01b5-7e0a-4e36-8a34-51079affd288
let
...
# ╔═╡ 793cce8b-65ae-43e9-b7e9-bfb44eff878e
let
    moon = load(download("https://svs.gsfc.nasa.gov/vis/a000000/a004600/a004675/phases.0001_print.jpg"))
    n = 1024 ÷ 4 # 2048
    θ = LinRange(0, π, n)
    ...
    rotate!(ax.scene, Vec3f(0, 0, 1), 3.0)  #Vec3f(0,0,1) 是z轴
	fig
end


# ╔═╡ f0738461-3697-4357-93fb-840e6482372b
let
    mars = load(download("https://nssdc.gsfc.nasa.gov/image/planetary/mars/marsglobe1.jpg"))
    n = 1024 ÷ 4 # 2048
    θ = LinRange(0, π, n)
    φ = LinRange(0, 2π, 2 * n)
    x = [cos(φ) * sin(θ) for θ in θ, φ in φ]
    ...
    fig = Figure(resolution = (1200, 800), backgroundcolor = :grey80)
    ax = LScene(fig[1, 1], show_axis = false)
    surface!(ax, x, y, z; color = mars, shading = true,
        lightposition = Vec3f(-2, -3, -3), ambient = Vec3f(0.8, 0.8, 0.8),
        backlight = 1.5f0)
    zoom!(ax.scene, cameracontrols(ax.scene), 0.65)
    rotate!(ax.scene, Vec3f(0, 1, 0), -0.4)  #Vec3f(0,0,1) 是z轴
