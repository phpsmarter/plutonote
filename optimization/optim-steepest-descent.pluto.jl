...
# ╔═╡ 18276cba-33e5-11ed-1f96-ffe149d56067
begin
	using StatsPlots, StatsBase, Symbolics, Latexify,LinearAlgebra
end

# ╔═╡ 8fa00a24-fe12-46cb-8fd9-190e714aaf9d
md"""
# 最速下降法
> 神经网络设计(第二版):p132
"""

# ╔═╡ eab63afe-2bea-412c-be29-b358a5ba26d9
begin
	@variables ∇f, x₁,x₂,α, F(x₁,x₂)
	vars=[x₁,x₂]
	F=(x₁)^2+25*(x₂^2)
	x₀=[0.5,0.5]
	α=0.01
    ∇f=grad= Symbolics.gradient(F,[x₁,x₂])
    xk(x)=Pair.(vars,x)|>Dict
	g₀=substitute(∇f,xk(x₀))
	F
...
# ╔═╡ 2bd70a56-bc96-44f4-9d4a-0efa42900891
∇f

# ╔═╡ f3f66f9c-4394-4cc7-8989-4155d8cdd5f6
H=Symbolics.hessian(F,[x₁,x₂])

# ╔═╡ cddc9aec-da36-4e6e-beb6-72fdd76f66c7
function steepest_descent(∇f, x, k_max;α=0.01) 
	 k=1
	 while  k ≤ k_max
	        Δ = substitute(∇f,xk(x))
	        x -= α*Δ
...
# ╔═╡ 5a7e3bfa-f0e5-4929-baea-169ef3d30486
f1(k)=steepest_descent(∇f,x₀,k;α=0.01)

# ╔═╡ 509e269e-0e06-4027-83a7-b2b775b32895
f1(1)

# ╔═╡ 96599ae1-5de2-433f-8d33-de10b0847f38
f1(2)

# ╔═╡ f2ea2abd-b271-4d61-89de-20abc99ba946
path1=[f1(k).|>d->Symbolics.value(d) for k in 1:100]  # 梯度的路径

# ╔═╡ ebe5eeb0-4e76-40bb-8852-2f5e04131967
begin
	span=-0.6:0.01:0.6
	f(X)=X[1]^2+25*(X[2]^2)
	xs, ys=span, span
	...
	xs1=[x[1] for x in path1]
	ys1=[x[2] for x in path1]
	plot!(xs1,ys1,label=false,ls=:dash,lc=:darkblue)
end

# ╔═╡ c21ebd4a-4a26-411e-a5d2-87b87add7297
f2(k)=steepest_descent(∇f,x₀,k;α=0.035)  #当步长为 0.035 的高阶函数

# ╔═╡ 8e607047-278d-4843-bc46-3a2db850e6ef
path2=[f2(k).|>d->Symbolics.value(d) for k in 1:100]

# ╔═╡ 66783cab-b3f0-49c8-9a4d-dfb513062297
let
	span=-0.6:0.01:0.6
	f(X)=X[1]^2+25*(X[2]^2)
	xs, ys=span, span
	zs=[f([x,y]) for x in xs, y in ys]
	contour(xs, ys,zs, label=false )
	xs2=[x[1] for x in path2]
	ys2=[x[2] for x in path2]
	plot!(xs2,ys2,label=false,ls=:dash,lc=:darkblue)
end

# ╔═╡ 691bec4d-2a7f-49ad-a06e-083e3b87e956
md"""
## 确定学习率$α$
学习率$α$,变大时的时候, 函数在向最小值收敛的时候是锯齿状的. 到底什么因素决定
"""

# ╔═╡ 1fd5adfe-1859-431f-b188-db1cf0e31449
let
	@variables  x,F(x),A,d,c,xₖ,xₖ₊₁,α,I  #I为单位矩阵
	F=1//2*(x')*A*x+(d')*x+c  # 二次函数
	∇f= Symbolics.gradient(F,[x])
	xₖ₊₁~xₖ-(α⋅∇f)
	xₖ₊₁~[I-α⋅A]⋅xₖ-α⋅d  
end

# ╔═╡ 1c296516-f5c6-48b5-ab27-03f3f8aab948
md"""
 这是一个动力系统, 可以参考线性代数及其应用的第五章内容. 
 一个动力系统要稳定, 矩阵特征值的绝对值需要小于$1$,利用黑塞矩阵就可以表示矩阵的特征值

$|(1-αλᵢ)|<1$

对于有强极小点的二次函数,特征值为正, 可以得到不等式:
...
# ╔═╡ 42d64527-1f1b-4a65-ae0e-aed1adf906da
alpha= Symbolics.hessian(F,[x₁,x₂]).|>(d->Symbolics.value(d))|>eigen|>d->d.values|>d->max(d...)|>d->2/d

# ╔═╡ b5aedaae-2a71-4c9d-b57d-1b3c75f47d73
f3(k)=steepest_descent(∇f,x₀,k;α=0.04001)  # 当alpha超过 0.04时,不会收敛, 始终处于震荡状态

# ╔═╡ f2352dab-bf71-4755-ab74-18456a60dcba
path3=[f3(k).|>d->Symbolics.value(d) for k in 1:35]

# ╔═╡ 55f56d95-86d2-461a-9a69-fbccd2d86bfe
let
	span=-0.6:0.01:0.6
	f(X)=X[1]^2+25*(X[2]^2)
	xs, ys=span, span
	zs=[f([x,y]) for x in xs, y in ys]
	contour(xs, ys,zs, label=false )
	xs3=[x[1] for x in path3]
	ys3=[x[2] for x in path3]
	plot!(xs3,ys3,label=false,ls=:dash,lc=:darkblue)
end
...
julia_version = "1.8.0"
manifest_format = "2.0"
project_hash = "32860ed50d99ddc94caea3c2caed15980a0d0f31"
