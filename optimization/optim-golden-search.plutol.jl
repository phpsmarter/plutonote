...
# ╔═╡ 7c81bfa0-31d6-11ed-0036-459bbe26c013
begin
	using Symbolics, LaTeXStrings,LinearAlgebra,Latexify,Plots
	using Symbolics:gradient,jacobian
end

# ╔═╡ 806fdd4a-47f2-4af0-b24e-363f2fdb5ba8
begin
    f(x)=x^4-14(x^3)+60(x^2)-70x
	a₀,b₀=0,2
	range2=a₀:0.01:b₀
	ρ=0.382
	N=log(0.15)/log(ρ)
	p1=plot(f,range2,label=false)
end

# ╔═╡ 16b104d3-7c3a-48b1-b8bd-82eaa0ba4a05
add_text(key, val)=(val,-24.5,
text("$(key)", pointsize=12, halign=:right, valign=:center, rotation=0))

# ╔═╡ 120dacc4-5706-483c-b35a-aadc84dc1e6a
begin
	
	
	a₁=a₀+ρ*(b₀-a₀)
	b₁=a₀+(1-ρ)*(b₀-a₀)
	data=Dict("a₀"=>a₀,"a₁"=>a₁,"b₀"=>b₀,"b₁"=>b₁)
	texts=[add_text(key, val) for (key, val) in data]
	val=[a₀,b₀,a₁,b₁].|>x->f(x)
	plot(f,range2,label=false)
	plot!(repeat([a₀,b₀,a₁,b₁]',2),[[-25,-25,-25,-25]';val'],label=false,xticks =[ a₀,b₀,a₁,b₁],ann=texts)
	
end

# ╔═╡ 089dcd9e-47a8-4f4e-b8cd-a8d8bfb7e7a0
begin
	...
	a₂=a₀+ρ*(b₁-a₀)
	d1=[a₀,a₂,b₁,b₂]
	data2=Dict("a₀"=>a₀,"a₂"=>a₂,"b₂"=>b₂,"b₁"=>b₁)
	texts2=[add_text(key, val) for (key, val) in data2]
	val2=[a₀,a₂,b₁,b₂].|>x->f(x)
	plot(f,range2,label=false)
	plot!(repeat([a₀,a₂,b₁,b₂]',2),[[-25,-25,-25,-25]';val2'],label=false,xticks =[a₀,a₂,b₁,b₂],ann=texts2)
	new_range2=range(a₂,b₁,40)
	span3=[f(x) for x in new_range2]
	plot!(repeat(new_range2',2),[repeat([-25],40)';span3'],color=:green,label=false)
