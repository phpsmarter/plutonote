...
# ╔═╡ b3df28ae-31ca-11ed-1ee3-2b904ec41e26
begin
	using Symbolics, LaTeXStrings,LinearAlgebra
	using Symbolics:gradient,jacobian
end

# ╔═╡ ba0f99e2-db2b-4700-b4d1-b550b73b1929
begin
	 @variables x₁,x₂,y(x₁,x₂)
	 y=5x₁+8x₂+x₁*x₂-x₁^2-2x₂^2
	 D₁ = Differential(x₁)
	 D₂ = Differential(x₂)
     grad=gradient(y,[x₁,x₂])
	jacmaxtrix=jacobian(grad,[x₁,x₂])
end

# ╔═╡ 4ec5bfad-d729-4e70-ba74-d8c4c332e095
md"""
!!! example 
    example 1 
    
	多元函数$f:R^3->R:f(X)=x₁*x₂*x₃$ 方向向量为$d=[\frac{1}{2},\frac{1}{2},\frac{1}{\sqrt{2}}]^T$
   
   $f$ 沿方向向量的方向导数为:
"""


# ╔═╡ e45830e4-7d6b-4766-9dd3-8619de17fbe4
let
     ################################################
	 # 方向导数定义为在该处的梯度与方向向量的点积
	################################################
   @variables x₁,x₂,x₃,y(x₁,x₂,x₃)
	 y=x₁*x₂*x₃
     grad=gradient(y,[x₁,x₂,x₃])
	 dvec=[1/2,1/2,1/√2]
	 dd=dot(grad,dvec)
