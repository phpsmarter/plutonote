...
# ╔═╡ a188b1a2-2cc0-11ed-0303-599f888a9c0d
begin
using Distributions, StatsPlots,Latexify
end

# ╔═╡ ea57a90a-4fe1-40ae-a667-d858bac75ab7
md"""
##  Julia  统计软件包里的临界值方法
"""

# ╔═╡ 62aa78cc-cee4-4251-a5c4-43d64027e199
d=Normal()

# ╔═╡ da9b51ea-66b5-4149-ae46-293d1713ac09
plot(d,label="Normal distributions")

# ╔═╡ 0a11bc24-4b85-42ff-8459-cbdfcbefb73a
begin
	percent=[1,5,10,25,50,75,90,95,99].|>(d->d/100)
	str=[1,5,10,25,50,75,90,95,99].|>(x->"$x%")
	println(str)
	n1=length(percent)
	quant=percent.|>(x->quantile(d,x))
	pdfs=quant.|>(x->pdf(d,x))
	 plot(d,label="Normal distributions",lw=2)
	 plot!(repeat(quant', 2),
         [zeros(1, n1);pdfs'], label = "", color = :grey, alpha = 0.5)
    	
end

# ╔═╡ 463ef384-f089-4758-943d-1093f8a07943
begin
	Δx=0.2
	plot(d,label="Normal distributions",lw=2)
	range=-3:Δx:3
	critical_valule=range.|>x->pdf(d,x)
	bar!(range,critical_valule,fillcolor=:green,label=false,fillalpha=0.2)
end

# ╔═╡ 41c2a4bd-cff1-4275-99f3-b4fc7093231a
begin
  n=length(range)
  range.|>(x->pdf(d,x)*Δx)|>sum
end
...
manifest_format = "2.0"
project_hash = "ab2ee3337cc8e7a45f726289d8469363c21668f6"
