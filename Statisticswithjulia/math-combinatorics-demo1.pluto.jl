...
# ╔═╡ 218dc3f0-2763-11ed-307a-cb049f61b140
begin
	using Combinatorics,Latexify, LaTeXStrings,Symbolics,Distributions,Shuffle
end

# ╔═╡ e8178aab-4a7f-4b67-921b-5f2fd64bee93
permutations(["t","f"])|>collect

# ╔═╡ e4fa1a19-e56c-4ae5-8e66-3ea4d3829405
permutations([1,0,1,0],2)|>collect

# ╔═╡ dc180343-41bd-4808-9698-afba7b7b7ca5
combinations([1,0,1,0],2)|>collect

# ╔═╡ 9d653d00-6708-4326-84fd-4832c70a1f76
strarr=["a","b","c"]

# ╔═╡ d860a4c2-ef78-4695-8d90-fe6c85097457
combinations(strarr,2)|>collect

# ╔═╡ 20e924ad-7c04-4b41-a33e-b1a37d9445e9
permutations(strarr,2)|>collect|>length

# ╔═╡ 42d231ae-5477-447c-8efa-2d6828ffd9fc
char=range('a','z')

# ╔═╡ a8a3ff63-6b8d-4df5-b846-d6860fed7f6c
length(char)

# ╔═╡ 66c18546-a136-4a8e-90c5-b2ac004ba116
dig=0:9

# ╔═╡ b9f35050-70b9-4539-80a5-7da3c9716729
first3=permutations(char, 3)|>collect|>length

# ╔═╡ 5889f81b-317f-4aa3-980a-a37f6395af8f
last4=permutations(dig, 4)|>collect|>length

# ╔═╡ eb40d9bb-7b45-451f-94d8-ff6d1e9e78a5
first3*last4  #不允许重复的车牌, 例如 aaa 不行

# ╔═╡ f37c6a6a-a0e5-4e3c-b850-0671000d0432
permutations('a':'c',3)|>collect.|>latexify

# ╔═╡ 1b11bb5d-5a03-448d-9e94-f0ea80a8d175
factorial(4)

# ╔═╡ ffc61492-1202-4c4c-bef2-c08872e17926
factorial(4)*factorial(6)

# ╔═╡ a01c79a4-aad7-453e-a4ab-244a1579ad16
[4,3,2,1].|>(b->factorial(b))|>prod

# ╔═╡ 0c5defdf-9d82-4af9-924a-4c6ac3b2ce15
permutations(['p','e','p','p','e','r'],6)|>length

# ╔═╡ 80981ae1-4f65-4ef8-b333-885034fbbbae
factorial(6,3)  #6*5*4

# ╔═╡ 9f06f6b4-3932-442d-a5ad-f7abfb59322f
factorial(6)

# ╔═╡ 3ef8471e-e2ec-4bec-8071-6cf6bf278424
str2=['🔴','🔵','🟡','🟢','🟣']

# ╔═╡ 701646f4-1d26-41f1-a7da-da6b6f6374b6
combinations(str2,3)|>collect.|>latexify

# ╔═╡ e3faf99c-4e30-4f43-9803-2e92c5f1a1e5
combinations(1:20,3)|>length

# ╔═╡ 1d8beb8e-c0ee-4a9d-b316-7d4e56a3f599
begin
	@variables x,y
	multiexponents(2, 3)|>collect.|>latexify
	expr=(x+y)^3|>expand
	#multinomial(())
end

# ╔═╡ 88ba44fd-b4e5-49d3-8a59-8f4cfa082cce
begin
	is7(d)=d[1]+d[2]==7
	isnot7=!is7
	sample_space=[[a,b] for a in 1:7 , b in 1:7]
	A=sample_space|>d->filter(is7,d)
	B=sample_space|>d->filter(isnot7,d)
    [sample_space,A,B].|>length
	intersect(B,sample_space)|>length
	intersect(B,sample_space)|>length|>d->println("size A∩S=",d)
	union(A,B)|>length|>d->println("size A∪B=",d)
end


# ╔═╡ 8dbe75f9-72c1-4f67-89a9-0192e630a2c9
begin
p=1/6
dice=Multinomial(100, vec(fill(p,6)))

#sum(vec(fill(p,6)))
end

# ╔═╡ 0906e2ed-76bf-47d4-a90d-dc6cbeefa3c8
begin
dice2=DiscreteUniform(1, 6)|>probval
	
end

# ╔═╡ 0443cef5-9791-4d0b-9691-0bc8f2235f1f
begin
	suite=Set(["♣️","♦️","♥️","♠️"])
	number=Set(['1','2','3','4','5','6','7','8','9','J','Q','K','A'])
	poker=[join(s,n) for s in suite, n in number ]|>vec|>shuffle
	# first_round=rand(poker,4)
    # poker=setdiff(poker,first_round)
	# second_round=rand(poker,4)
	# poker=setdiff(poker,second_round)|>length
end
...
manifest_format = "2.0"
project_hash = "e69a0b0e88d499804db590875929a117cf187365"
