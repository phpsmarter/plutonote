...
# ╔═╡ 3804a816-1e9d-11ed-2086-0704207c25c4
begin
	using Latexify,PlutoUI,RDatasets,DataFrames,Random,FileIO,HTTP,Images,Plots, StatsPlots,Statistics,StatsBase,LaTeXStrings,Symbolics,ImageView
   
end

# ╔═╡ f52ea471-014f-4fb3-97a2-fffaaebdc610
begin
	using CairoMakie
	f = Figure(800,800)
...
# ╔═╡ 2b5727e0-5e91-4c49-86ce-a2d8d1157b34
@bind sample CounterButton("掷色子!")

# ╔═╡ 956ca08b-782e-4c68-a24f-f16160178160
begin
	@variables matrix
    range=1:6
	matrix=[[i j ] for i in range , j in range]
	latexify(matrix)
end

# ╔═╡ 4034ca14-8117-4150-b13c-81092699fcd7
let
	@variables matrix
    range=1:6
	matrix=[i+j for i in range , j in range ]
...
# ╔═╡ f95211d4-c19f-45de-b57c-be37fa4e75a9
begin
	@variables A, P(A),events,space,N,faces
	N, faces = 10^6, 1:6 
	events=sum([iseven(i+j) for i in faces, j in faces])
	space=length(faces)^2
    P~events/space
end

# ╔═╡ 46c91261-4c59-47a3-a421-0671c176340e


# ╔═╡ b15c2aeb-a84f-4556-a641-37a89e1de8a7
...
# ╔═╡ c1b4c522-b45e-4b13-b4e5-60f7433978e8
begin
	url1="https://tva1.sinaimg.cn/small/e6c9d24egy1h5aq332qs5j22380astah.jpg"
	url2="https://tva1.sinaimg.cn/small/e6c9d24egy1h5aq35fwbej222k0a6q47.jpg"
	white_img= load(download(url1))
	black_img=load(download(url2))
    wd=white_dice=sprite(white_img,1,6)
	bd=black_dice=sprite(black_img,1,6)
    ()
end 

# ╔═╡ e151d481-3b88-49c6-8335-9cd93314374e
begin
	sample
    [wd[rand(1:6)],bd[rand(1:6)]]
end
...
manifest_format = "2.0"
project_hash = "07fb09c392aefce5a787aa75180a4687d737aab3"
