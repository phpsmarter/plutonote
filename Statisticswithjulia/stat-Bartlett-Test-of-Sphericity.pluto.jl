...
# ╔═╡ 970b2e34-228f-11ed-24ba-3d7a07434a58
begin
	using PlutoUI,RDatasets,DataFrames,Random,Plots, StatsPlots,Statistics,StatsBase,Distributions,HypothesisTests,LinearAlgebra,RCall,Latexify
    TableOfContents(title="Bartlett’s Test of Sphericity")
end

# ╔═╡ 622fa0a4-9c42-4fe4-99b4-85b367151bc5
md"""
#  Bartlett’s Test of Sphericity  检验
"""

# ╔═╡ a9bcd592-40c1-4200-9833-a52f28f2b20f
md"""
!!! info
    Bartlett’s Test of Sphericity 比较观察协方差矩阵与全等矩阵.  

    目的是检测观测矩阵是否有冗余列. 也就是检测是否有线性相关列存在.  

    单位正交矩阵的向量是相互垂直的. 所以也可以形象的认为 这个检测是在检查观察协方差矩阵的列是否正交.  如果列之间都是正交关系, 则没有冗余列 

    

    **Bartlett’s Test of Sphericity 的零假设就是:  矩阵中列都是正交的**

要注意Julia 没有用于Bartlett’s Test of Sphericity 的方法 :

`BartlettTest(X::AbstractMatrix, Y::AbstractMatrix)`  方法是用于方差齐性检测,不是用于检测协方差矩阵冗余列的方法


这里我们使用`DataFrame.jl` 生成数据, 然后调用 `R` 语言的方法来实现 这个假设检验
"""

# ╔═╡ 0ce76aaa-2d92-4e4c-962f-3b2546c25e72
struct Statparms
		μ::Float64
		σ::Float64
end

# ╔═╡ 1530ff8b-a0f6-4cc4-8135-385a22580bcf
data=[Statparms(1,4),Statparms(3,6),Statparms(5,8)].|>p->Normal(p.μ,p.σ)  #生成三组正态分布数据

# ╔═╡ 6f395631-a8bb-4f80-8335-0bd135638884
function make_df(data,n=50)
    return data.|>(d->rand(d,n))|>(x->DataFrame(x,:auto))
end

# ╔═╡ 2845e58d-5a58-468d-b107-0c7dfe14688a
function bartlett_test_of_sphericity(df)
	@rput df
	R"""
	 #  1. 导入软件包
	 ...
	 #  3. 运行检测函数
	   ...
	   bartres<-cortest.bartlett(cor_matrix, n = nrow(df))
    """
	return @rget bartres
end

# ╔═╡ 9c0bd50b-c318-4c27-bba8-f78f766389d3
barttest=data|>make_df|>bartlett_test_of_sphericity #执行检测

# ╔═╡ b03f75c6-a9c6-49a1-853f-6c52831484d7
latexify(barttest,env=:mdtable)

# ╔═╡ f75cb568-71fe-4c04-9742-922526333fc3
md"""
  统计结果:  $p-value:$ $(barttest[:p_value])
  
  结论:  $p-value>0.05$, 推断数据集没有冗余列
"""

# ╔═╡ d3947b75-78d5-4185-aefc-aa7540aef23c
Random.seed!(0)
...
manifest_format = "2.0"
project_hash = "78ae5f4258f680ce4d26f456dd21572b7db0e167"
