...
# ╔═╡ 568a00aa-1ecb-11ed-0705-43bff6e45ab9
begin
	using Latexify,PlutoUI,RDatasets,DataFrames,Random,FileIO,HTTP,Images,Plots, StatsPlots,Statistics,StatsBase,LaTeXStrings,Symbolics
    TableOfContents(title="变异程度的测量")
end

# ╔═╡ bc97bead-6fe4-4598-a379-ecdd1b1c25d3
md"![](https://tva1.sinaimg.cn/large/e6c9d24egy1h59xcrdsggj20dp07haac.jpg)"

# ╔═╡ 6d41dc82-957a-4cc6-94f1-9e38f16d2314
md"""# ch03 数据集合变异程度的测量
...
# ╔═╡ c825911b-e3e5-4274-9eb9-7ddaacaa5d25
md"""
## 极差 

极差统计的是数据集合中某属性最大测量值和最小测量值的差值, 常用的$25\%,75\%$处的数字也属于这个范畴.
...
# ╔═╡ ccdc060c-4689-4efe-a055-1053c3ef925b
md"""
!!! example 
    example 1 
    就散下面一组样本数据的离差
...
# ╔═╡ 2ef2f70b-c3f3-450b-9cd4-c00ebdd5d578
sample_data=[3,5, 6, 7,8, 4, 5, 6]

# ╔═╡ aeb3bc22-7a45-4b2d-a77e-f478620fc2cd
dev_arr=sample_data.-mean(sample_data)

# ╔═╡ f9adbc82-9988-43d8-b319-f152f1feb69d
begin
df=DataFrame(i=sample_data,x=dev_arr)
rename!(df, Dict(:i => "X", :x => "X-X̄"))
end

# ╔═╡ 59792880-277e-4d3f-af40-f829b2f05af4
md" 如果直接统计离差的和, 无论什么数据集合结果都会是 $0$,所以需要解决这个问题,引入方差就是要解决这个问题"

# ╔═╡ 59615223-40a5-4ee6-a0e9-ee0bcabdadd8
sum(dev_arr)

# ╔═╡ 60ba6964-f1b7-4846-a265-73ed60525f47
md"""
## 方差和标准差
...
![](https://tva1.sinaimg.cn/large/e6c9d24egy1h5azz80axoj21d00qigqd.jpg)
"""

# ╔═╡ 2fedf657-ff0d-455b-b111-a830330a485b
md"""
 ### 平方的意义
...
# ╔═╡ 0c0e3ef8-2494-442d-88db-ec454e1d3fe6
md"""
!!! example
    example  1
    下面数据为工作日随机抽取的管道的直径测量值. 
"""

# ╔═╡ 2983b817-7500-4f95-8c65-797ab300980e
tube=[53.35674558144255
53.45887516134873
52.282838627926125
52.98746570643515
51.993167774733486
...
# ╔═╡ cf4f399c-c80e-45bd-a256-55f2648cec8b
describe(tube)

# ╔═╡ cf47a366-962e-4053-9894-ff441b0597e7
var(tube)  #  方差

# ╔═╡ ea588ab9-f929-483a-9c22-b9ba7f41c8b1
std(tube)  # 标准差

# ╔═╡ 737e26c6-493a-452d-ab0e-24201979ac2c
md"#### 自定义样本方差和标准差方法
...
# ╔═╡ 23a324cb-8212-472a-a15b-50f25d1c1c4f
SS=dev_arr.^2|>sum

# ╔═╡ db2168ee-e9e6-4942-a8f5-71ea952ca7f2
md"方差定义为:离差平方和除以样本数减 $1$

$Var=\frac{SS}{n-1}$
"

# ╔═╡ d3ccfc5c-395b-4a3f-b4a4-d7ca8902ba76
Var=SS/(length(sample_data)-1)

# ╔═╡ e24ee0c6-cb25-4f7d-a852-490f95854bfc
md""""
标准差定义为方差执行开方操作
...
# ╔═╡ 170a24a4-15b9-4d15-9b7b-136c9fbeae20
Std=sqrt(Var)

# ╔═╡ 5373aea3-e2b1-4568-9992-acd87f9c6cbc
md"""
看起来比较复杂, 实际从内部到外层执行, 写成一个管道操作形式比较好理解, 管道中每个组件完成一项操作
"""

# ╔═╡ a0bcc801-9a24-4b1a-b32d-d0f9cbfa2c17
function var2(data)
	...
	return  data.|>sub_mean.|>square|>sum|>div
end

# ╔═╡ 77e68aaa-59e2-493b-a620-84008295672a
md"""
标准差也采用管道操作
"""

# ╔═╡ 7cbcc888-54ff-47e6-8924-4c725e9a04f1
std2(data)=data|>var2|>sqrt

# ╔═╡ 9cd1499c-ee17-4153-ad72-76421b00cc01
["var2"=>var2(tube),"std2"=>std2(tube)]

# ╔═╡ e336416f-2f87-4caa-8c34-99d592c11ba3
svar(data::Vector)=(sum((data.-mean(data)).^2))/(length(data)-1) #另一而种定义,比管道符号难理解

# ╔═╡ b59ec5db-b7cf-4f37-b252-e16da04ca4db
stdev(data::Vector)=sqrt(svar(data))

# ╔═╡ 0b3e7dc6-0c31-4611-bb2c-22b31b04a709
["var"=>svar(tube), "std"=>stdev(tube)]

# ╔═╡ 989c3a00-217e-44ec-9d59-ed0d880cbc8a
md"""
当方差接近于$1$ 的时候, 标准差也接近于 $1$, 这有点特殊, 但是方差和标准差的单位其实不同, 方差开方以后单位才和原统计值一样. 也就是标准差的单位和统计值单位一样. 

利用标准差可以直观的表示测量值和均值之间的平均差异.  但是方差并不是多余的, 因为计算标准差之前一定要先计算方差, 而且在很多统计方法中需要首先计算方差作为中间步骤. 
"""

# ╔═╡ 3773f98d-ec77-4f33-9dca-7295821e7ae4
[1.0,1.01, 1.1, 1.2,2,4].|>sqrt #接近于1的值开方后也接近于1,但是如果带有物理单位,两者量纲不同

# ╔═╡ 6e182bd4-4d6b-4768-b27f-fe0a37cb0b54
md"""
### 计算机程序的差异
...
# ╔═╡ bbc35490-2f94-46f6-989e-3c7e6ef83d94
md"""
!!! summary
...
manifest_format = "2.0"
project_hash = "8f2b12f997083e96c826693f1c208605b927e166"
