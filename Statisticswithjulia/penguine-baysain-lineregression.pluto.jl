...
# ╔═╡ 35e92ec4-28f4-11ed-3a39-8ffc8a1e2e8b
begin
	using Distributions, StatsPlots, Turing ,DataFrames,MCMCChains, Plots,Random,PalmerPenguins
...
# ╔═╡ 85ff1bca-d83b-485e-b824-f95370c3c64d
md"""
!!! title
    使用贝叶斯多项式逻辑斯蒂回归对企鹅进行分类

    使用 Turing.jl 的贝叶斯多项式逻辑斯蒂回归对企鹅种类进行分类:

    - 变量为: 喙的长度和高度, 脚蹼的长度, 体重. 

    - 分类目录为:["Adelie", "Gentoo", "Chinstrap"]
    
"""

# ╔═╡ 32bc0168-d0b9-4e03-b2a7-bec3b6de0516
data = dropmissing(DataFrame(PalmerPenguins.load()))

# ╔═╡ 0db5cc2f-372c-424d-a936-d9ff67ddd70e
names(data)

# ╔═╡ 481d7d83-3434-47df-9ffc-8fae6fc43dbf
begin
...
# ╔═╡ 5b337b60-70ff-4875-b7e5-f1610f1e048c
begin
...
colnames=[:bill_length_mm,:bill_depth_mm,:flipper_length_mm,:body_mass_g]
...
# ╔═╡ 496b9455-4989-418c-80f9-29af62f5fcb3
begin
	...
	@model function logistic_regression(x, y, σ)
	    ...
	    coefficients_Gentoo ~ MvNormal(4, σ)
	    coefficients_Chinstrap ~ MvNormal(4, σ)
	
	    # Compute the likelihood of the observations.
	    values_Gentoo = intercept_Gentoo .+ x * coefficients_Gentoo
	    values_Chinstrap = intercept_Chinstrap .+ x * coefficients_Chinstrap
	    for i in 1:n
	        # the 0 corresponds to the base category `setosa`
	        v = softmax([0, values_Gentoo[i], values_Chinstrap[i]])
	        y[i] ~ Categorical(v)
	    end
	end;
end

# ╔═╡ a7ec99b9-02f9-4565-b360-f8a3e107e1b2
begin
	m = logistic_regression(train_features, train_target, 1)
	chain = sample(m, HMC(0.05, 10), MCMCThreads(), 1_500, 3)
end

# ╔═╡ 5a19a851-c4cc-4208-a20c-cc1bcac0ab63
plot(chain)

# ╔═╡ a3277f39-33c4-4075-bc02-60661b908ac5
corner(
    chain,
    MCMCChains.namesingroup(chain, :coefficients_Gentoo);
    label=[string(i) for i in 1:4],
	
)

# ╔═╡ 20c33fbe-66ad-4f0a-9628-93ba010cc92a
corner(
    chain,
    MCMCChains.namesingroup(chain, :coefficients_Chinstrap);
    label=[string(i) for i in 1:4],
	
)

# ╔═╡ 7323bc57-359b-46a7-9266-64c235afb1af
function prediction(x::Matrix, chain)
    # Pull the means from each parameter's sampled values in the chain.
    intercept_Gentoo = mean(chain, :intercept_Gentoo)
    intercept_Chinstrap = mean(chain, :intercept_Chinstrap)
    coefficients_Gentoo = [
        mean(chain, k) for k in MCMCChains.namesingroup(chain, :coefficients_Gentoo)
    ]
    coefficients_Chinstrap = [
        mean(chain, k) for k in MCMCChains.namesingroup(chain, :coefficients_Chinstrap)
    ]
    ...
    values_Chinstrap = intercept_Chinstrap .+ x * coefficients_Chinstrap
    species_indices = [
        argmax((0, x, y)) for (x, y) in zip(values_Gentoo, values_Chinstrap)
    ]

    return species_indices
end;

# ╔═╡ 2fe1dc8a-6611-49d8-bc13-d01d43c917cd
begin
...
# ╔═╡ 47afd1c0-378c-44b6-b908-7a09d0297d87
begin
	for s in 1:3
...
manifest_format = "2.0"
project_hash = "d689dbf6480e43dcefe4780ea4b46a6e3a9a6bd5"
