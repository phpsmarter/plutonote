...
# ╔═╡ f7644a60-1f60-11ed-3c63-c9226928840d
begin
    ...
    TableOfContents(title="频数统计")
end

# ╔═╡ c9f77815-ae70-48af-9249-a125c1351c1b
md"""
## julia语言频数统计使用`StatsBase`方法 :

1. `extrema`: 获取最小和最大值
2. `span` : 生成最小值和最大值,间隔为 1 的数组
3. `counts`: 获取频数统计值
4. `fit, Hisgogram` :生成分组区间频数统计值
---
"""

# ╔═╡ 1bcf0acf-5fa7-4048-afba-6ed2fb24c7c8
md"""
!!! example
    example 1      
...
# ╔═╡ 0d36accb-944b-49cc-a1d2-c57105e8b21d
data1=[8,9,8,7,10,9, 6 ,4, 9, 8 ,7 ,8 ,10, 9, 8, 6, 9, 7, 8 ,8]

# ╔═╡ 2d44972f-eec1-441d-ad07-23d610c99961
min,max=extrema(data1)  #极值

# ╔═╡ 795104eb-f643-4ef2-865e-052f5a88478a
xf=DataFrame(X=span(data1),f=counts(data1)) #生成数据框

# ╔═╡ 77774d45-d88a-4988-8bc0-88c0bb728555
bar(xf[:,1],xf[:,2],label=false,xticks=(span(data1)))  # 没有分组的条形图

# ╔═╡ 4b134f45-d0f3-472a-8cda-a3a76c80adbb
begin
	#以 2为间隔装箱
	bins =min:2:max;
    h = fit(Histogram, data1, bins,closed=:right)
end

# ╔═╡ 34b837dd-c623-4cc8-8730-8d4a1448626b
begin
    ymin,ymax=extrema(h.weights)  #获取频数的区间范围
 	bar(h,label=false,yticks=(ymin:ymax))  # 分组频数结果
end

# ╔═╡ 2fb0e8e6-e539-4142-a663-f3571994ea0f
md"""
...
# ╔═╡ e40da066-b516-4e9a-9665-353200b3053b
data2=[14 8 27 16 10 22 9 13 16 12 10 9 15 17 6 14 11 18 14 11]

# ╔═╡ 6429c094-85dd-4672-8f50-88d57ebb3c1e
xf2=DataFrame(X=span(data2[1,:]),f=counts(data2[1,:]))  #数据封装使用了矩阵形式, 所以提取时用矩阵形式,下同

# ╔═╡ 85ca2353-c81a-43ab-93ab-ba20b65b2193
bar(xf2[:,1],xf2[:,2],label=false,xticks=(span(data2)))  # 没有分组的条形图

# ╔═╡ a9e53b00-489c-48d2-83f5-70b0b031bfd0
begin
	min2,max2=extrema(data2)
	bins2 =min2:4:max2; #以4为间隔装箱
    h2 = fit(Histogram, data2[1,:], bins2,closed=:left)
	ymin2,ymax2=extrema(h2.weights)  #获取频数的区间范围
 	bar(h2,label=false,yticks=(ymin2:ymax2),xticks=bins2,alpha=0.5,gridlinewidth=2,fgcolor_grid=:red,gridalpha=0.3)  # 分组频数结果
