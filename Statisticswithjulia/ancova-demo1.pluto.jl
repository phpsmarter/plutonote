...
# ╔═╡ e3e112b2-1b7a-11ed-3602-39767f553558
begin
	using RCall,Plots,StatsPlots,DataFrames,HypothesisTests
end

# ╔═╡ 98ffe888-7c01-49ab-b7d6-4fd75a6cfdca
begin
	R"""
    set.seed(10)
...
# ╔═╡ 764a8132-0737-4cb4-b372-311b1884c717
R"summary(data)"

# ╔═╡ c5b631d1-beb2-499f-ae55-f6e8f9f38225
begin
data=@rget data
  df=DataFrame(data)
end

# ╔═╡ 9d4a96ce-0f0b-4ae7-9ba6-cec4afcd81ed
gd = groupby(df, :technique)

# ╔═╡ 6178300a-f818-4e94-97ed-c854eac11dba
begin
	plot(title="group",xticks=(1:3),xlabel="Studying Technique",ylabel="Exam Score")
	boxplot!(gd[1][:,3],label="A")
	boxplot!(gd[2][:,3],label="B")
	boxplot!(gd[3][:,3],label="C")
end

# ╔═╡ 43381ddc-7de9-4762-b779-2b715edee589
begin
	plot(title="group",xticks=(1:3),xlabel="Studying Technique",ylabel="Current Grade")
	boxplot!(gd[1][:,2],label="A")
	boxplot!(gd[2][:,2],label="B")
	boxplot!(gd[3][:,2],label="C")
end

# ╔═╡ d729e665-1072-42e4-8a9b-a94ff9796b5e
md"""
  > 首先执行单样本 T检验, 检验目的是看一下参与三个不同教育方法之前,三个抽样组之间的差异信息
"""

# ╔═╡ 40ef4e81-de7f-4072-adc1-a035041f79d7
begin
	res=OneWayANOVATest(gd[1][:,2],gd[2][:,2],gd[3][:,2])
end

# ╔═╡ 79578455-6681-428f-8129-4eaac19b62c0
md"""
$\because pvalue= 0.5517 > 0.05$
$\therefore  三个组之间当前成绩没有差异$
...
# ╔═╡ a02a461f-bb25-4745-a4b7-13165e7a4486


# ╔═╡ 5ed46f3c-6c67-425e-8e21-84d5bae088ee


# ╔═╡ 3bd5404a-633d-495c-b9eb-2e526ffeabbd
md"""
!!! info
    fail to reject h_0 的意思是由于 p值较大, 根据目前信息无法得出分配到$A,B,C$ 三个组的学生属于不同的总体
"""

# ╔═╡ ec0baece-fa9f-4235-bfca-814eddb7df96
BrownForsytheTest(gd[1][:,2],gd[2][:,2],gd[3][:,2])

# ╔═╡ 8bae9bc1-c12a-40bd-98e5-68ad5deeb882
md"""
> 对实验后的三个分组测试成绩执行方差齐性检测
"""

# ╔═╡ 33e211ad-dfa7-4f6b-a321-f051ff048bdc
LeveneTest(gd[1][:,3],gd[2][:,3],gd[3][:,3])

# ╔═╡ 551b1eb9-b390-4702-85a8-df45810f521d
begin
	R"""
	 library(car)
...
# ╔═╡ 86fca557-3850-44bc-8c04-e3b891e75cd7
begin
...
manifest_format = "2.0"
project_hash = "6a814a36e001855b4f8f67f5d03082ec411ea42e"
