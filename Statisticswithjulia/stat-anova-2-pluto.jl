...
# ╔═╡ c7b98456-25e9-11ed-23d5-613b0ba82d65
begin
	using CSV, Distributions, HypothesisTests,HTTP,DataFrames,Statistics,Plots,RCall,StatsPlots,PlutoUI,StatsBase
	TableOfContents(title="ANOVA 2")
end

# ╔═╡ 5459e5cb-3790-4288-a171-04663cc8ca75
md"""
# 检测五个使用者在四个不同网页上停留时间的区别
"""

# ╔═╡ cd4d611c-c468-418d-af22-58dfcab00d9b
data=[
    164 172 177 156 195 ;
    178 191 182 185 177 ;
    175 193 171 163 176 ;
...
# ╔═╡ af8d57aa-615a-492d-ae7d-5846fa409b5e
df=data|>data->Dict("page$(i)"=>data[i,:] for i in 1:4)|>DataFrame

# ╔═╡ e36fb31a-b260-47e1-8269-f9fc306261cf
begin
	bp(idx)=boxplot!(["page$idx"],df[:,idx], label=false,outliers=true)
	plot()
	plot!(bp(1),bp(2),bp(3),bp(4),label=false)	
end

# ╔═╡ b8e3f61a-3728-4713-9153-dda1b48512bc
fa=OneWayANOVATest(eachcol(df)...)
...
manifest_format = "2.0"
project_hash = "14e764badc0c0534f5eaed3789fa6fe11f9d3167"
