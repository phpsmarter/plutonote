...
# ╔═╡ 31a723c8-3023-11ed-27a4-050a47673e33
begin
...
# ╔═╡ d4c1579d-443a-4d9a-bc02-1f9bbe8c8049
md"""
# 数据的来源-2
...
!!! info
    ...
    因为正弦曲线在$[0,2\pi]$定义域区间内的函数值分为正半周和负半周. 如果随机从定义域区间取点求函数值, 理论上说从从每次都可以获得两个绝对值相同的点(值的符号相反), 所以每次抽取的点的和理论上应该等于$0$, 由于随机性, 不可能正好等于$0$, 如果我们重复多次,得到的值应该分布在$0$的附近. 

    以正弦函数为基础采样的样本也有自己的均值和标准差(方差也可以).   对于正弦函数, 函数图形是直观的, 抽样的散点图可以看出来,但是对于一个未知的函数如果均值
"""

# ╔═╡ 7ac8ecfc-f2ff-4241-921d-03dde17799d6
begin
	######################################
...
# ╔═╡ c8a5acdb-9eed-4416-8283-81289452249a
df=DataFrame(x=ran,sinx=data)

# ╔═╡ ec641461-884c-4163-a9a1-d391988b9015
md"""
如果定义一次实验为:从曲线中随机抽取 100 个点.  这 100 个点的值可以求出均值和标准差.

根据上面的的分析, 均值应该在 0左右. 这里的标准差和正态分布的标准差不同(因为曲线和正态分布不同),也不符合概率密度定义, 因为曲线下面积不为 $1$

这样的实验, 我们重复 500 次, 看看每次获得的 100 个点的均值和标准差的情况. 
"""

# ╔═╡ 1cf10a3d-985b-4ca9-aa11-38af84beb07e
begin
...
# ╔═╡ 00abb955-4d56-4181-b129-20c1b7c1acd5
md"""
可以看到:1000 次重复实验的均值大部分都集中在 0附近. 
...
# ╔═╡ a3b16995-e24e-45ae-ac95-bffade66f87a
...
# ╔═╡ 5ae2be0e-7ce2-4ee9-b499-94c9d48f4e30
begin
...
# ╔═╡ 4cc99e87-f9d7-4d8e-b519-c5b7b0d5ee69
function sin_scatter(num)
	 xs=rand(df[!,1],num)|>sort
     ys=sin.(xs)
     y_means=mean(ys)|>d->round(d,digits=2)
     scatter(xs,ys, title="sample:$(num)", label="mean:$(y_means)", ms=2,ma=0.4,mc=:red,frame=:box)
end

# ╔═╡ f42d8d16-7a44-40e7-940f-ec66274a7ce9
...
julia_version = "1.8.0"
manifest_format = "2.0"
project_hash = "8b5fc2bbe284342a8cedf9ba7cbb9e60a7237925"
