...
# ╔═╡ 21b8e126-3306-11ed-31fc-57dd1df8856f
begin
	using StatsKit,StatsPlots,Distributions
end

# ╔═╡ 5e7c93d7-70d4-4031-8db5-b8e85e88a5cc
begin
	span=0:0.1:5
	line(x)=2x
	get_rand_val(x)=(rand()-0.5)*0.4+line(x)
	randval=[get_rand_val(x) for x in span]
   
	n1=Normal(1,2)
	BB1 = bbox(0.1,0.0, 0.50,0.45)
	BB2 = bbox(0.1,0.2, 0.30,0.35)
	BB3 = bbox(0.1,0.55, 0.10,0.25)
	plot()
	 plot!(span, randval, label=false)
	plot!(n1,inset = (1, BB1), subplot = 2,axis=false,label=false,permute=(:x,:y),ylims=(0,1.2),flip=true,color=:red,frame=:none,background=false)
	
	plot!(n1,inset = (1, BB2), subplot = 3,axis=false,ticks=false,label=false,permute=(:x,:y),ylims=(0,1.2),flip=true,color=:green,background=false)
	plot!(n1,inset = (1, BB3), subplot = 4,axis=false,ticks=false,label=false,permute=(:x,:y),ylims=(0,1.2),flip=true,color=:pink,background=false)
...
julia_version = "1.8.0"
manifest_format = "2.0"
project_hash = "27c94c6c452e7672f5f8128115e75d6d1756fd58"
