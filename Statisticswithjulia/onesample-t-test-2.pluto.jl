...
# ╔═╡ 1eee7cde-1aac-11ed-319c-334d6624efc8
begin
	using  Distributions,DataFrames,Statistics,Plots,RCall,StatsPlots,StatsBase,LaTeXStrings,CSV,HTTP,Symbolics,HypothesisTests
end

# ╔═╡ 27a72edd-1e32-4efb-a2b0-831530bcc83e
md"""
 # 单样本 T检验

!!! qestion
    根据管道生产中对直径的要求(总体参数:$\mu_{0}:52.1$,$\sigma :1.2$),作为假设检验的依据, 使用单样本假设检验推断生成流程中的工艺,设备运转是否正常.

!!! solution

    依据单样本$T$检验对获取的样本直径均数
    
"""

# ╔═╡ 4e064574-a6c4-4cb1-80f8-882f35458f94
begin
	@variables X̄,μ₀,n,σ
	...
	data=retrivedata(url)[:,1]  # 数据在第一列
    μ₀,σ=52.2,1.2
    X̄, n = mean(data), length(data)
end

# ╔═╡ f6348106-7299-4010-8a36-88bbc044bdb4
describe(data)

# ╔═╡ 98e3394a-b8af-4ac7-b23a-24b43b0fd77e
boxplot([L"diameter"],data,label=false,size=(300,400,))

# ╔═╡ f526e1cd-9f15-40de-b313-d06d0c6223ad
test1 = OneSampleTTest(X̄, σ, n, μ₀)
...
manifest_format = "2.0"
project_hash = "67a7b5cc11be26458642740faac44c1789df1361"
