...
# ╔═╡ f1bbfcf0-2046-11ed-3080-01d0d84aca7a
begin
    ...
    TableOfContents(title="概率介绍")

	
end

# ╔═╡ 4a33d2f0-868d-4917-943c-2bd8c3fb58b7
md"""
# ch06  概率介绍
"""

# ╔═╡ b7532881-7624-468d-b6cd-ab86ce1d593a
md"""
我们用布袋中彩球开始概率介绍
...
!!! example
    ...
    袋中有红色, 蓝球各 $50$ 个.  每次摸出一个球, 记录颜色然后返回布袋, 混合后再重复试验. 进行 $1000$ 次, 记录红球和蓝球出现的次数
"""

# ╔═╡ 935ed70e-e9ac-4e38-a351-7055285ba932
md"""
当红球蓝球的比例相当的时候(50:50), 从布袋抽1000, 比例会接近于 1:1 .  对于这样一个总体, 不同颜色球的比例是一个参数, 随着抽样进行, 抽样结果比例会接近这个信息.
"""

# ╔═╡ 507a1c4f-86a8-4e3b-a011-5292d07709d7
...
!!! example
    ...
    袋中有红色$90$个. 蓝色球 $10$ 个 每次摸出一个球, 记录颜色然后返回布袋, 混合后再重复试验. 进行 $1000$ 次, 记录红球和蓝球出现的次数

"""

# ╔═╡ a76c68ec-0aa9-404d-a0d3-3e3ef0436484
md"""
当总体的信息发生改变, 红色球很蓝色球比例为$9:1$ 时, $1000$ 次抽样的频数也接近于这个比例. 
"""

# ╔═╡ 838117b6-1667-4dfc-b22c-e6b69a46ba21
md"""
### 统计推断

上面两个示例里, 总体的信息是已经知道的, 通过大量的采样, 获取的信息逼近了总体的比例信息. 

在现实世界中有很多总体的信息我们无从得知. 借助于大量的样本信息, 逆着上面实例的流程会得到样本代表的总体的近似信息. 

这就是统计推断的过程
"""

# ╔═╡ f83a4255-ec6c-441e-9e71-76c0c0cc43f7
begin
	red,blue='🔴','🔵'
	is_redball(x)=x==red
	is_blueball(x)=!is_redball(x)
	redball(sample)=sample.|>is_redball|>count
	blueball(sample)=sample.|>is_blueball|>count
end

# ╔═╡ b33a4179-c0de-439b-9221-3af9f05ac1e1
box1=[50, 50]|>x->[repeat([red],x[1]) ,repeat([blue],x[2])]|>x->vcat(x...)|>x->reshape(x, 10,10)

# ╔═╡ 0c7daa18-14a8-476d-a8fa-aebc3ef4386b
sample1=rand(box1,1000)  #1000次采样

# ╔═╡ 0eaa08ff-4cae-4bc2-bdf7-c026060f3d0c
ratio50vs50=["🔴"=>redball(sample1), "🔵"=>blueball(sample1)]

# ╔═╡ ada93b97-0521-484e-8eb3-7e12b0a9c7b5
bar([L"red",L"blue"],[redball(sample1),blueball(sample1)],color=[:red,:blue],label=false,size=(200,300),alpha=0.8)

# ╔═╡ 14938189-723e-4b89-ad1c-a825c9df9df2
box2=[90, 10]|> x->[repeat([red],x[1]) ,repeat([blue],x[2])]|> x->vcat(x...)|> x->reshape(x, 10,10)

# ╔═╡ b17af393-fa2b-4ea9-a054-0af7d56d8349
sample2=rand(box2,1000)

# ╔═╡ af14663f-4ced-44cb-940b-6e3107de3132
ratio90vs10=["🔴"=>redball(sample2), "🔵"=>blueball(sample2)]

# ╔═╡ 327e8ceb-1c37-4281-aed7-6c10fe43371f
bar([L"red",L"blue"],[redball(sample2),blueball(sample2)],color=[:red,:blue],label=false,size=(200,300),alpha=0.8)

# ╔═╡ 74ad2408-9afa-43ff-bba3-56159df4d838

begin
	coin_image = load(download("https://gss0.baidu.com/-4o3dSag_xI4khGko9WTAnF6hhy/zhidao/wh%3D600%2C800/sign=1d583dbe9182d158bbd751b7b03a35e0/b3119313b07eca80a96982b79c2397dda1448376.jpg"))
    head = coin_image[:, 1:end÷2]
    tail = coin_image[:, end÷2:end]
	coin=[head, tail]
end


# ╔═╡ 59048398-31cc-4d7a-b9cb-be7f74e82a90
md""""
### 概率的定义
当一个事件存在多种输出可能时, 概率就是某一类特定输出在全部输出中所占的比例.  

例如在一副去掉大王和小王的扑克牌中. 随机抽取一张花色为方块的是多种结果中的一种.  总的可能性为 52, 每张牌被抽到的机会都为$p= \frac{1}{52}$,  抽到方块(♦️)的机会等于所有方块的机会总和,就是p(♦️)$=\frac{13}{52}$,   抽到$A$ 的机会为$p(A)=\frac{4}{52}$, 因为一副牌中有四张$A$.  实际上概率就是理想状况下的比例. 
