...
# ╔═╡ f0141900-3162-11ed-388f-9b71518426cf
begin
	using DataFrames, StatsPlots,StatsBase,LaTeXStrings,RDatasets
	
end

# ╔═╡ e10df019-1718-44e8-8021-b55bc9f79a2a
begin
	range=1:7
	x1=[3, 4 ,2, 6, 8, 2 ,5]
...
# ╔═╡ c5416716-e7d3-472e-9baa-06e1e093df35
begin
	l = @layout [
    [a{0.1w} b] 
            [_ c{0.1h} ] 
    ]
	p1=scatter(repeat([0.1],7),df.x2,xlims=(0,0.2),label=false,size=(3,500),grid=false,xticks=false,xaxis=false,mc=:black,ylabel=L"x2",frame=false)
	p2=scatter(df.x1,df.x2,label=false,mc=:black,frame=:box)
	p3=scatter(df.x1,repeat([0.1],7),ylims=(0,0.2),label=false,size=(500,3),grid=false,yticks=false,yaxis=false,mc=:black,xlabel=L"x1",frame=false)
	
	plot!(p1,p2,p3,layout = l)
end

# ╔═╡ 43a777c4-fcc1-4de7-a7ea-7cc89f3e4bfb
begin
