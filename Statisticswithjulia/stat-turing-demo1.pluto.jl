...
# ╔═╡ d6c6dc58-2698-11ed-3beb-6d3ed1bad6e2
begin
	using Plots, StatsPlots, Distributions, LaTeXStrings,PlutoUI,Turing,Random
	TableOfContents(title="概率编程实例 1")
end

# ╔═╡ 29727404-84a8-4313-ae1c-f199f05fce02
md"""
!!! info

     我们知道一个事实就是不管筛子是有问题,还是没有问题, 6 个面出现的概率之和为 1, 这可以用迪雷特里分布来表示.  然后我们有一个由某分布生成的随机数据, 模型根据数据修改先验分布的参数. 

     这就是贝叶斯方法流程. 
"""

# ╔═╡ 2d8f00a0-aa7e-4d5a-8d60-4cb0fb480528
begin
...
# ╔═╡ c6c73514-4e73-4ece-89a0-99c4a850a728
begin
	Random.seed!(123);
...
# ╔═╡ 3a0b822a-e7d8-4e34-ab21-4e771f17458d
model = dice_throw(data);

# ╔═╡ 75461863-5ab7-4dfc-bbff-3c339cbcae3e
chain = sample(model, NUTS(), 2_000);

# ╔═╡ 3e1ddb2c-38c0-48cd-ac28-8a93e420bfdc
summaries, quantiles = describe(chain);

# ╔═╡ 84670683-d964-4363-9ce3-ce923b183ecb
summaries

# ╔═╡ 5c3ee6d5-b786-43df-a9ba-e1b6ac2ee290
sum(summaries[:, :mean])

# ╔═╡ efffc1d8-d02e-4d87-9581-78fe9366a95e
[idx * i for (i, idx) in enumerate(summaries[:, :mean])]|>sum

# ╔═╡ ce7111d1-9ae0-4bc5-aca8-1d24e0e383cd
plot(chain)

# ╔═╡ 9329e0be-46a3-466c-9f65-8631ef10fe82
...
manifest_format = "2.0"
project_hash = "ef9af7cdb1433c738a673719270c080429303b7e"
