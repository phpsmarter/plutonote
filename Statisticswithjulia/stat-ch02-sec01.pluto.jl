...
# ╔═╡ 90085df0-1e13-11ed-031a-cfec19ad3c66
begin
...
# ╔═╡ c2b0d1b4-f101-437e-83c2-5d73d6661938
md"""
#  ch02 sec2.1
...
# ╔═╡ 244e9fe6-2724-4969-a1fd-c93faea1f796
md"""
!!! example
    example 1 
    学生考试成绩
...
# ╔═╡ f007b5db-08cd-4226-942f-3769726cd621
begin
...
# ╔═╡ 8088faa0-12d3-42d1-8912-12ab8aa6432c
mode(score_df[:,2])   # 众数是考试乘积里出现最多的分数

# ╔═╡ 24162729-8d83-4c1f-bed6-6b7774fe7f35
median(score_df[:,2])  #中位数也是 100

# ╔═╡ c836a5f7-ec89-470e-ba9f-14472c5cf77f
mean(score_df[:,2])   #考试分数均数

# ╔═╡ 313503c6-ca7e-44f1-829f-471699393069
boxplot(score_df[:,2], label=false,size=(300, 300))

# ╔═╡ ffef3004-04b6-403b-b7df-626b5d6e5116
md"""
!!! example
    example 2
    假设我们想要了解五年级学生对于学习的态度, 给定一个量表, 从不重要-> 重要设置为 1-5 分的等级
    随机选取了 30 个学生进行调查.
...
# ╔═╡ fb8ed74f-0d95-406d-af93-33d6cecb3652
begin
...
其中符号表示含义: $\bar{X}$ : 样本均值, $\mu$ : 总体均值, $\sum$ :求和符号,  $X$: 单个对象的取值, $n$:一次采样中获取样本中对象的个数, $N$ 总体中单个对象的个数. 
...
但是总是可以在采样中获取的一个样本中$n$个对象的值.  因此样本均值$\mu$ 总是可以计算出.
...
# ╔═╡ bbd0514c-080a-4a69-93f7-ee44603554c4
md"""
...
# ╔═╡ 4163b79d-c6d3-4de0-bfcb-e8e0f8c5e09b
md"结果如下:"

# ╔═╡ ce5b8ea6-4a84-4bb7-a4df-71334da15796
begin
	data2=[6,2,4,2]   # 数据
    substract2(x)=x-2  # 减法
	square(x)=x^2     #平方
	a=data2|>sum     #求和
	b=data2.|>square|>sum  # 对数组元素分别平方, 然后求和, 针对每个元素的操作用"." 操作符
	c=data2|>sum|>square   # 求和然后平方
    d=data2.|>substract2|>sum  # 每个元素-2, 然后求和
	e=data2.|>substract2.|>square|>sum  #每个元素-2,每个元素平方, 然后求和
...
# ╔═╡ 97cf613c-619d-4015-8a7c-9e0796f503fe
key

# ╔═╡ d847f1f6-3fb3-46c9-901f-12a36a3d4071
md"""
实例3 表达式方法描述如下:
"""

# ╔═╡ 7365e1cd-4147-49dc-b415-a769dd7ea17e
description
