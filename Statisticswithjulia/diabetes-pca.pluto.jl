...
# ╔═╡ b18c0de8-2913-11ed-39c5-23c14a66499c
begin
...
# ╔═╡ ce647f54-23f1-4a26-9649-32c0ea1733f9
data=CSV.read(diabetes_url2,DataFrame)

# ╔═╡ c013f74a-de55-4666-92a4-7921e25ee3c2
names(data)|>length

# ╔═╡ 2c4896d8-7941-43e5-8100-317bf6a07a96
begin
	 
	Xtrain = Matrix(data[1:2:end,3:32])'
    Xtrain_labels = Vector(data[1:2:end,35])
	# split other half to testing set
	Xtest = Matrix(data[2:2:end,3:32])'
	Xtest_labels = Vector(data[2:2:end,35])
end

# ╔═╡ a1fca8f7-5437-4565-aa27-ca37e5c00a2c
M = fit(PCA, Xtrain; maxoutdim=8)

# ╔═╡ 463e3629-88ce-4997-bd58-3921ecca673d
Mds = fit(MDS, Xtrain; maxoutdim=3, distances=false)

# ╔═╡ d9ce0ef7-243b-43b5-805e-35cc19781068
Y = predict(Mds)

# ╔═╡ 6f11c12e-cad8-4a27-aa08-ab2173932d79
begin
has = Y[:,Xtrain_labels.==1]
none= Y[:,Xtrain_labels.==0]


scatter(has[1,:],has[2,:],has[3,:],marker=:circle,linewidth=0)
scatter!(none[1,:],none[2,:],none[3,:],marker=:circle,linewidth=0)

end

# ╔═╡ 3a9ddeb7-044f-4a56-b5f3-f9f17fd8c77a
md"""
压缩到三个维度, 只解释了 50% 的变异, 所以效果不太好
"""
