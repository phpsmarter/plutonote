...
# ╔═╡ 77c04e56-361e-11ed-0caf-6d5abc640607
begin
	using StatsPlots,StatsBase,LaTeXStrings,Distributions,Latexify
end

# ╔═╡ f3899422-2da3-4484-a9a7-f2e12a8f221e
md"""
# 总体随机抽样实验介绍
"""

# ╔═╡ f0bc453b-45ea-4851-ac4d-9a054f14138b
md"""
!!! info
...
# ╔═╡ 91b1d290-a020-421d-9f65-91473cb071d1
md"""
##  1.构建总体

总体是客观存在,统计学没有对总体的形式做任何限制. 比如我们可以$99$乘法表作为一个总体
...
# ╔═╡ cc59308c-8a70-45a7-80c1-617b6531400f
p=population=[x*y for x in 1:9 for  y in 1:9]|>Vector

# ╔═╡ 45e783e2-3552-48e7-8786-65b5ec438d2e
md"""
### 下面是对$99$乘法表的描述统计. 
...
# ╔═╡ 3d4a3e40-9c3d-479c-ac8b-73b8ff71de31
population|>summarystats

# ╔═╡ 93022ebe-6d9a-4eac-a7ca-b682f7b1206f
Dict("mean"=>mean(p),"extreme"=>p|>d->maximum(d)-minimum(d),"std"=>p|>d->std(d))

# ╔═╡ ffbd9c56-f7e5-4ca1-859c-5ed8dd78d085
md"""
在统计学中经常会谈到一个术语叫自由度(degree of freedom,$df$).

 $99$乘法表的自由度$(df)$是多少?  

  $99$乘法表的自由度$(df)$为$0$, 因为一旦这个总体构建完成, 任何的度量值都不会再发生变化. 
...
$x+y=5$
  ...
  的自由度为$1$, 因为给定一个$x$值, $y$的值就确定了, 因为是等式,$x+y$要满足等式条件.$y$的变化是受$x$变化控制的.  
...
# ╔═╡ d007206d-0d54-4d67-baba-d325491663eb
md"""
平均值为什么是 $25$? 
...
# ╔═╡ db9f8d89-e005-4965-8071-6958e26df459
md"""
下面我们还是返回到$99$乘法表,开始抽样实验流程
"""

# ╔═╡ face83cc-2572-49c0-b132-3686e5fd9246
md"""
## 2.随机抽样
...
以$99$乘法表的数字为例, 我们只关注序号, 所以可以以二维表的形式排列,  这就是99乘法表,可以表述如下$(1,1,1)$,$(2,4,8)$, 前两个数字表示序号, 最后一个数字表示值. 

从$99$乘法表中抽取一个数字是均一分布, 因为每个数字被抽到的机会均等,均为 $\frac{1}{81}$
"""

# ╔═╡ c40b1b07-1771-4f7c-aa10-3efcaa532b96
md"""
以二维表形式呈现:, 接着就可以根据行列坐标进行抽样
"""

# ╔═╡ 9ea5ca59-a7ff-4e6d-b699-95413eb6dd3d
md"""
在上面的图中, 我们以二维表格的形式展示乘法表,  每次点击会随机从表中抽取 5 个值. 

因为这$5$个值来源于乘法表, 那么必然具有乘法表总体的某些相似的性质.  
...
# ╔═╡ a6c7e733-1526-40fa-ac4d-d26ff96d7cf3
observations= [sample(p,5) for i in 1:500] #随机采样使用的放回式采样, 取出一个数字,读取值, 然后再放回取, 进行下一次抽取, 所以会出现重复值

# ╔═╡ f104c9e4-285e-4094-a663-583cf56b9052
mean_observations=observations.|>d->mean(d)

# ╔═╡ 91bc8528-7ef2-4ffd-ad41-c4d607ef09a3
md"""
似乎每次取出的$5$ 个数字的平均值差异很大, 下面绘制出均值的残差图,  就是 500 次抽样的均值分别减去总体的均值, 然后绘制出茎叶图
"""

# ╔═╡ 7bc5afa8-2026-4930-822c-a4d62d00603a
begin
...
# ╔═╡ 9944f977-d5a6-4236-b396-0c64a900187a
md"""
### 2.2 随机采样2-加大随机采样的个数到 20 个

因为总体有$81$个, 所以每次抽样取出 $5$ 个数字,可能提供的信息过少, 我们把每次抽取的数字增加到 20 个,看看是什么情况
"""

# ╔═╡ cfd4657d-de05-4fa2-b8ca-0984bb3aa66f
md"""
实际从上图中可以直观的看到, 当加大抽取的数字时, 每次抽取的数字覆盖的面积总和比$5$个数字时的面积要大. 抽取的个体越多, 提供的信息也就越多. 
"""

# ╔═╡ 2ef35560-c231-4eac-9f4e-87009fd1bf9e
md"注意y 轴的值, 已经幅度已经比5 个数字时小很多"

# ╔═╡ 92f38d4e-a1b9-4c9e-ab0e-36cf0b4cdfed
md"""
### 2.3 每次取出不同数量数字时的残差
...
# ╔═╡ 31dab1ee-8b42-45ca-b971-9f97e2f1313b
[sample(p,60) for i in 1:500].|>(d->mean(d))|>histogram

# ╔═╡ 0ba725cc-c60e-4e6a-9c9a-e571b73773d3
md"""
##  结论

随机抽样实验试图通过总体中少量的个体的信息来代表整个总体的信息. 当我们从一个总体中抽取的个体数达到一定的数量(越多越好, 一般取到$n=30$). 我们并不是要完全捕获总体的所有信息, 有的总体容量是无限大的, 根本不可能捕获所有的信息. 


抽样的时候抽取的个体集合的信息不能完全和总体相符, 这里的信息偏差是抽样统计需要考虑的问题. 
...
# ╔═╡ 71841689-ac93-4678-b47c-c3fd328cf677
function nineninetable()
	span=1:9
	offset=0.5
	function plot_num(x,y)
		num=x*y
		(x+offset,y+offset,text(L"%$(num)",pointsize=12, halign=:center,valign=:center))
    end
...
# ╔═╡ 72faf0cf-d76e-4285-a633-f7207923b51e
nineninetable()

# ╔═╡ 27c8e817-bd57-4530-b98d-6b76cfffc712
function stem(res::Array,color=:grey)
	#residual=observations.-mean(p)
    n=length(res)
    range=1:n
    plot(repeat((1:n)', 2),
         [zeros(1, n); res'], label = "", color = :grey, alpha = 0.5,size=(800,600),frame=[:zerolines, :box])
    plot!(1:n, res, color = :grey, markershape = :circle,
          alpha = 0.5, label = "", linewidth = 0)
end

# ╔═╡ 0a2bd194-ae81-4657-b9c6-a18ef4da0616
(mean_observations.-mean(p))|>stem
    

# ╔═╡ 76d683d8-95d2-4f77-84b6-f696e4aa3831
[sample(p,20) for i in 1:500].|>(d->mean(d)).|>(d->d-mean(p))|>stem

# ╔═╡ 84c96b56-4347-4941-a501-323ffabbb6fe
  begin
	  res_plot(p;n=20)=[sample(p,n) for i in 1:500].|>(d->mean(d)).|>(d->d-mean(p))|>stem
...
# ╔═╡ 91f2baa3-254e-4163-96e3-0f252f32a6f2
d=Uniform(0,8)

# ╔═╡ 515ae1f3-9180-4182-961f-a9e791051300
rand_sample(;range=0:8,n=5)=sample(range,n,replace=false)

# ╔═╡ 113ae999-84ef-4aa6-a283-3c0e71d8607e
function plot_sample(;n=5)
	sample_num=n
	sample_span=0:1:9
	span=1:10
	offset=0.5
	xs,ys=rand_sample(;dist=d,n=sample_num).+offset,rand_sample(;dist=d,n=sample_num).+offset
	println(xs.+offset)
	println(ys.+offset)
...
# ╔═╡ 26e1e33d-c426-42ed-8050-89818edf22e4
function plot_sample2(;n=5)
	...
	xs,ys=rand_sample(;n=sample_num).+offset,rand_sample(;n=sample_num).+offset
	plot(size=(300,300),lims=(1,10),frame=:box,bg = :linen,ann=ann,axis=false,ticks=false)
	hline!(span,label=false,lc=:black)
	p1=vline!(span,label=false,lc=:black)
	scatter!(xs,ys,label=false,marker=:square,ms=12,mc=:red,ma=0.3)
end

# ╔═╡ 8983d24e-2cdf-4291-ba98-56ff1577b6cc
plot_sample2(;n=5)# 点击右边运行符号

# ╔═╡ 8b5b8c59-44f2-4979-a031-b940ac71745c
plot_sample2(;n=20)# 点击右边运行符号

# ╔═╡ d80001da-f82c-4601-bb58-c7e3f20589bb
rand_sample()
...
julia_version = "1.8.0"
manifest_format = "2.0"
project_hash = "cc4a6d2435461d7a78e2b8e29a38e5ba056ab4da"
