...
# ╔═╡ e7739458-2a76-11ed-1234-0fb68b9be4f6
begin
	using StatsPlots,DataFrames,StatsBase,MultivariateStats,LinearAlgebra, RDatasets
end

# ╔═╡ e2129674-293b-4edb-8dde-6019da10c67f
begin
	cars = dataset("datasets", "mtcars")
	df=select(cars,[:MPG,:Disp, :HP, :DRat])
	X=df[!,2:4]|>Matrix
	y=cars[!,:MPG].|>d->convert(Float64,d)

	
	
end

# ╔═╡ 50c69bf1-5955-41bf-a45b-ce7223298f61
#@df df corrplot(cols(1:4), grid = false)

# ╔═╡ 88ccf610-2f13-4549-b99e-a0b5281ab760
begin
p1=scatter(df[!,:2],df[!,:1],grid=false,label=false)
p2=scatter(df[!,:3],df[!,:1],grid=false,label=false)
p3=scatter(df[!,:4],df[!,:1],grid=false,label=false)
plot(p1,p2,p3,label=false,layout=(1,3))
end

# ╔═╡ fac76cee-0455-45a6-b73b-e028c4d8b330
md"""
## least square  regerssion
"""

# ╔═╡ bb99cfad-b503-42f8-a9ce-8e259c7883d1
lmodel=llsq(X, y,bias=true)

# ╔═╡ f6f20f5c-155e-417e-a84f-d43859a47d65
md"""
## ridge regression
"""

# ╔═╡ f22603aa-4c17-40c0-aa1d-7ee4cc13cd50
ridgemodel=ridge(X, y,0.5,bias=true)

# ╔═╡ 251afea7-1227-40cf-940d-9ef2e632e173
A,b=lmodel[1:end-1], lmodel[end]

# ╔═╡ 5e231e38-adf0-404a-9769-344764530be9
yp = X * A .+ b'
...
manifest_format = "2.0"
project_hash = "bed126a9c7abc0587b3c995dd78b6cf9a98419f9"
