...
# ╔═╡ 8cc6c3ba-277c-11ed-03e0-7b4516d70ec3
begin
	using Combinatorics,Latexify, LaTeXStrings,Symbolics,Shuffle,Distributions,QuadGK
end

# ╔═╡ 52fc67bb-004b-4ec0-8943-197e85b45e21
begin
	first3(d)=d[1]==3
	is8(d)=d[1]+d[2]==8
	sample_space=[[a,b] for a in 1:7 , b in 1:7]
	B=sample_space|>(d->filter(first3,d))|>d->filter(is8,d)
end

# ╔═╡ bbeea85c-211e-4328-a3e3-6a57c1d4ac6f
begin
   first_dice_3=sample_space|>d->filter(first3,d)
  sum_8=first_dice_3|>d->filter(is8,d)
  ["第一个色子为 3的机会"=>first_dice_3,"合计为 8 点的机会"=>sum_8]
end

# ╔═╡ d27954a6-172c-4070-8ab4-972e64b1f2d0
begin
	dice1,dice2=1:6,1:6
	Dice(x,y)=1//36
    res= Dict(["1,$y"=>Dice(5,y) for y in dice2 ]) # 第一个色子固定为 5 的实验
	res|>d->latexify(d,env=:mdtable)
	res|>values|>sum|>latexify   

end

# ╔═╡ cf8b6877-1b84-4d5e-8a88-ea0167f1dcf7
let
	
	
    dice1,dice2=1:6,1:6
	function  add_to(dig)
		   return d->d[1]+d[2]==dig
	end
	isseven=add_to(7)
    #[(x,y)  for x in dice1, y in dice2 ]|>d->filter(isseven,d)
  #    res2=[]
	 # for x in dice1
		#  for y in dice2
		#      if isseven([x,y])
	 ...
	 # end
	 # res2
     sample_space=[(x,y)  for x in dice1, y in dice2 ]
	 filter_event=sample_space|>d->filter(isseven,d)
	function  P(event::Int)
		isevent=add_to(event)
		sample_size=sample_space|>length
		event_size=sample_space|>d->filter(isevent,d)|>length
        if   event_size !=0
			 return event_size//sample_size|>d->round(d*100,digits=2)|>d->L"%$d \%"
		else
			return event_size
		end
	...
	[P(event) for event in 1:12]
	
end

# ╔═╡ 0a9caddf-f8a2-42bd-952d-83dbf8f5b372
let
	dice1,dice2=['❶','❷','❸','❹','❺','❻'],['❶','❷','❸','❹','❺','❻']
	isseven(d)=d[1]+d[2]==7
     res3=[]
	 for x in eachindex(dice1)
		 for y in eachindex(dice2)
		     if isseven([x,y])
			    push!(res3,(dice1[x],dice2[y]))
			 end
		 end
	 end
	 res3.|>latexify
...
# ╔═╡ 3ccfd0dd-d311-43d9-9822-17996869fa7a
let
	
	
    dice1,dice2,dice3=repeat([1:6],3)
	getdice(d::Int=1)=repeat([1:6],d)
	dice=getdice(3)
	"""
	 add_to
	 function add_to(dig)
    ...
    sample1=[(x,y)  for x in dice1, y in dice2 ]
	sample2=[(d₁,d₂,d₂)  for d₁ in dice1, d₂ in dice2,d₃ in dice3 ]
	
	function  P(sample,eventfun,event::Int)
		event=eventfun(event)
		sample_size=sample|>length
		event_size=sample_space|>d->filter(event,d)|>length
        if   event_size !=0
			 return event_size//sample_size|>latexify
		else
			return event_size|>latexify
		end
...
manifest_format = "2.0"
project_hash = "33a95253bec736610362466209cbb7f0c99df151"
