...
# ╔═╡ dd3baa78-1fa1-11ed-1273-df3e4de87bf9
begin
	using Latexify,PlutoUI,RDatasets,DataFrames,Random,FileIO,HTTP,Images,Plots, StatsPlots,Statistics,StatsBase,LaTeXStrings,Symbolics,CSV
    TableOfContents(title="Z 分数和标准分布")
end

# ╔═╡ f8c138c5-4627-4de8-bb0d-6f07052c6809
md"""
Z 分数计算公式:
...
# ╔═╡ c910dac3-82a9-444e-bee0-6343926ffa69
md"""
...
# ╔═╡ 1818a732-4f52-4d0a-9b13-7768884b89d4
sampla_data1=[78 73 72 66 63 71 67 42 62 73 45 22 62 99 73 91 52 37 55 97 91 51 44 23 46 64 97 62 97 31 21 49 93 91 89 46 73 82 55 98 12 56 73 82 37 55 89 83 73 27 83 82 73 46 97 62 57 96 46 55 46 19 13 67 73 26 58 64 32 73 23 94 66 55 91 73 67 73 82 55 64 62 46 39 87 11 99 73 56 73 63 73 91 82 63 33 16 88 19 42 62 91 12 82 32 92 73 46 68 19 11 64]

# ╔═╡ fe6f9d46-8480-4fe2-98a8-20d6c7c243a0
begin
...
# ╔═╡ 02d91392-a71a-4d2d-a8c1-579596eafadd
get_zscore= z_score(sampla_data1)

# ╔═╡ beafb777-7aaa-4516-99a1-195236e20875
get_zscore(87)

# ╔═╡ 413cfd8d-aceb-424c-8b36-6aa33b6c16bd
quad=quantile(sampla_data1[1,:],[0,0.25,0.5, 0.75,1.0]) #求sampla_data1的四分位数

# ╔═╡ f1054f11-5d43-4cfe-a10a-a160b05a0e6d
get_zscore.(quad)    #四分位数的 z 标准分

# ╔═╡ 0290b10a-b635-4735-9625-a0c6db2abbd0
md"""
如果是标准的正态分布, 集合的四分位数转换的$z$ 分数的绝对值会呈现对称. 中位数的 z 分数会是 0 . 
"""

# ╔═╡ 6a12c604-3c5e-4282-a4a9-6131e247bd5f
md"""
!!! example

    example 2 

    使用NBA 统计数据 计算 斯蒂夫库里 和姚明的z分数

    NBA 身高数据
 
	https://raw.githubusercontent.com/kuriousrajib/SportsAnalytics/main/player_data_v1.csv


 注: 由于姚明的数据里 大学有缺失, 可以补充一个大学名称,否则使用 dropmissing函数之后会查不到数据
     具体方法: 编辑器打开 csv, 查找 yao ming  然后添加一个大学名字

"""

# ╔═╡ 97e811f1-9067-465c-bc29-43706d50df4d
begin
	# preparing processing 
	local_server_url="http://127.0.0.1:8080/nba-player-data.csv"   
	#remote_server_url="https://raw.githubusercontent.com/kuriousrajib/Sports-Analytics/main/player_data_v1.csv"
...
# ╔═╡ a6205f6e-1cb2-46aa-b6ac-01890de4ecd5
md"""
 # ch04 Z标准分和标准分布

!!! info
   
    在 NBA 比赛里,解说员总是或说某位球员是矮个子球员. 比如很多人喜欢的斯蒂夫.库里

    

    或许很多人会觉得这很荒谬.  对于统计学和一个故事一样, 需要背景的上下文材料, 统计信息才算完整. 
    对于解说的这个看法, 的确是有一定理由. 这里我们借用$NBA$ 的身高资料来看看解说员所说的是否有依据. 
    
     .   | .
    :------- | :---------------: |
         | ![](https://tva1.sinaimg.cn/orj180/e6c9d24egy1h5cgyc2ma2j20d608smx7.jpg) | 
     

 $(curry_data)

 对于普通人身高, 是比较高的. 但是在 NBA的数据集可能是另外一种情况.  同一个统计数字出现在不同的上下文 
 里意义不同.  数据的核心是不是数值, 而是位置(排名)关系.  
...
在使用总体计算 z 分数, 位置信息是参数, 一旦总体确定, z分数就不会改变. 
...
# ╔═╡ 5adb58e0-5863-424a-a56d-a37bbc84ae6d
md"""
!!! info
    NBA身高数据有三个问题要注意
    1. 时间跨度
...
![](https://tva1.sinaimg.cn/mw690/e6c9d24egy1h5d1ng34zbj20d509v0t9.jpg)

$(top10)

"""

# ╔═╡ 1adcfb3c-5268-4028-bee0-f74b33f1411d
get_player_zscore= z_score(later2000_player[:,:hc])

# ╔═╡ 553ae9c7-a422-4e53-955c-02446df98a35
curry_zscore=curry_data|>getheight|>get_player_zscore  #库里z分数

# ╔═╡ 065b8196-df00-4faa-b002-53f36f272af9
yaoming_zscore=yaoming_data|>getheight|>get_player_zscore #姚明 z分数

# ╔═╡ c2df0eb1-4777-4209-841e-cf725a0daff2
md"""
...
# ╔═╡ 15833d21-24a3-4fab-aaf7-6706c2fbdf54
md"""
### z 分数转换为实际测量值
以总体 z分数公式为例:
...
# ╔═╡ 25c262a7-fdeb-4cc8-a3ca-76c2ecac3881
length(later2000_player[:,:hc])

# ╔═╡ 71d96aff-94a0-4bd8-9815-419f0e78b2ae
begin
...
manifest_format = "2.0"
project_hash = "f0d882c55ffb1f8a748177367b9f6b9d0c1e926a"
