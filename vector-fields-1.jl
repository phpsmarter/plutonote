...
# ╔═╡ bebe368b-bc38-4136-8781-5f1a46ac380d
begin
	using CairoMakie,GLMakie
    GLMakie.activate!()
end

# ╔═╡ d7371bdc-c39f-11ec-38e7-3f2208f703dc
md"# Arrow"

# ╔═╡ e5df9482-840a-4784-992a-1c74f7afb459
let
	# 𝑓(𝑥,𝑦)=𝑥2+2𝑦2
    xs = LinRange(-3, 3, 20)
    ys = LinRange(-3, 3, 20)
    us = [2x  for x in xs, y in ys]  # 梯度要求导数
...
# ╔═╡ 2cbc71b3-a8b5-47d8-8c5a-839b55e4eaee
let
	# 𝑓(𝑥,𝑦)=𝑥2+2𝑦2
    xs = LinRange(0.0001, 0.00011, 2)
    ys = LinRange(-3, 3, 20)
    us = [2x  for x in xs, y in ys]  # 梯度要求导数
    vs = [4y  for x in xs, y in ys]
    strength = vec(sqrt.(us .^2 .+ vs .^2))
...
# ╔═╡ b35da0c4-5e00-4ff7-947f-34288e28ee16
let
	# 𝑓(𝑥,𝑦)=𝑥2+2𝑦2
    xs = LinRange(-3, 3, 20)
    ys = LinRange(-3, 3, 20)
...
# ╔═╡ a788e6bc-744c-4117-ae60-b911ae337624
let
	# 𝑓(𝑥,𝑦)=𝑥2+2𝑦2
    xs = LinRange(-3, 3, 20)
    ys = LinRange(-3, 3, 20)
    us = [2x  for x in xs, y in ys]  # 梯度要求导数
    vs = [0  for x in xs, y in ys]
    strength = vec(sqrt.(us .^2 .+ vs .^2))
    cmap = :gnuplot
    fig = Figure(resolution = (600, 400))
    ax = Axis(fig[1,1], xlabel = "x", ylabel = "y", aspect = DataAspect())
...
# ╔═╡ 69480ab3-33bc-4f6b-93e8-be89cc7413da
let
	# 𝑔(𝑥,𝑦)=5−𝑥2−2𝑦2
    xs = LinRange(-3, 3, 20)
    ys = LinRange(-3, 3, 20)
...
# ╔═╡ 40abab7d-3814-4ea6-a84e-fe2111691f46
let
	# f(x,y)=5 + 2𝑥 − 𝑦
    xs = LinRange(-3, 3, 21)
    ys = LinRange(-3, 3, 21)
	f1(x,y)=2
	f2(x,y)=-1
    us = [2  for x in xs, y in ys]  # 梯度要求导数
    vs = [-1  for x in xs, y in ys]
    strength = vec(sqrt.(us .^2 .+ vs .^2))
    cmap = :gnuplot
    fig = Figure(resolution = (600, 400))
    ax = Axis(fig[1,1], xlabel = "x", ylabel = "y", aspect = DataAspect())
    arrows!(ax, xs, ys, us, vs, arrowsize = 10, lengthscale = 0.1,
        arrowcolor =:red, linecolor = :red, colormap = cmap)
    limits!(ax, -3,3,-3,3)
    colsize!(fig.layout, 1, Aspect(1, 1.0))
    # display(fig)
	fig
