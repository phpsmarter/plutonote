...
# ╔═╡ cbb3b151-cf1f-4fff-9d43-fb62b722b900
begin
	using PlutoUI,Plots,JuMP,HiGHS
	gr()
	theme(:bright)
...
# ╔═╡ fb414424-daf5-11ec-2a68-4b5ba57c5328
PlutoUI.Resource("https://tva1.sinaimg.cn/thumbnail/e6c9d24egy1h2alsw1tzxj20m80gomxn.jpg")

# ╔═╡ 5f20cd9e-f9de-4538-99e2-444a56809449
md""" # JuMP  package  demo 1 

     文档里的第一个示例
"""

# ╔═╡ 327171c6-ed7e-44e4-9221-a0acb2505ee1
let
...
# ╔═╡ 7d8750c6-7d8e-4e30-919a-58e0ea149fa4
let
	#最优化导论 p69 示例
	model = Model(HiGHS.Optimizer)
	@variable(model, x>=0 )
	#@variable(model, 0 <= y <= 3)
	@objective(model, Min, (x+1)^2+x)
	#@constraint(model, c1, 6x + 8y >= 100)
	#@constraint(model, c2, 7x + 12y >= 120)
	print(model)
	optimize!(model)
...
# ╔═╡ c4655323-1962-4f17-b326-92130a202deb
let
	xs=-4:0.2:4
	ys=[(x+1)^2+x for x in xs]
	plot(xs,ys,label=false,frame=:origin)
end

# ╔═╡ 663fb5b3-8c73-4fb0-8807-1b4216fa5de4
let
	#最优化导论 p72 示例 
	xs=0:0.2:5
	ys=0:0.2:5
	zs=[x^2+0.5*y^2+3*y+4.5 for x in xs, y in ys]
