...
# ╔═╡ 70e9fd68-14a5-11ed-1ffa-4d8520d5e805
begin
	using Symbolics, Unitful, Latexify
	Latexify.set_default(; starred=true)
end

# ╔═╡ 1ad9bc7c-4064-47fc-b40a-48cc08990205
let
	@variables  a, b
...
# ╔═╡ c7e76942-c08d-4852-81b8-080744705c19
let
	@variables x, y, z,a, ϕ,θ,i⃗,j⃗,k⃗,p1,p2, r⃗,i,j,k
	D1=Differential(ϕ)
	D2=Differential(θ)
    x=a*sin(ϕ)*cos(θ)
	y=a*sin(ϕ)*sin(θ)
	...
	p1=expand_derivatives(D1(r⃗))
	p2=expand_derivatives(D2(r⃗))

	simplify(expand(p1*p2))
	
end

# ╔═╡ 23bf1f4d-fd65-4343-9e5f-7d8103f3b161
let
	@variables C,T,y,t,f(t)
...
# ╔═╡ c9fd725d-1d0c-40d7-b1bb-661dd6671e29
begin
	@variables r,f(r),A

	expr1=latexify(A~f~π*r^2)
end

# ╔═╡ 33586441-d664-41b4-a181-ee6287534727
let
	ind="h"
    dep="((3+h)^2-9)/h"
	f(h)=((3+h)^2-9)/h
	r1=[-0.1,-0.001,0.001,0.01,0.1]
	r2=[f(h) for h in r1]
	latexify([hcat(ind,r1');hcat(dep,r2')], env=:mdtable)
end

# ╔═╡ de425aff-efca-4894-9aaf-e1a34f799134
let
   @variables x,h
    f(x)=x^3
     diffq=(f(x+h)-f(x))/h
	 [latexify(f(x)),latexify("=>"),expand(diffq),latexify("=>"), simplify(diffq)]
	
end

# ╔═╡ 89276aab-7091-41a8-9797-260e5609e364
let
	speed=6u"m/minute"
    time=10

	dist(t)=speed*(t*u"minute")
    tspan=1:10
	ts=[t*u"minute" for t in tspan]
	dis=[dist(t) for t in tspan]
	latexify([ts';dis'], env=:mdtable)
end

# ╔═╡ c6831114-0630-4318-ac20-0c90edfba678
let
	@variables t, x(t),y(t)
    D=Differential(t)
	x~5cos(π*t)
	y~5sin(π*t)
	dx=expand_derivatives(D(5cos(π*t)))
	dy=expand_derivatives(D(5sin(π*t)))
	speed=expand(sqrt(dx^2+dy^2))
end

# ╔═╡ 21138700-a733-4138-b9aa-d62bfb7434e0
begin
	lstr = @latexrun f1(x; y=2) = x/y
end

# ╔═╡ 141fa5e3-670c-4e6e-9158-331823dce18a
begin
	m=["x=4" "=>" f1(4)]
	latexify(m)
...
manifest_format = "2.0"
project_hash = "b0e02c4401e03cdfda26c4cc387660a53327a0a6"
