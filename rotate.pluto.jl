...
# ╔═╡ 514c2890-623b-4b05-a1e6-8a9573741496
begin
	using PlutoUI,Plots,Rotations
...
# ╔═╡ 6d1ddf66-d8ae-11ec-039f-8b7269a4cf89
PlutoUI.Resource("https://tva1.sinaimg.cn/thumbnail/e6c9d24egy1h2alsw1tzxj20m80gomxn.jpg")

# ╔═╡ 2148958f-d28a-4a17-a0f0-303ae1e7a3c2
let
	rot=0.5*π
	matrix=m = [cos(rot) -sin(rot);sin(rot) cos(rot)]
	rotmatrix=RotMatrix{2}(m)
	vec=[0 0;
		1 1]
	rotvec=vec*rotmatrix
	plot(vec[:,1],vec[:,2],ratio=:equal,size(320,320),fs=:origin,label="origin",arrow=0.5)
	plot!(rotvec[:,1],rotvec[:,2],ratio=:equal,size(320,320),fs=:origin,label="rotated",arrow=0.5)
