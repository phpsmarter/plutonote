...
# ╔═╡ bc9041de-0784-4542-b770-c3e0df68a04c
begin
	using GLMakie
end

# ╔═╡ 55429836-d535-11ec-0975-57ba934ea296
md"# m1 cairomakie demo"

# ╔═╡ 9dd33861-7369-450d-bbb8-4285c89cc3e1
let
...
# ╔═╡ d288b432-0ce1-49a5-9c4e-f2a5b4ed0dff
let
	tspan=0:0.02:2*pi
	origin=[0 0]
	figure = Figure(resolution = (400, 400))
	
	ax=Axis(figure[1,1])
	radius=1
	theta=rot=[1/6*pi]
	theta=rot[1]
	angle_span=0:0.02:rot[1]
	x(t)=radius*cos(t)
	y(t)=radius*sin(t)
	arcx(t)=x(t)*0.4
	arcy(t)=y(t)*0.4
	...
	arcxs=[arcx(t) for t in angle_span]
	arcxy=[arcy(t) for t in angle_span]
	arcpoint1=[x(theta) y(theta)]
	arcpoint2=[x(0) y(0)]
	
	lines!(ax,xs,ys)
	scatter!(ax,[0],[0],color=:red)
	arrows!(ax,origin[:,1],origin[:,2],arcpoint2[:,1],arcpoint2[:,2],color=(:red,0.6))
	arrows!(ax,origin[:,1],origin[:,2],arcpoint1[:,1],arcpoint1[:,2],arrowsize=6)
	lines!(ax,arcxs,arcxy)
    figure
end
