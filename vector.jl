...
# ╔═╡ daccff54-bccc-49ed-b9d6-741124736671
begin
	#GR 支持箭头符号
...
# ╔═╡ f9901416-bd1d-11ec-0e65-7582f104b532
md"# Vector "

# ╔═╡ 7dfcf314-ef8a-406d-8840-b616654f9574
begin
	x_vals = [0 0 0 ; 2 -3 -4]
	...
	plot(x_vals, y_vals, arrow = true, color = :blue,
     ...
     framestyle = :origin,lw=3)

end

# ╔═╡ 31f17fa7-9f42-4069-9f95-2a81619b5db4
let

	x_vals = [0 0 0 ; 1 0 0]
	y_vals = [0 0 0 ; 0 1 0]
	z_vals = [0 0 0 ; 0 0 1]
	plot(x_vals, y_vals,z_vals, arrow = true, color = :blue,
     	legend = :none, xlims = (-1, 1), ylims = (-1, 1), zlims = (0, 2),
     	
     xticks = -2:1:2, yticks = -2:1:2,zticks = 0:1:2,
     framestyle = :origin,lw=3)
end


# ╔═╡ cd62ded0-81bd-430d-8449-72a1d7a22e84
begin
 drawLine=(x)-> plot(x->1, ribbon =10,label=false)
 range=1:10
 arr=[]
 #plot(x->1,range,ribbon=3,label=false)
 for r in range
	 p=plot(x->r,range,label=false)
	 push!(arr,p)
...
# ╔═╡ 7a69396a-efe2-4756-a836-e3ef8bf9335f
md"## Parametric equations page112-example6"

# ╔═╡ 3d68aeed-6b7e-445e-8f9f-29c07542f091
md"""
...
# ╔═╡ 2097fc16-b250-45c5-adc4-aadcc065ee20
let
	x(t)=1+t
	y(t)=2-6t
	z(t)=6t
	range=-10:10
	plot(x,y,z,range,framestyle=:origin,label=false,lw=5,arrow=true)
end

# ╔═╡ c825eda9-7e2f-4653-a051-1afb73afdfab
let
	r=2
	x(t)=r*sin(t)
	y(t)=r*cos(t)
	range=0:0.02pi:2pi
	plot(x,y,range,framestyle=:origin,label=false,lw=4,scale=:none)
end

# ╔═╡ b9f1bdf9-6085-4ebf-86a4-0b29c92f246d
let
	# x(t,s)=1+t+s
	# y(t,s)=2-s
	# z(t,s)=s
	# xs = collect(0.1:0.05:2.0)
 #    ax = collect(0.2:0.1:2.0)
	# x_grid = [x for x = xs for y = ax]
 #    a_grid = [y for x = xs for y = ax]
	# plot(x_grid,a_grid,x.(x_grid,a_grid),y.(x_grid,a_grid),z.(x_grid,a_grid) , st = :surface)
end
