...
# ╔═╡ 33a9c212-33ce-11ed-3832-01791688cf1a
begin
	using StatsPlots,StatsBase,DataFrames,Latexify
end

# ╔═╡ 63f42dca-65ca-4cc3-8373-579143b86a07
begin
	x1,x2=[1,1,0,0],[1,0,1,0]
	sample=DataFrame(x1=x1,x2=x2)
end

# ╔═╡ 9280ad91-62c1-4d61-b0fa-fb8a6fc3e3d9
begin
	offset=0.1 # 避免标记重叠在一起
    xs=-0.2:0.1:2
	f(x)=1.2-0.5x
	cls(X)=-1.2+0.5X[1] +X[2]
	make_marker(X)=cls(X) >0 ? :cross :  :hline
	marker=[make_marker(X) for X in eachrow(sample)]
	plot(xs,f,label=false)
	scatter!(sample.x1,sample.x2,markershape=marker,label=false,ms=8)
	scatter!((sample.x1).+offset,sample.x2,label=false,markeralpha=0.5)
end

# ╔═╡ 4eb7ceda-c777-4fe9-95a1-da14ff8c4b6a
md"""
在上面的线性分类中, 落在直线下方的点标记为 $-$, 位于直线上方的点标记为 $+$


对于二元线性分类, 决策平面表示为一条直线.  $csl$ 函数可以归纳为:

$w_{0} +w_{1}x_{1} +w_{2}x_{2} =0$

在机器学习中就是要通过数据点寻找$w_{i}$ 的空间, 这里用空间, 因为不止一组值可以用来进行分类. 


"""

# ╔═╡ 163f1459-ef7d-4530-b876-b7c60856e01f
md"""

我们在机器学习软件包里调用的线性分类模型实际存储的就是这样一组$w$ 的参数.  
"""
...
julia_version = "1.8.0"
manifest_format = "2.0"
project_hash = "7753f9f7db0cbfbe71f5a09c8c5cd521a9ff7427"
