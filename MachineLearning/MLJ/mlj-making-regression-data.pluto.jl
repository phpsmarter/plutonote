...
# ╔═╡ 4566d7ab-18f8-49b2-be38-20593ebfff6b
begin
        using PlutoUI,MLJ, DataFrames,StatsPlots,StatsBase,RDatasets,ScientificTypes    
	    PlutoUI.TableOfContents()
end

# ╔═╡ 4a86c15d-0ee6-493a-b9a4-b26264064af3
md"""
## 1. 生成线性回归数据
[Regression-data-generated-from-noisy-linear-models
](https://alan-turing-institute.github.io/MLJ.jl/dev/generating_synthetic_data/#Regression-data-generated-from-noisy-linear-models)

"""

# ╔═╡ ef2f0e90-2be6-11ed-2662-5917aab4bddc
begin

X1, y1 = make_regression(100, 5; noise=0.5, sparse=0.2, outliers=0.1)
dfRegression = DataFrame(X1)
dfRegression.y = y1
first(dfRegression, 3)
end

# ╔═╡ 8bbe83c6-fc37-4bb6-a427-4e40e6d23903
md"""
## 2. load data from  RDatasets
  > 使用数据,boston housing.  收集了波士顿地区房屋的一些特征和平均价格, 在线性回归中可以使用特征数值来预测房价. 特征数值作为预测变量(predict variables), 房价作为响应变量(responsive variable)
"""

# ╔═╡ e7dc059e-c139-4335-851f-047fed40cc73
boston = dataset("MASS", "Boston");

# ╔═╡ ed5ca0b8-49fd-4e3e-a3d5-c2297cb3f1b5
schema(boston)  # using  ScientificTypes to show data type

# ╔═╡ a3f421e7-491b-4ccb-bdb2-ba8ebbcc2ccd
describe(boston)

# ╔═╡ 6031c96d-0786-4119-9f6e-c473102afde9
md"""## 3. using MLJ  ensemble model"""

# ╔═╡ 4bcfaeb2-dabb-44e1-9a83-8f35df232657
begin
...
# ╔═╡ 68505ba8-69bb-4ccd-89db-07f26b20ccfa
data = coerce(boston, autotype(boston, :discrete_to_continuous)); #由于很多计算方法要用到浮点类型, 执行这个操作将整数类型转化为浮点类型

# ╔═╡ 21662ae8-a3d0-4eb1-a12a-bc5963edb3ec
y,X = data.MedV,select(data, Not(:MedV))


# ╔═╡ 41b5d501-d199-4b70-9a19-29548b2ee3cc
mdl = LinearRegressor()  # 实例化线性模型

# ╔═╡ b7a679f1-69f0-448b-9897-4b5bdb87680e
begin
	mach = machine(mdl, X, y)  #使用数据生成模型
	fit!(mach)
...
# ╔═╡ ec666bcb-f4ee-4231-a489-9047dc84ae8c
begin
	ŷ = MLJ.predict(mach, X)
    round(rms(ŷ, y), sigdigits=4)
end

# ╔═╡ 9536e845-79cc-43db-a7a9-f410d4d47b60
begin
	n=size(boston)[1]
	range=1:n
    res = ŷ .- y     #   预测值和实际值逐一相减,得到每个数据点的残差集合
    plot(repeat((1:n)', 2),
         [zeros(1, n); res'], label = "", color = :grey, alpha = 0.5,size=(800,600))
	plot!(1:n, res, color = :grey, markershape = :circle,
          alpha = 0.5, label = "", linewidth = 0)
end

# ╔═╡ 5b317815-e715-4384-8c5a-82bdeafdd2e8
histogram(res,label=false,fc=:lightblue,fillalpha=0.5)  #残差频数统计柱形图

# ╔═╡ 3c150e2a-0fcc-44cf-82a9-fc926c1b7138
begin
	"""
	stem
	
	plot  residual stem plot
	
	```julia
	# res=y.-ȳ
	stem(res::Array,color=:grey)
	```
	"""
	function stem(res::Array,color=:grey)
	    bar(res,fillcolor=color,width=0.5,label=false,alpha=0.5,size=(800,600))
		plot!(1:length(res), res, color = color, markershape = :circle,
	          alpha = 0.5, label = "", linewidth = 0)
	
	end
end

# ╔═╡ 9caaa2a9-a081-4e4e-9273-246fd04801b5
stem(res)
...
julia_version = "1.8.0"
manifest_format = "2.0"
project_hash = "89d28c0fff2652d3bdffd79757ced3d621436407"
...
git-tree-sha1 = "7f6bedcaffb7d013dca3cb8067da6267e86b42e7"
...
git-tree-sha1 = "66ab866c0c214fb4bde679ae0af6255b90faa5f6"
...
git-tree-sha1 = "447ebc9030a620ad9b21f799a6d63824a7bf3a23"
...
[[deps.UnicodePlots]]
deps = ["ColorSchemes", "ColorTypes", "Contour", "Crayons", "Dates", "FileIO", "FreeTypeAbstraction", "LazyModules", "LinearAlgebra", "MarchingCubes", "NaNMath", "Printf", "Requires", "SparseArrays", "StaticArrays", "StatsBase", "Unitful"]
git-tree-sha1 = "20d15a9cf2f1e50b618ac61ffb39e220043863c8"
