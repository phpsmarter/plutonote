...
# ╔═╡ 146dc206-2a58-11ed-3a58-4d3c1a873415
begin
	using StatsPlots,DataFrames,RDatasets,Clustering,CSV
	using MLDataUtils:rescale!
end

# ╔═╡ c77784f8-6aab-44fe-9c5b-7a27d35176e9
# ╠═╡ disabled = true
#=╠═╡
begin
...
# p1=scatter(iris.PetalLength, iris.PetalWidth, marker_z=result.assignments,
#         color=:lightrainbow, legend=false)
# p2=scatter(iris.PetalLength, iris.PetalWidth, marker_z=iris[:,5],
#         color=:lightrainbow, legend=false)
# plot!(p1,p2,label=false)
end
  ╠═╡ =#

# ╔═╡ eb9673e6-0c24-4d1c-82ae-a6e8ba7924c8
begin
	url="/Users/lunarcheung/Public/github/StatsWithJuliaBook/data/iris.csv"
	data = CSV.read(url, DataFrame)
end

# ╔═╡ 846b3eef-b1e1-4f1b-9a6e-3e6ce87e35de
begin
 features = collect(Matrix(data[:, 1:4])'); # features to use for clustering
 rescale!(features)  # 对特征得分标准化
 result = kmeans(features, 3); # run K-means for the 3 clusters
end

# ╔═╡ 5a55f502-31cc-497b-a93a-bd8c9bdbde3f
begin
color=[:RdYlBu_5 :BuGn_3 :PRGn_4]
p1=scatter(data.petal_length, data.petal_width, marker_z=result.assignments,
        color=color, legend=false,title="k-means clustering")
p2=scatter(data.petal_length, data.petal_width, marker_z=data[:,6],
        color=color, legend=false,title="actual clustering")
plot!(p1,p2,label=false)
end
...
manifest_format = "2.0"
project_hash = "ca9294de0318d45128a76a128dc693c8a3666bb8"
...
git-tree-sha1 = "7fe76d94700a1e29a06509322fd114460bd92bb9"
