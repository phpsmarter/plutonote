...
# ╔═╡ ec29dae8-2deb-11ed-0219-0bc813330b49
begin
	using RDatasets,ScikitLearn,DataFrames,CSV,Latexify,StatsPlots,GLM
    @sk_import linear_model: LinearRegression
end

# ╔═╡ 7882a3f6-1edd-4d0b-9ca8-b5b308d0e751
begin
	path="/Users/lunarcheung/Public/github/StatsWithJuliaBook/data/cars.csv"
	data=CSV.read(path, DataFrame).|>float
	row,col=size(data)
	X,y=data[!,1]|>Array,data[!,2]|>Array
	X=[[x] for x in X]
end


# ╔═╡ 6aebbd1d-188d-46fe-b8b6-6f402b2343d1
data|>first

# ╔═╡ 2b6b23cf-c659-4085-9c78-ff2ff624701d
md"""
### 1. 使用 scikitlearn 拟合结果
"""

# ╔═╡ 0ad77ad4-fe94-4615-91a7-d153efdf08b7
model = LinearRegression()

# ╔═╡ 77a8553a-411e-4dd0-8769-2a6cde583c3d
reg=ScikitLearn.fit!(model, X, y);

# ╔═╡ b221eae5-aa77-453d-95d5-2bc9511678da
reg.score(X,y)

# ╔═╡ 34f1cef4-ec75-44cb-98c7-87a975662c5c
latexify([reg.predict(X)';y'],env=:mdtable)

# ╔═╡ 49e73bf2-e3a4-4b73-aca0-148069005572

line1(x)=reg.intercept_[1] + reg.coef_[1]*x

# ╔═╡ 9df3ff0d-6f4d-4724-9146-75ff772c59a6
begin
	scatter(data[!,1],data[!,:2],label=false, ms=2)
	plot!(0:25,line1, label=false )
end

# ╔═╡ b03198b1-5fe1-45f7-8c0d-a16e0dc4ddd3
md"""
### 2. 使用 GLM 拟合的结果
"""

# ╔═╡ 4c0a84db-70bc-4253-bf0e-4efe7d4882aa
ols = lm(@formula(dist~speed), data)

# ╔═╡ 45b1494a-4a2b-43d0-be83-7ba20f775e7e
line2(x)= coef(ols)'*[1, x]


# ╔═╡ c00db7be-6a1c-4f74-8e73-8ef0c64195b1
latexify([GLM.predict(ols)';data[!,:2]'],env=:mdtable)

# ╔═╡ c4728a1e-eccb-4342-962a-6dfe6bc243f8
begin
	scatter(data[!,1],data[!,:2],label=false, ms=2)
	plot!(0:25,line2, label=false )
end

# ╔═╡ 20305324-1ed4-4d4d-8829-14c36656a405
begin
	res2=ScikitLearn.fit!(model,[[0, 0], [1, 1], [2, 2]], [0, 1, 2])
end
...
julia_version = "1.8.0"
manifest_format = "2.0"
project_hash = "07ab6d4c9fc4d9ffa11f9a7952d4df88f43cb84e"
