...
# ╔═╡ cde117a4-2d05-11ed-0374-ed2b5c4225f4
begin
	using ScikitLearn,StatsPlots,StatsBase,DataFrames
	using RDatasets: dataset
end

# ╔═╡ c5830827-e964-42fe-963b-9e3f598797e4
begin
	iris = dataset("datasets", "iris");
	first(iris, 5)
end

# ╔═╡ 8ad57ca2-6b82-4555-9957-95e32ca99ce7
begin
	X = iris[!, [:SepalLength, :SepalWidth, :PetalLength, :PetalWidth]]|>Array
	y = iris[!, :Species]|>Array
end

# ╔═╡ faa31ae0-3f01-4ce0-9a68-3701c4393601
begin
	@sk_import linear_model: LogisticRegression
	model = LogisticRegression(fit_intercept=true, max_iter = 200)
	res=ScikitLearn.fit!(model, X, y)
end

# ╔═╡ 5f6a858e-ff5c-4b77-9ee8-e9ab68f02df9
begin
...
# ╔═╡ e2750517-f2be-4b08-bc87-69cdb0cf0b83
accuracy = ScikitLearn.score(model, X, y)

# ╔═╡ 6e13e041-c014-4d0c-9c7a-e2e38f2730f6
begin
tags=res.predict(X)
#insertcols!(iris,:tag=>Array(tags))
end

# ╔═╡ 864eb487-b2e1-48a7-9351-adad977bf754
begin
...
test_shape=[get_shape(x) for x in tags ]
test_color=[get_color(x) for x in tags ]
actual_shape=[get_shape(x) for x in  iris[!,:5]]
actual_color=[get_color(x) for x in  iris[!,:5]]
 #p1=scatter(iris[!,1],iris[!,2],mc=test_color,label=false,title="fit plot")
 p1= @df iris scatter(:SepalLength, :SepalWidth, group = :Species, title = "My awesome plot", xlabel = "Length", ylabel = "Width", marker = (0.5, [:cross :hex :star7], 12), bg = RGB(0.2, 0.2, 0.2))
 p2=scatter(iris[!,1],iris[!,2],mc=actual_color,label=false,title="actual plot")
 plot(p1,p2)
end
...
manifest_format = "2.0"
project_hash = "229c0dd0b6294c2e55f10631dbf10d97d0c7f6d7"
