...
# ╔═╡ 649bb5c0-2c1e-11ed-3a52-c3cda6d57a22

	begin
        using PlutoUI,MLJ, DataFrames,StatsPlots,StatsBase,RDatasets,ScientificTypes ,CSV,MultivariateStats
	PlutoUI.TableOfContents(title="Default  Classfier")
 url="/Users/lunarcheung/Public/github/StatsWithJuliaBook/data/default.csv"
end


# ╔═╡ c95eebf3-9534-4ec4-b6de-c1fc6105e63b
data=CSV.read(url,DataFrame).|>float|>d->first(d,8000)

# ╔═╡ de83671b-b258-4870-b5d5-59abe7b5a3ae
histogram(data.default,label=false)

# ╔═╡ cc924db6-77e3-4358-8162-ada86bc32c05
gdf0,gdf1 = groupby(data, :default)

# ╔═╡ a90acd9c-f174-456b-9813-626482295f6b
begin
	scatter(gdf0.balance,gdf0.income,shape=:circle,mc=:lightgreen,ma=0.5,label=false,ms=1)
	scatter!(gdf1.balance,gdf1.income,shape=:cross,mc=:red,label=false,ms=5)
end

# ╔═╡ 678a9933-5a37-45c4-bef1-0359c9b95581
begin
	X = Matrix(data[1:2:end,2:4])'
    X_labels = Vector(data[1:2:end,1])
end

# ╔═╡ cc7b3584-308b-46e2-ad45-dbf212f17402
lda = fit(MulticlassLDA, X, X_labels; outdim=2)

# ╔═╡ 4c63c8d9-5e54-44a0-af14-311aa8153ac4
Ylda = MultivariateStats.predict(lda, X)

# ╔═╡ 2e0ca48c-7e07-45c3-9a05-243519583d6b
begin
	points0 = Ylda[:,X_labels.==0]
	points1 = Ylda[:,X_labels.==1]
    scatter(points0[1,:],points1[2,:],shape=:circle,mc=:lightgreen,ma=0.5,label=false,ms=1)
scatter!(points1[1,:],points1[2,:],shape=:cross,mc=:red,label=false,ms=5)
...
manifest_format = "2.0"
project_hash = "1b9d2cf11f7d9bbaea40d59928cef1d2b968659c"
