...
# ╔═╡ 4ff71d64-c10f-11ec-0485-d5d3257b5c3c
begin
using Plots
...
# ╔═╡ 37a0d2f8-a177-42eb-9adc-8308518568aa
let
	f(x,y) = 2 - (x^2 + y^2)
	xs = ys = range(-2, stop=2, length=100)
      zs = [f(x,y) for y in ys, x in xs]
   gr()
   contour(xs, ys, zs, levels = [-1.0, 1.0])
end

# ╔═╡ 3f2225d1-6d91-4897-aefa-0d6bbb52a0d6
let
	gr()
a, b = -1, 2
...
# ╔═╡ 7c8433ad-2d45-4884-bb64-99936e3dbfb8
let
...
# ╔═╡ 023eea1c-91f9-4099-b441-b3544550a74b
let
	xs = ys = range(-pi/2, stop=pi/2, length=100)
f(x,y) = sinc(sqrt(x^2 + y^2))   # sinc(x) is sin(x)/x
contourf(xs, ys, f)
...
# ╔═╡ d718ce36-7d33-4a2f-baf7-77fff95c4a05
let
	xs = ys = range(-2, stop=2, length=100)
f(x,y) = 2 + x^2 + y^2
zs = [f(x,y) for y in ys, x in xs]
