...
# ╔═╡ d1bf2d89-8da0-4da5-823a-3edaf1679eb7
begin
	using AlgebraOfGraphics,CairoMakie,PlutoUI
	
end

# ╔═╡ 8698e44a-c071-11ec-2e95-0db1c4e21d38
md"# Algebraic graphic "

# ╔═╡ 9f6e9adf-a4c1-47ed-bfec-e1adae3fc18f
begin
x, y = collect(-8:0.4:8), collect(-8:0.4:8)
z = [ X^2+Y^2 for X ∈ x, Y ∈ y]

wireframe(x, y, z, axis=(type=Axis3,), color=(:red,0.3))
end

# ╔═╡ 70c302f0-7504-4dff-a525-7d2ccabb520c
let
x, y = collect(-8:0.4:8), collect(-8:0.4:8)
z = [-( X^2+Y^2) for X ∈ x, Y ∈ y]
wireframe(x, y, z, axis=(type=Axis3,), color=(:green,0.3))
x1=collect(-8:0.4:8)
y1=[x^2 for x in x]
...
# ╔═╡ 11cd3ea5-9492-4d55-931b-b013164a8191
let
	xs = LinRange(-4, 4, 10)
    ys = LinRange(-4, 4, 10)
	fz(x,y)=x^2+y^2
	zs = [x^2+y^2  for x in xs, y in ys]
    zs5=[2  for x in xs, y in ys]
	#surface!(xs, ys, zs, axis=(type=Axis3,))
	wireframe(xs, ys, zs5,  axis=(type=Axis3,))


end

# ╔═╡ 8716c0af-7b23-4e9b-8316-f34e86e8b557
let
x, y = collect(-8:0.4:8), collect(-8:0.4:8)
z = [ X^2+Y^2 for X ∈ x, Y ∈ y]

contour(x, y, z, axis=(type=Axis3,), color=:red)
end

# ╔═╡ ec991998-a197-4462-b21b-5f0d9cc1298c
let
	x, y = collect(-4:0.2:4), collect(-4:0.2:4)
    z = [sqrt(X^2+Y^2) for X ∈ x, Y ∈ y]

    wireframe(x, y, z, axis=(type=Axis3,), color=(:green,0.8))
	surface(x, y, z, axis=(type=Axis3,), color=(:green,0.8),overdraw = true, transparency = true)
end

# ╔═╡ cbc6931f-b55f-48f0-80a5-c491d0ad302b
let
    x = y = LinRange(-2, 2, 50)
    z = [sqrt(X^2+Y^2) for X ∈ x, Y ∈ y]
...
# ╔═╡ 0c8c8d99-04ad-4491-9caa-d59b8dfcb8a9
let
	x, y = collect(-4:0.2:4), collect(-4:0.2:4)
    z = [sqrt(X^2+Y^2) for X ∈ x, Y ∈ y]

    contour(x, y, z, axis=(type=Axis3,), color=:blue)
	#contour(x, y, z, color=:blue)
end

# ╔═╡ 96aac4aa-566e-45d9-ad69-1f291ff598ec
let
	x, y = collect(0:0.2:6), collect(-3:0.2:3)
    z = [2X+3Y+1 for X ∈ x, Y ∈ y]

    contour(x, y, z, color=:blue)
	#contour(x, y, z, axis=(type=Axis3,),color=:blue)
end

# ╔═╡ 57dca12a-886b-4877-a0c0-ab906dfbc892
let
    x = y = LinRange(-1, 1, 50)
	f1(x,y)=x^2+y^2
	f2(x,y)=x^3+0.5*y
    z = [ f1(X,Y) for X ∈ x, Y ∈ y]
    zmin, zmax = minimum(z), maximum(z)
    cmap = :viridis
    set_theme!(theme_dark())
    fig = Figure(resolution = (1200, 800), fontsize = 22)
...
# ╔═╡ b6232880-7cac-4117-8e9b-c4d537a7a8fc
let
    x = y = LinRange(-1, 1, 50)
    z = [-( X^2+Y^2) for X ∈ x, Y ∈ y]
    zmin, zmax = minimum(z), maximum(z)
...
# ╔═╡ 6bb6b39d-5820-4c59-9c98-46d825f6fbb8
let
    x = y = LinRange(-1, 1, 50)
    z = [-( X^2+Y^2) for X ∈ x, Y ∈ y]
	 z1 = [-0.5 for X ∈ x, Y ∈ y]
    ...
    xm, ym, zm = minimum(ax.finallimits[])
    wireframe!(ax, x, y, z,color = (:red, 0.9),overdraw = false)
...
# ╔═╡ c10e0be8-7b7a-462b-974d-880bce00260a
let
    x = y = LinRange(-0.5, 0.5, 50)
	y = LinRange(0.001, 0.002, 2)
    z = [-( X^2+Y^2) for X ∈ x, Y ∈ y]
	 z1 = [-0.5 for X ∈ x, Y ∈ y]
    zmin, zmax = minimum(z), maximum(z)
    cmap = :viridis
...
# ╔═╡ 86db8795-85ea-49d0-bf21-93da7196aee4
let
    y= LinRange(-0.5, 0.5, 50)
	x = LinRange(0.001, 0.0011, 2)
    z = [-( X^2+Y^2) for X ∈ x, Y ∈ y]
	cmap = :viridis
    set_theme!(theme_dark())
    fig = Figure(resolution = (1200, 800), fontsize = 22)
...
# ╔═╡ 65746c8c-6c84-4daf-8d22-7717eeab50d4
let
    x = y = LinRange(-0.1,0.1, 50)
    z = [-( X^2+Y^2) for X ∈ x, Y ∈ y]
	 z1 = [-0.5 for X ∈ x, Y ∈ y]
    zmin, zmax = minimum(z), maximum(z)
    cmap = :viridis
...
# ╔═╡ 33edd7d1-257f-41cd-8474-bf6c0c70a010
let
    x = y = LinRange(-0.05,0.05, 50)
    z = [-( X^2+Y^2) for X ∈ x, Y ∈ y]
	 z1 = [-0.5 for X ∈ x, Y ∈ y]
    zmin, zmax = minimum(z), maximum(z)
    cmap = :viridis
...
# ╔═╡ 9266d2e7-635e-4681-acb9-c421b04318d7
let
    x = y = LinRange(-0.005,0.005, 50)
    z = [-( X^2+Y^2) for X ∈ x, Y ∈ y]
	 z1 = [-0.5 for X ∈ x, Y ∈ y]
    zmin, zmax = minimum(z), maximum(z)
    cmap = :viridis
...
# ╔═╡ 3641eef4-3e5a-41c1-82bc-b5c3c81ce7a2
let
    x = y = LinRange(-1, 1, 50)
    z = [X^2-Y^2 for X ∈ x, Y ∈ y]
    zmin, zmax = minimum(z), maximum(z)
...
# ╔═╡ 68ce37c9-c62a-4ef9-801a-2bd410e5dfcb
let
    x = y = LinRange(-1, 1,100)
    z = [X^2+X*Y+Y^2 for X ∈ x, Y ∈ y]
    zmin, zmax = minimum(z), maximum(z)
    cmap = :viridis
    set_theme!(theme_dark())
    fig = Figure(resolution = (1200, 800), fontsize = 22)
...
# ╔═╡ 63ade3ee-dd15-4082-80ec-3c419187e87d
let
    x = y = LinRange(-1, 1, 50)
	f1(x,y)=x^2+y^2
	f2(x,y)=x^3+0.5*y
